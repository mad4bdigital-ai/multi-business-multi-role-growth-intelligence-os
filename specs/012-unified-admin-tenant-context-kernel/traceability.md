...
| Credential boundary and two-stage readiness | FR-027, FR-056 to FR-057 | pre-credential gates → guarded materialization → provider readiness | no-secret, premature-materialization rejection, credential-validity/reachability, membership/scope/revision invalidation tests | credential-boundary and readiness gate |
...
| OAuth authorization and reconnect state | FR-059 to FR-060, NFR-010 | signed, expiring, nonce-bound, single-use, redirect-allowlisted exact-context state; reconnect target/revision/account binding | signature, replay, expiry, redirect, connection-revision, provider-account, and context-mismatch tests | OAuth security release gate |
