...
- zero mutable-location, filename, alias, or timestamp accepted as sufficient high-risk Artifact version identity;
- zero canonical/stored checksum substitution or in-place mutation of an immutable Artifact/Knowledge Version;
- required source identity, signer scope, attestation, transparency inclusion/root/witness, expiry, and revocation validation passes for critical Artifact families;
- zero signature-valid content promoted as fact or publication-eligible without independent claim/trust/policy gates;
- zero prohibited provenance cycle, forged cross-Tenant derivation, or hidden material contradiction accepted;
- every consequential citation resolves to an exact authorized Source Version and valid locator, and every high-risk factual claim satisfies its support policy;
- zero composite trust-score override of mandatory authenticity, factual-support, license, policy, audience, freshness, review, or retraction gate;
- derived Policy Envelopes are never less restrictive without verified approved declassification evidence;
- selective-disclosure projections expose no unauthorized content/identity/graph shape and never represent omitted material evidence as complete;
- every certified reproducible build records exact source/handler/model/prompt/parameter/environment/canonicalization evidence and honest comparison classification;
- zero unauthorized or mutable-alias Source/Chunk/Embedding membership in an active Knowledge Index Version;
- retrieval gate-before-rank and generated claim-to-citation support tests have zero retracted/stale/private/unlicensed source bypass;
- correction creates a new Version and retraction/attestation revocation propagates to affected indexes, caches, manifests, queued pre-publication work, memory, evaluations, exports, provider copies, backups, and promotion candidates within the approved emergency SLO;
- erasure/legal-hold tests prove minimal tombstones contain no erased payload and holds grant no read authority;
- zero cross-Tenant Artifact/Version/Source/Claim/Citation/provenance/trust/index/retrieval/checksum/opaque-reference access or inference;
- compatibility backfill invents no source, signature, license, verification, freshness, claim support, ownership, or trust evidence;
- artifact eligibility/correction/retraction/disposition/index-build/reproduction/provenance-projection previews prove zero content write, signing, transformation, publication, correction, retraction, deletion, indexing/embedding, provider/model call, credential read, invalidation, notification, or external write;
