...
- **FR-201:** Every Artifact has one stable logical identity and one or more immutable content-addressed versions; mutable locations, filenames, aliases, or timestamps are never sufficient version identity.
- **FR-202:** Each Artifact Version binds artifact/type/schema versions, canonical-content checksum, stored-representation checksum, media/encoding metadata, source/version vector, producer, runtime manifest, policy envelope, and governance epoch.
- **FR-203:** A content change, correction, redaction, translation, or new representation that changes canonical semantics creates a new immutable Artifact Version rather than mutating a published version.
- **FR-204:** Source identity, content integrity, publisher authenticity, provenance completeness, factual support, methodological quality, freshness, license compatibility, policy eligibility, reproducibility, human review, and publication approval remain separate evidence dimensions.
- **FR-205:** Source, ingestion, transformation, verification, publication, correction, retraction, and disposition attestations bind exact versions, signer identity/authority, signed checksum, algorithm/key reference, issuance/expiry/revocation, trust domain, and verification result without storing raw signing secrets.
- **FR-206:** Critical Artifact families maintain tamper-evident ordered transparency entries and periodic Merkle roots or equivalent append-only proofs; invalid or missing required proofs block eligible use.
- **FR-207:** Claims are addressable versioned objects with exact location, semantic type, canonical checksum, effective context, confidence evidence, review state, and usage limits.
- **FR-208:** Claim relations preserve support, contradiction, derivation, qualification, equivalence, supersession, aggregation, and context validity; conflicting evidence cannot be hidden by a single aggregate score.
- **FR-209:** Consequential citations target exact immutable Source Versions and validated locators such as page/line/range, JSON Pointer, row/key, timestamp, image region, chunk, or immutable repository commit/path/range.
- **FR-210:** Floating URLs, latest aliases, mutable queries, or current files are insufficient high-risk evidence unless the exact observed version/content is captured and checksummed.
- **FR-211:** Trust policies are versioned by task, risk, publication, audience, and use case and declare mandatory dimensions, thresholds, confidence, freshness, review, corroboration, and zero-tolerance conditions.
- **FR-212:** Composite trust scores may rank already eligible Artifacts but cannot override a failed mandatory trust, policy, data, license, audience, freshness, or retraction gate.
- **FR-213:** Reproducibility manifests bind exact ordered source versions, transformation definition, handler/build digest, model/provider/version/profile, prompt/template checksum, parameters, environment/tool dependencies, runtime manifest, canonicalization rules, and recorded nondeterministic inputs.
- **FR-214:** Reproduction runs classify results as bit-reproducible, semantically reproducible, bounded nondeterministic, or not reproducible and preserve difference/tolerance/reviewer evidence without rewriting the original build.
- **FR-215:** Every Artifact Version resolves an immutable Policy Envelope covering classification, purpose/lawful basis/consent, audience, ownership/license, attribution, transformations, model/provider use, residency/transfer, retention, legal hold, subject restrictions, publication, and export.
- **FR-216:** Derived policy is at least as restrictive as every applicable source and destination policy unless an approved declassification/redaction/anonymization transformation and verification proves a lawful narrower envelope.
- **FR-217:** Access authority never implies license permission, license never implies object authority, authenticity never implies truth, factual support never implies publication approval, and verification never implies unrestricted reuse.
- **FR-218:** Provenance supports registered public, Tenant, operator, auditor, legal, regulator, and future selective-disclosure projections that expose only authorized evidence while preserving material contradictions and disclosure omissions.
- **FR-219:** Knowledge sources, chunks, embeddings, indexes, and retrieval evidence are versioned Artifacts linked to exact source/build/model/profile/policy versions and cannot be silently replaced through mutable aliases.
- **FR-220:** Each Knowledge Chunk Version binds exact Source Version and locator; each Embedding Version binds exact Chunk Version, preprocessing, embedding model/profile, dimensions, distance semantics, and checksum.
- **FR-221:** Each Knowledge Index Version binds exact eligible source membership, filter decisions, normalization/redaction/chunking profiles, chunk/embedding versions, retrieval/reranking policies, index checksum, classification/audience/license/residency, freshness, and invalidation state.
- **FR-222:** Consequential retrieval records query/intent checksum, Tenant/principal/context/purpose, eligible and excluded index/source versions, retrieved chunks/locators, scores/reranking, citations/claims, manifest, trust/freshness decisions, and governance epochs.
- **FR-223:** Retrieval ranking cannot override object authority, purpose, data, license, audience, freshness, retraction, correction, trust, or verification gates.
- **FR-224:** Generated consequential answers link each factual claim to retrieval/citation evidence or explicitly classify it as unsupported, assumption, opinion, recommendation, estimate, or prediction.
- **FR-225:** Corrections create new immutable versions and explicit corrects/supersedes relations; historical outputs retain their original source versions and receive impact/review status rather than silent rewriting.
- **FR-226:** Retraction prevents new eligible use, advances the Artifact governance epoch, preserves permitted historical evidence, and invalidates affected indexes, caches, manifests, retrieval sets, promotion candidates, and dependent use.
- **FR-227:** Retraction is distinct from erasure; erasure/disposition follows DFR-003 and may retain only a permitted minimal no-content tombstone while propagating delete, rebuild, invalidate, retract, anonymize, aggregate, archive, hold, or notify actions.
- **FR-228:** Correction, restriction, consent withdrawal, retraction, and erasure propagation covers summaries, chunks, embeddings, indexes, Agent memory, evaluation datasets, analytics, reports, exports, caches, provider copies, backups, and promotion candidates.
- **FR-229:** Artifact/knowledge eligibility resolves exact version, object authority, purpose/data use, integrity/authenticity, provenance/contradiction, mandatory trust dimensions, freshness/effective time, audience/selective disclosure, license, residency, retention/legal hold, dependency invalidation, manifest, and governance epoch.
- **FR-230:** The Effective Runtime Manifest binds exact Artifact/Source/Claim/Citation/Chunk/Embedding/Index versions, checksums, attestations/transparency proofs, provenance graph checksum, transformation/reproducibility manifests, trust/policy/freshness decisions, retrieval evidence, correction/retraction/disposition state, epoch, and expiry.
- **FR-231:** Before consequential use or publication, runtime revalidates version existence, checksum, source/attestation revocation, trust/freshness, policy/license/audience eligibility, correction/retraction/invalidation state, and governance epoch.
- **FR-232:** Artifact/knowledge registries may select only allowlisted schemas, verification methods, transformation handlers, signing providers, storage adapters, model profiles, and policy operators and cannot inject arbitrary code, SQL, shell, URLs, headers, or credential values.
- **FR-233:** Current `output_artifacts`, `json_assets`, subject/scope links, platform graph, session Drive artifacts, summaries, knowledge surfaces, and execution evidence remain compatibility sources until family-specific provenance/trust/policy parity, propagation, rollback, and cutover pass.
- **FR-234:** Backfill never invents source, signature, license, verification, freshness, claim support, or provenance; incomplete and unknown evidence remains explicit and cannot be promoted silently.
- **FR-235:** Missing, stale, conflicting, revoked, unsupported, insufficient, ambiguous, or unregistered Artifact, Source, Claim, Citation, Attestation, Trust, Policy, Reproducibility, Knowledge Build, Retrieval, Correction, Retraction, Disposition, or governance evidence fails closed with stable structured errors.
