...
يفصل بين `inventory_only` و`dry_run_ready` و`apply_supported` و`sandbox_certified` و`production_certified`. وجود Inventory Function لا يعني Apply Support.
...
WooCommerce Standard يمكن أن يملك المجالات المحددة مثل Catalog وInventory وCheckout وOrders، والمنصة توفر الحوكمة والمحتوى وSEO والقياس والتشغيل والملفات والنمو والتنبيهات والمصالحة. هذا النمط لا يدعي Atomic Unique-item Reservation بين WooCommerce وPOS وLive.

Governed Bridge Plugin تضيف Brand/Site identity وSigned Requests وReplay Protection وIdempotency وExpected Version وReservation/Release/Commit وOperation Ledger وReadback وWebhook Inspection.

WooCommerce + ERPNext يدعم إما Woo authority مع ERP downstream mirror/accounting، أو ERP authority مع Woo projection وmediated checkout. لا يسمح للنظامين بالكتابة في المجال نفسه.
...
الاتصالات تكون `brand_owned` أو `workspace_delegated` أو `platform_managed`، ولا يوجد Silent Fallback بين Brand وWorkspace وPersonal أو بين البراندات.
