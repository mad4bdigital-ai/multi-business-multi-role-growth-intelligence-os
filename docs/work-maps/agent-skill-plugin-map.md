# Agent, Skill, Plugin, and Intelligence Map

> Generated text documentation. Do not edit this file manually.
> Source hash: `d8a8c0ce1b98ae025a6d6dca04a002f02e5b3503b317949bae8d20c24d9483e3`
> Source count: **26**
> Sources: `http-generic-api/migrations/009_sprint05_logic_graph.sql`, `http-generic-api/migrations/016_sprint20_agent_registry.sql`, `http-generic-api/migrations/017_sprint21_output_sink_router.sql`, `http-generic-api/migrations/019_sprint24_workflow_seeds_and_skills.sql`, `http-generic-api/migrations/021_sprint26_dev_agent_growth_loop.sql`, `http-generic-api/migrations/125_sprint64_platform_plugin_contributions.sql`, `http-generic-api/migrations/126_sprint64_platform_plugin_private_runtime.sql`, `http-generic-api/migrations/130_sprint64_summary_development_automation.sql`, `http-generic-api/migrations/134_sprint64_summary_development_agent_approvals.sql`, `http-generic-api/migrations/137_sprint64_openclaude_provider_registry.sql`, _... +16 more_
> Rendering: Mermaid plus Markdown tables; no image assets or raw database rows are generated.

```mermaid
flowchart TD
  subgraph OperatingFlow["Governed operating flow"]
  c_0["Intent"]
  c_1["Agent resolution"]
  c_2["Skill grants"]
  c_3["Logic / engine"]
  c_4["Model policy"]
  c_5["Tool / plugin binding"]
  c_6["Execution evidence"]
  c_0 --> c_1
  c_1 --> c_2
  c_2 --> c_3
  c_3 --> c_4
  c_4 --> c_5
  c_5 --> c_6
  end
  subgraph DiscoveredSchema["Discovered schema objects"]
  n_agent_delegations["agent_delegations<br/>table"]
  n_platform_plugin_smoke_certifications["platform_plugin_smoke_certifications<br/>table"]
  n_agent_skill_grants["agent_skill_grants<br/>table"]
  n_agent_supervision_policy["agent_supervision_policy<br/>table"]
  n_agent_chain_events["agent_chain_events<br/>table"]
  n_agent_logic_pack_bindings["agent_logic_pack_bindings<br/>table"]
  n_agent_tool_bindings["agent_tool_bindings<br/>table"]
  n_agent_workflow_bindings["agent_workflow_bindings<br/>table"]
  n_dev_agent_proposals["dev_agent_proposals<br/>table"]
  n_logic_definitions["logic_definitions<br/>table"]
  n_logic_packs["logic_packs<br/>table"]
  n_platform_engine_execution_runs["platform_engine_execution_runs<br/>table"]
  n_platform_plugin_contributions["platform_plugin_contributions<br/>table"]
  n_platform_plugin_smoke_recertification_policies["platform_plugin_smoke_recertification_policies<br/>table"]
  n_v_platform_plugin_collation_issues["v_platform_plugin_collation_issues<br/>view"]
  n_agent_handoff_state_access_log["agent_handoff_state_access_log<br/>table"]
  n_agent_handoff_state_registry["agent_handoff_state_registry<br/>table"]
  n_agent_model_runs["agent_model_runs<br/>table"]
  n_agent_response_profile_registry["agent_response_profile_registry<br/>table"]
  n_agent_skills["agent_skills<br/>table"]
  n_agent_tool_calls["agent_tool_calls<br/>table"]
  n_agent_tool_index["agent_tool_index<br/>table"]
  n_agents["agents<br/>table"]
  n_ai_model_providers["ai_model_providers<br/>table"]
  n_ai_model_registry["ai_model_registry<br/>table"]
  n_dev_agent_provider_registry["dev_agent_provider_registry<br/>table"]
  n_dev_agent_runs["dev_agent_runs<br/>table"]
  n_dev_agent_runtime_provider_profiles["dev_agent_runtime_provider_profiles<br/>table"]
  n_dev_agent_runtime_registry["dev_agent_runtime_registry<br/>table"]
  n_external_prompt_artifact_registry["external_prompt_artifact_registry<br/>table"]
  n_intelligence_engines["intelligence_engines<br/>table"]
  n_platform_engine_policy_registry["platform_engine_policy_registry<br/>table"]
  n_platform_engine_policy_rules["platform_engine_policy_rules<br/>table"]
  n_platform_engine_registry["platform_engine_registry<br/>table"]
  n_platform_engine_skill_prompt_registry["platform_engine_skill_prompt_registry<br/>table"]
  n_platform_engine_strategy_registry["platform_engine_strategy_registry<br/>table"]
  n_platform_engine_validator_result_log["platform_engine_validator_result_log<br/>table"]
  n_platform_orchestration_plugins["platform_orchestration_plugins<br/>table"]
  n_policy_logic_bindings["policy_logic_bindings<br/>table"]
  n_policy_logic_mirror_classification["policy_logic_mirror_classification<br/>table"]
  n_repo_skill_candidates["repo_skill_candidates<br/>table"]
  n_skill_manifests["skill_manifests<br/>table"]
  n_summary_development_agent_approvals["summary_development_agent_approvals<br/>table"]
  n_v_activation_agent_bindings_catalog["v_activation_agent_bindings_catalog<br/>view"]
  n_v_activation_agent_catalog["v_activation_agent_catalog<br/>view"]
  n_agent_delegations --> n_agents
  n_agent_logic_pack_bindings --> n_agents
  n_agent_skill_grants --> n_agents
  n_agent_supervision_policy --> n_agents
  n_agent_tool_bindings --> n_agents
  n_agent_workflow_bindings --> n_agents
  end
```

## Discovered object inventory

| Object | Type | Domain | Source migrations | Columns | References |
|---|---|---|---:|---:|---|
| `agent_chain_events` | table | Agents & intelligence | 1 | 12 | `tenants` |
| `agent_delegations` | table | Agents & intelligence | 1 | 13 | `agents`, `plans`, `tenants`, `users` |
| `agent_handoff_state_access_log` | table | Agents & intelligence | 1 | - | - |
| `agent_handoff_state_registry` | table | Agents & intelligence | 1 | - | - |
| `agent_logic_pack_bindings` | table | Agents & intelligence | 1 | 5 | `agents` |
| `agent_model_runs` | table | Agents & intelligence | 1 | - | - |
| `agent_response_profile_registry` | table | Agents & intelligence | 1 | - | - |
| `agent_skill_grants` | table | Agents & intelligence | 1 | 10 | `agents`, `tenants` |
| `agent_skills` | table | Agents & intelligence | 1 | 11 | - |
| `agent_supervision_policy` | table | Agents & intelligence | 1 | 6 | `agents`, `tenants` |
| `agent_tool_bindings` | table | Agents & intelligence | 1 | 5 | `agents` |
| `agent_tool_calls` | table | Agents & intelligence | 1 | - | - |
| `agent_tool_index` | table | Agents & intelligence | 1 | - | - |
| `agent_workflow_bindings` | table | Agents & intelligence | 1 | 5 | `agents` |
| `agents` | table | Agents & intelligence | 1 | 16 | - |
| `ai_model_providers` | table | Agents & intelligence | 1 | - | - |
| `ai_model_registry` | table | Agents & intelligence | 1 | - | - |
| `dev_agent_proposals` | table | Repository & development | 1 | 19 | `tenants` |
| `dev_agent_provider_registry` | table | Agents & intelligence | 1 | - | - |
| `dev_agent_runs` | table | Agents & intelligence | 1 | 10 | - |
| `dev_agent_runtime_provider_profiles` | table | Agents & intelligence | 1 | - | - |
| `dev_agent_runtime_registry` | table | Agents & intelligence | 1 | - | - |
| `external_prompt_artifact_registry` | table | Agents & intelligence | 1 | - | - |
| `intelligence_engines` | table | Agents & intelligence | 1 | - | - |
| `logic_definitions` | table | Agents & intelligence | 1 | 12 | `tenants` |
| `logic_packs` | table | Agents & intelligence | 1 | 11 | `tenants` |
| `platform_engine_execution_runs` | table | Agents & intelligence | 2 | - | - |
| `platform_engine_policy_registry` | table | Agents & intelligence | 1 | - | - |
| `platform_engine_policy_rules` | table | Agents & intelligence | 1 | - | - |
| `platform_engine_registry` | table | Agents & intelligence | 1 | - | - |
| `platform_engine_skill_prompt_registry` | table | Agents & intelligence | 1 | - | - |
| `platform_engine_strategy_registry` | table | Agents & intelligence | 1 | - | - |
| `platform_engine_validator_result_log` | table | Agents & intelligence | 1 | - | - |
| `platform_orchestration_plugins` | table | Agents & intelligence | 1 | 17 | - |
| `platform_plugin_contributions` | table | Agents & intelligence | 2 | 22 | - |
| `platform_plugin_smoke_certifications` | table | Agents & intelligence | 2 | 26 | `tenants`, `users` |
| `platform_plugin_smoke_recertification_policies` | table | Agents & intelligence | 1 | 18 | `tenants` |
| `policy_logic_bindings` | table | Agents & intelligence | 1 | 12 | - |
| `policy_logic_mirror_classification` | table | Agents & intelligence | 1 | - | - |
| `repo_skill_candidates` | table | Repository & development | 1 | - | - |
| `skill_manifests` | table | Agents & intelligence | 1 | - | - |
| `summary_development_agent_approvals` | table | Repository & development | 1 | - | - |
| `v_activation_agent_bindings_catalog` | view | Activation & onboarding | 1 | - | - |
| `v_activation_agent_catalog` | view | Activation & onboarding | 1 | - | - |
| `v_activation_agent_skill_catalog` | view | Activation & onboarding | 1 | - | - |
| `v_activation_agent_skill_grants` | view | Activation & onboarding | 1 | - | - |
| `v_activation_agent_tool_catalog` | view | Activation & onboarding | 1 | - | - |
| `v_activation_logic_pack_catalog` | view | Activation & onboarding | 1 | - | - |
| `v_activation_platform_plugin_catalog` | view | Activation & onboarding | 1 | - | - |
| `v_activation_plugin_contributions` | view | Activation & onboarding | 1 | - | - |
| `v_activation_skill_manifest_catalog` | view | Activation & onboarding | 1 | - | - |
| `v_activation_skill_package_catalog` | view | Activation & onboarding | 1 | - | - |
| `v_agent_runtime_ledger_counts` | view | Agents & intelligence | 1 | - | - |
| `v_agent_runtime_ledger_quality` | view | Agents & intelligence | 1 | - | - |
| `v_agent_runtime_ledger_readiness` | view | Agents & intelligence | 1 | - | - |
| `v_platform_engine_validator_latest_failures` | view | Agents & intelligence | 1 | - | - |
| `v_platform_engine_validator_result_summary` | view | Agents & intelligence | 1 | - | - |
| `v_platform_plugin_collation_issues` | view | Agents & intelligence | 2 | - | - |
| `v_policy_logic_mirror_classification_detail` | view | Agents & intelligence | 1 | - | - |
| `v_policy_logic_mirror_classification_summary` | view | Agents & intelligence | 1 | - | - |
| `v_skill_runtime_coverage` | view | Agents & intelligence | 1 | - | - |

## Coverage counters

- Matching schema objects: **61**
- Diagram objects shown: **45**
- Source migrations: **26**
