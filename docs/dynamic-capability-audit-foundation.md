...
never recorded in SQL. `v_dynamic_audit_pipeline_readiness` reports scheduler
freshness, key-level bridge lag, all-source rollup lag, repo audit coverage,
Drive readback coverage, checkpoint completion, DB semantic quality, duplicate
keys, and secret/evidence violations.
