...
| GitHub REST fallback PR state patch support | `adminCliRoutes.js` plus this governance note. The fallback may map `PATCH /pulls/{number}` and `PATCH /issues/{number}` only when the body contains `state=open` or `state=closed`; no arbitrary issue/PR mutation, no secret return, and no protected branch write is authorized by this mapping. |
| Session Insight target-write post-write readback | `openapi.yaml`, `Updating Registry Patch Index.md`, `deployment_parity_checklist.md`, Docs Agent governance, generated surface-contract docs, targeted readback tests, tool registry migration, and explicit read-only/no-target-mutation/no-rollback/no-provider/no-secret evidence |
