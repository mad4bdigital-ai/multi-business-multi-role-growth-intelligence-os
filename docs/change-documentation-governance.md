...
| Session Insight target-write post-write readback | `openapi.yaml`, `AI_Agent_Knowledge_Guide.md`, `Updating Registry Patch Index.md`, `deployment_parity_checklist.md`, `docs/change-documentation-governance.md`, generated surface-contract docs, targeted readback tests, tool registry migration, and explicit read-only/no-target-mutation/no-rollback/no-provider/no-secret evidence |
