...
For migration `285_sprint68_session_insight_target_write_readback.sql`, update `AI_Agent_Knowledge_Guide.md`, `Updating Registry Patch Index.md`, `deployment_parity_checklist.md`, this governance note, and OpenAPI route coverage. Documentation must state that target-write readback is read-only post-write validation: it may record readback evidence, but it must not create target writes, modify target items, execute rollback, call providers, read credential payloads, perform external writes, return raw transcripts, or include secrets.
