...
Route classifier remediation must document route class semantics before changing OpenAPI scoring: `http_route` remains OpenAPI-required, while `admin_tool_registry_route`, `tenant_tool_registry_route`, `system_tool_dispatch_route`, and `registry_only_surface` are evidence-only exemptions with reasons. External Delivery admin controls in `955_sprint68_external_delivery_admin_control_surface.sql` use `admin_platform_endpoint_tools` rows and must be classified as registry-governed controls rather than missing standalone OpenAPI paths.
