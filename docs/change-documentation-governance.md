...
Post-lock future-surface remediation must explicitly name `307_sprint69_hostinger_deploy_restart_option_support.sql` and `1001_sprint68_tenant_ticket_admin_gpt_link_support.sql` when they appear in surface queues. Migration 307 is governed registration metadata only for deploy/restart support and remains non-executing until approval, capability envelope, feature flags, audit, and readback. Migration 1001 is additive support-ticket/Admin GPT link support only and must not replace tenant ticket routes or perform external sends, provider writes, credential payload reads, raw secret handling, deploys, or secret return.
