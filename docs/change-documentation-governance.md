...
> Change-governance contract for `20260728_brand_scoped_user_skill_activation.sql`: review must confirm the migration remains additive, SQL-primary, and non-destructive; creates only `brand_skill_policies`, `user_brand_skill_grants`, and `v_effective_user_brand_skill_grants`; preserves baseline `agent_skill_grants`; and does not seed policies or activate skills. Apply must remain a separate checksum-bound governed migration with schema/view and effective-grant readback. Review must preserve `no_provider_call`, `no_credential_payload_read`, `no_raw_secrets`, `no_external_send`, `no_external_write`, and `secrets_included=false`, and reject any claim that merge or documentation authorizes migration apply, deployment, provider execution, or production grants.
