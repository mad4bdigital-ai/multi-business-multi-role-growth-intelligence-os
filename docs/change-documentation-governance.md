...
Legacy surface backlog closure may be used only for explicit historical ranges/prefixes that have already passed release readiness and are no longer blocking current runtime promotion. `surface-contract-legacy-backlog-closure-v1` currently covers migration ranges `001-291`, `305`, `900-910`, `950-960`, `997-999`, and `20260611_*`. It marks historical docs/OpenAPI/safety backlog as reviewed while preserving future detection for migrations outside the closure. Closure evidence must remain documentation/classification only and must not authorize provider calls, credential payload reads, runtime mutation, database writes, external sends, deploys, or secrets.
