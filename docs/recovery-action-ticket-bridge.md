...
`production_live` is opt-in and remains fail-closed unless the strict Hostinger Production runtime identity, `RECOVERY_SERVER_MANAGED_BINDING_MODE=production_live`, and a configured server-managed binding module all resolve together. The server composition root calls `createProductionRecoveryComposition()` through `productionRecoveryCompositionFactory.js`; the factory accepts no caller adapters, credentials, tickets, or provider controls. A Production request without a provider, a mode-mismatched envelope, an incomplete authority graph, a non-independent store, or an invalid live authorization returns the fail-closed composition without opening a target database connection. The live mode becomes active only when `mad4b.recovery-live-authority-readiness.v2` derives every required component and binding gate as ready: `recoveryStore`, `executionTicketSigner`, `approvalVerifier`, `recoveryLock`, `readbackVerifier`, `hostLocalMutationExecutor`, and `deploymentIdentityProvider`, plus mutation-grade durability, exact-SHA authorization, server-side approval resolution, and same-cycle readback. The default mode remains `disabled`.
