...
- fetch exact pull-request metadata before delegation consumption, require numeric `changed_files`, reject targets above GitHub's 3,000-file pull-request-files API cap, and require the successfully fetched filename count to equal `changed_files`;
- treat pull-request file API, authentication, pagination, metadata, or inventory-completeness failures as fatal before Recovery delegation consumption;
- derive any dynamic binding authority only from the complete exact-target filename inventory, then require `specs/[0-9][0-9][0-9]-[a-z0-9][a-z0-9-]*/work-map-integration.json` producer-compatible syntax, exact manifest `feature_key` identity, `review_state=ready_for_implementation`, and file existence before delegation consumption;
