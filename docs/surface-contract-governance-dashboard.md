# Surface Contract Governance Dashboard

- Discovery schema: surface-contract-discovery-v3
- Queue schema: surface-contract-gap-queue-v1
- Triage schema: surface-contract-gap-triage-v1
- Gate schema: surface-contract-new-gap-gate-v1
- Queue items: 5
- Triaged items: 5
- Gate candidates: 2
- New-gap gate: fail
- Blocking new items: 3
- Docs completion: 98.93%
- SQL route OpenAPI coverage: 96.88%

## Top immediate items

- `20260808_github_issue_comment_dispatch_parity.sql` — critical_review, classify route-like strings, then add OpenAPI contract or false-positive exemption
- `1050_github_repository_policy_controller_bootstrap_repair.sql` — critical_review, add targeted patch-index, parity, and governance documentation
