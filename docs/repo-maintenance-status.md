...
- Durable governed response chunk changes are covered by `test-governed-tool-response-chunk-store.mjs`, `test-gpt-tools-response-chunking.mjs`, route syntax regression, manifest validation, OpenAPI split governance, and route coverage. Migration rollout remains additive, runner-authorized, preflighted, confirmation-gated, and verified by cache-eviction-to-MySQL recovery readback.
