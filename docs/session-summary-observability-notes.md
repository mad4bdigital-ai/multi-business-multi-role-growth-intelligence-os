...
The SQL-primary `execution_log` table is the durable execution record. Transcript fallback paths such as `missing_drive_jsonl_id` are classified as `success_with_warnings` rather than plain `success`, so legacy/preview-only summaries are visible in monitoring. Each completed summary write creates a high-level `execution_log` row with:
