...
This branch is a clean implementation branch for the first Local Connector reachability recovery lanes:
...
This branch must not apply migrations, implement runtime handlers, mutate live connector routes, generate installers, rotate credentials, call Cloudflare, use connector.mad4b.com as a tenant fallback, or mark any device recovered.

## Required invariants

- Select target first.
- Read before repair.
- Tenant and user work uses auth.mad4b.com only.
...
4. Error responses use a structured ErrorEnvelope with stable code, message, details, and requestId.
...
After a governed migration dry-run or apply, verify local_connector_devices, local_connector_device_aliases, local_connector_routes, local_connector_heartbeats, local_connector_probe_results, and local_connector_recovery_plans.

The migration is additive-only, includes target-selection indexes, and stores hashes or redacted evidence rather than plaintext secrets or raw machine identifiers.
