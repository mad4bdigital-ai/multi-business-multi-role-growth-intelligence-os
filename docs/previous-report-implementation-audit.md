...
لا توجد بنود P0/P1/P2 مخفية خلف عبارة «تم التنفيذ» دون evidence في الكود أو الاختبارات. أُغلقت الآن cookie forwarding policy وtrusted-boundary attestation contract وshared mutation adapter وoperation registry وbrowser-like cross-host canary. بقيت حدود تشغيلية لا يجوز اعتبارها مغلقة: **لم يُجرَ production subdomain ownership audit، ولم تُنفذ live-DB أو external canary deployment فعليًا، ولا اختبار ChatGPT الفعلي خارج sandbox**.
...
| P1-4 mutation governance | adapter مشترك fail-closed مغلق للتفعيل | `sharedMutationPolicy.js` يطبق decision path موحدًا، `openApiMutationGovernance.js` يطابق 29 mutation operation مع registry، وMCP يستهلك نفس checks؛ parity test ناجح | جميع 29 entries `unbound` عمدًا؛ لا تُسمح أي write في هذه الدفعة، وprovider mutation/scope promotion خارج النطاق |
...
| P1-9 trusted host | مغلق كعقد code/staging، fail-closed في production | تعارض headers يُرفض، gateway يكتب headers موثوقة، metadata تعود 503 في production عند غياب proxy/attestation/strip الثلاثية | يلزم production ingress owner إثبات overwrite/strip فعليًا خارج المستودع |
| P1-10 runtime coverage | مغلق جزئيًا قويًا | auth 180، gateway، PKCE، SSO round-trip/revoke، refresh concurrency fake-pool، read-only live-schema/index/transaction readiness probe، token binding، MCP OAuth، splitter/parity، وbrowser-like cross-host canary | لا يزال live production DB replay/concurrency وChatGPT browser test الخارجي غير منفذين |
...
الفرع صالح للمراجعة وstaging evidence، لكنه ليس production-ready بالمعنى الكامل. أصبح المستودع يملك contracts fail-closed لكل external gate، ويعرض `operational_readiness` بدل إخفاء النقص. يلزم قبل production/canary خارجي فقط: **subdomain ownership/trust audit، live DB replay/concurrency verification، production ingress owner attestation، واختبار ChatGPT الفعلي خارج sandbox**. جميع writes والمigrations والprovider mutations ما زالت مغلقة.
