...
A scoped approval request binds an approval to:

- execution envelope id;
- execution envelope manifest hash;
- tenant, workspace, boundary, and capability envelope scope;
- requested permissions;
- single-use maximum;
- issued and expiry timestamps;
- no-provider-apply execution boundary.
...
Each decision is a hash-chained record with:

- request manifest hash;
- decision sequence;
- previous decision hash;
- approved, rejected, or abstained decision;
- approver identity;
- decision note hash.

The kernel appends decisions by returning a new log and never mutating the provided log. Existing records are validated before any append. Tampering with any previous decision breaks the hash chain.

Approved and rejected decisions are terminal for the scoped request. Further decisions are rejected once a terminal decision exists.
