...
| Runtime workflow selection | Runtime loaders resolve explicit `workflow_id` first, accept only unique active `workflow_key` matches, and block ambiguous keys. Migration 206 was applied live on 2026-06-06 and promotes the exact identity into execution plans. Before migration 209 apply, corrected live readback confirmed 10 uniquely resolvable legacy plans, 0 ambiguous, 0 identity-missing, 2 unresolved, and 1 identityless plan; governed dry-run passed with zero risks. | `runtimeWorkflowResolver.js`; migrations 206/209; `scripts/workflow-execution-identity-readback.mjs`; `test-runtime-workflow-resolver.mjs`; PR #705 evidence. |
...
| Medium | Apply workflow-identity backfill migration `209_sprint67_execution_plan_workflow_identity_backfill.sql` through the governed runner, then retain post-backfill readback evidence. | Corrected live readback and governed dry-run already passed: 10 uniquely resolvable plans, 0 ambiguous, 0 identity-missing, 2 unresolved, 1 identityless, and zero preflight risks. Apply remains an explicit production mutation. |
