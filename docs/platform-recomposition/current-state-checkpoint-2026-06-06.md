...
| Runtime workflow selection | Runtime loaders resolve explicit `workflow_id` first, accept only unique active `workflow_key` matches, and block ambiguous keys. Migration 206 was applied live on 2026-06-06 and promotes the exact identity into execution plans. The governed readback verifies schema, ledger, live resolver behavior, and classifies legacy plans before migration 209 backfills unique-only matches. | `runtimeWorkflowResolver.js`; migrations 206/209; `scripts/workflow-execution-identity-readback.mjs`; `test-runtime-workflow-resolver.mjs`. |
...
| Medium | Re-run the corrected workflow-identity readback, then apply migration 209 through the governed runner only if its unique-only count is confirmed; retain post-backfill evidence. | The earlier readback found 10 unique-key candidates, no ambiguous legacy plan matches, 2 unresolved keys, and 1 identityless plan. The corrected classifier now separates unique active workflows that still lack an exact `workflow_id`, so the final eligible count must be revalidated before apply. |
