...
The policy permits only `dev.mad4b.com` and `mcp-dev.mad4b.com` as Staging tunnel hostnames. `auth.mad4b.com`, `mcp.mad4b.com`, `activation.mad4b.com`, and `activation-dev.mad4b.com` are rejected as deployment targets. The workflow and local watcher carry no secrets; local tunnel and OAuth values remain in the ignored `http-generic-api\.env.staging` file.
