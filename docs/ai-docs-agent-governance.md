...
> Documentation contract for `20260728_brand_scoped_user_skill_activation.sql`: generated notes must include the additive `brand_skill_policies` and `user_brand_skill_grants` tables and readback view `v_effective_user_brand_skill_grants`; preserve signed-user, tenant, brand, agent, resource, operation, and baseline `agent_skill_grants` authority; state that configured policies fail closed while absent policies preserve legacy behavior; and state that no policy is seeded and no skill is automatically activated. Generated documentation must preserve `no_provider_call`, `no_credential_payload_read`, `no_raw_secrets`, `no_external_send`, `no_external_write`, and `secrets_included=false`, and must not infer migration apply, deployment, provider execution, or live grants from repository state.

> Documentation contract for `20260727_remote_runtime_hostinger_production_branch_allowlist.sql`: generated notes must state that the governed Hostinger deploy executor accepts only `main` or `Production`, still requires an exact pinned commit SHA and existing approval/path/readback controls, and that the migration merely updates existing registry contracts and policies. Documentation must preserve `no_provider_call`, `no_credential_payload_read`, `no_external_send`, `no_external_write`, and `secrets_included_false`, and must not imply that merge or documentation applies the migration or authorizes deployment.

> Documentation contract for `20260715_dynamic_container_canary_promotion_tool.sql`: generated notes must preserve the governed, read-only canary boundary; registry evidence for `dynamic_container_canary_promotion_policy_v1`, `dynamic_container_canary_promotion`, and its app integration binding; ledger run `2b2015d2-73d0-40a2-af33-57678af0389a`; checksum `1c0327e8a8c5533683f92a4906d221af052ba633421464605a68488d3e5b665f`; four-statement zero-risk preflight; fresh plan-bound Capability Envelope use; exact typed confirmation; transactional consumption; and same-cycle readback. Generated documentation must state that global rollout remains `shadow`, mutation enforcement is not enabled, only one `read_only_canary` may be active, and merge or documentation is not execution approval. It must preserve no-provider/no-credential/no-raw-secret/no-external-send-or-write semantics and `secrets_included=false`.

> Documentation contract for `1031_sprint69_github_actions_diagnostics_endpoints_seed.sql`: generated notes must preserve that the migration is endpoint-registry-only, idempotent, and diagnostic/readback-oriented for GitHub Actions workflow jobs and pending deployments. They must mention OpenAPI coverage for `/repos/{owner}/{repo}/actions/runs/{run_id}/jobs` and `/repos/{owner}/{repo}/actions/runs/{run_id}/pending_deployments`, registry binding target `github_list_jobs_for_workflow_run`, and explicit safety markers `no_provider_call`, `no_credential_payload_read`, `no_raw_secrets`, `no_external_send`, `no_external_write`, and `secrets_included_false`. Static docs must not imply CI bypass, deployment approval, credential access, or provider mutation without typed governance.

> Documentation contract for `20260709_resource_api_dynamic_db_surfaces.sql`: generated notes must preserve that the migration is additive, idempotent, and SQL-primary for `platform_data_table_registry` Resource API surfaces. They must mention the ten active surfaces, the governed ledger run `ea0c2abf-b2d1-4660-8929-7229b1fcb851`, checksum `51ae02ddb9d83305432dc3d997426c6bee6ba5048eb8c3bfec3e2cdc6a68b498`, one-statement zero-risk preflight, no provider call, no credential payload read, no raw secrets, no external send, no external write, and `secrets_included=false`. Static documentation must not imply deployment or provider execution.

> Documentation contract for `1030_sprint69_canonical_capability_domain.sql`: generated notes must preserve that the canonical capability domain is additive internal SQL for `canonical_capabilities`, `capability_aliases`, and `v_capability_alias_integrity`, backfilled from existing Admin/Tenant tool catalogs while existing dispatch registries remain runtime authority. They must state `no_provider_call`, `no_credential_payload_read`, `no_raw_secrets`, `no_external_send`, `no_external_write`, and `secrets_included=false`, and must not present repository merge as governed migration authorization, production apply, runtime cutover, or provider execution.

> Documentation contract for `1029_sprint69_minimal_dynamic_brand_resolution.sql` and `1030_sprint69_generic_platform_resource_context.sql`: generated notes must preserve additive registry-only semantics, deterministic final matching, signed-principal Tenant authority, bounded candidate generation, compatibility with `brand_workspace_context_resolve`, safe connection metadata, and provider connectivity reported as `not_checked` until a separate diagnostic. They must state the explicit safety markers `no_provider_call`, `no_credential_payload_read`, `no_raw_secrets`, `no_external_send`, `no_external_write`, and `secrets_included=false`. Static documentation, merged SQL, or tool registration must never be presented as migration authorization, successful apply, provider execution, deployment, or live connectivity proof.
...
> Spec 007 virtual-tool projection contract: changes to `20260717_virtual_tool_capability_projection.sql`, `20260717_virtual_tool_readback_readiness.sql`, `platformVirtualToolCapabilityReconciler.js`, or its assurance-transaction integration must keep the registry patch index, deployment parity checklist, canonical fragment, Agent Knowledge Guide, regression test, and auto-doc notes synchronized. Generated evidence must preserve deterministic canonical identity, Admin/Tenant separation, fail-closed typed debt, shadow readback readiness independent from generic certification, `apply_allowed=0`, additive compatibility, and no provider call, credential payload read, raw-secret output, external send/write, deployment, or automatic capability activation; `secrets_included=false`.
...
- `1023_sprint69_github_rest_endpoint_dispatch_foundation.sql` — SHA-256 `27c6810e4a6b60611373a8b9de2d366930eb061383c59244b70f440f6004b25c`; surfaces: tools=1, routes=3; static preflight: pass/0; runtime reviews: verify_tool_registry_binding.
...
- `1023_sprint69_sql_cache_runtime_policy.sql` — SHA-256 `50424aac877e6c3924191599b295a460007b98d01fbe009d615e06457e24fdc7`; surfaces: policies=1; static preflight: pass/0; runtime reviews: verify_policy_seed_readiness.
...
- `1025_sprint69_activation_archive_dynamic_control_authority.sql` — SHA-256 `b80dc9f8fe7848e708ac63918823ad10f0e41320a5575f41ab89b69433c2ad93`; surfaces: tools=2, routes=2; static preflight: pass/0; runtime reviews: verify_tool_registry_binding.
...
- `1026_sprint69_github_actions_runs_read_dispatch.sql` — SHA-256 `6cffc1fa244dc6ed86e3d0a63a4e8c1bea28d5de9e9e940d38ca43f265d1810c`; surfaces: routes=1; static preflight: pass/0; runtime reviews: none.
- `1026_sprint69_repository_reconciliation_automation.sql` — SHA-256 `0791c588721a948c0e0e935073d795e0f9880d90d6c5132b0688f6e0f7cedc8a`; surfaces: tools=5, policies=1; static preflight: warn/1; runtime reviews: verify_policy_seed_readiness, verify_tool_registry_binding; evidence: human review by platform_admin:f242960c-2857-4b4d-a504-ee50f8a278b4.
...
- `1043_sprint69_tenant_managed_execution_lifecycle.sql` — SHA-256 `a11dff751fca4df19a6acfc188ca7310d8e1a90aa5c3f06fe0c3efeb1213a2a9`; surfaces: views=1; static preflight: pass/0; runtime reviews: verify_readback_view.
...
- `1050_github_repository_policy_controller_bootstrap_repair.sql` — SHA-256 `564d40ec46dba2cee0faf540f2b1de11295d36e5338cb5dd6964ef747489b79c`; surfaces: tools=2, routes=1; static preflight: pass/0; runtime reviews: verify_tool_registry_binding.
- `1051_github_repository_policy_live_apply_authority.sql` — SHA-256 `d5e82b75cd8eeff8a5fb4d76bca4b5ff607361066504237b03df10f3a8fa3fad`; surfaces: policies=1; static preflight: pass/0; runtime reviews: verify_policy_seed_readiness.
- `1052_tenant_platform_plugin_managed_repair_authority.sql` — SHA-256 `ba98da0406f416227cdb0122527455baa4a4cdbb87e196f51357f6cac2c87890`; surfaces: views=2; static preflight: pass/0; runtime reviews: verify_readback_view.
...
- `20260704_platform_resource_authority_grant_tool.sql` — SHA-256 `b4b92c27329cb7099ecc63d1c04c8332b3cfe26c66d7a55538f005d976fa03c3`; surfaces: tools=1, routes=1; static preflight: pass/0; runtime reviews: verify_tool_registry_binding.
...
- `20260711_transactional_outbox_shadow_sync_foundation.sql` — SHA-256 `6eb9bd78d097b18537fbd40c0ab933302046b247f0704b100b953fd71e4c4ee4`; surfaces: tools=1, routes=1; static preflight: pass/0; runtime reviews: verify_tool_registry_binding.
...
- `20260718_github_list_issue_comments_endpoint.sql` — SHA-256 `5f3b09807002eb76864e7c3bf1e16ac8d52369fbbdbc5a67c481e218c5e2d866`; surfaces: routes=1; static preflight: pass/0; runtime reviews: none.
...
- `20260725_growth_control_shadow_parity.sql` — SHA-256 `af7a56bed4efdee59cdd385dccf90231e5eab0e34a8171c992f4963f89316f0e`; surfaces: views=1; static preflight: pass/0; runtime reviews: verify_readback_view.
...
- `20260810_remaining_tenant_runtime_lifecycle_gap_closure.sql` — SHA-256 `355a4a375a12c50b19ad96299e9b95c67b83bd4835a86832e771824bd2a8ccb0`; surfaces: views=3; static preflight: pass/0; runtime reviews: verify_readback_view.
- `20260811_runtime_break_glass_authority_metadata_v1.sql` — SHA-256 `3031712f8443a625e20c7caba09719c5a0203edcd90a368328bd355266cd2a69`; surfaces: policies=1; static preflight: pass/0; runtime reviews: verify_policy_seed_readiness.
- `20260812_dynamic_canonical_resource_registry.sql` — SHA-256 `1ced4a0e5df432e452421a0f35db7f969ea496af09f73ad62676b1bc8f27acfb`; surfaces: views=1; static preflight: pass/0; runtime reviews: verify_readback_view.
- `20260812_resource_surface_policy_decisions_consolidated.sql` — SHA-256 `e4427cd06efadcd099cbf4f26e268412ec3a9072e6fccd6e7f515d8866bb4827`; surfaces: views=2, policies=1; static preflight: pass/0; runtime reviews: verify_policy_seed_readiness, verify_readback_view.
- `20260813_virtual_tool_endpoint_scoped_capability_identity.sql` — SHA-256 `701537932b947424c0fb19634bcb2202e9e3042e9a261508d1ea21aff80f4953`; surfaces: views=6; static preflight: pass/0; runtime reviews: verify_readback_view.
- `20260815_platform_configuration_catalog_and_resolver.sql` — SHA-256 `cc4672125fea34f1ce363c8989a47b0277e336fa16afeec43f6333ea915f2379`; surfaces: views=1; static preflight: pass/0; runtime reviews: verify_readback_view.
- `20260911_staging_activation_gateway_apply_adapter.sql` — SHA-256 `2102e799cc7e6b4d48e7d96414cfa7fb8043c56ec45b13e3197385ff36177a33`; surfaces: views=1, policies=1; static preflight: pass/0; runtime reviews: verify_policy_seed_readiness, verify_readback_view.
- `20260920_wordpress_staging_plugin_deploy_v2_authority.sql` — SHA-256 `c7599c65fb858b92efd749895227af5f81ac8a4d778773c0dfbba2deeb8eda74`; surfaces: tools=1, routes=1; static preflight: pass/0; runtime reviews: verify_tool_registry_binding.
- `20260920_wordpress_staging_plugin_deploy_v2_canonical_seed.sql` — SHA-256 `f6d9047bfda2c77a150ab0f5fca288f0504c9f6fe9deb268313c8c8d48e7f226`; surfaces: tools=1, routes=1; static preflight: pass/0; runtime reviews: verify_tool_registry_binding.
- `20260922_local_manager_control_templates_registry.sql` — SHA-256 `e2e7cb35e32928d4c0ba5565e515bbc86408f7615d0cafee7b791a655f6de4b3`; surfaces: views=1; static preflight: pass/0; runtime reviews: verify_readback_view.
...
- `313_sprint69_zzzz_platform_plugin_bindings_binding_status_width_alignment.sql` — SHA-256 `67332efed42414e3ff3a549c8be2c69ec4ffe739a295ae62a381f5364111e98a`; surfaces: views=1; static preflight: pass/0; runtime reviews: verify_readback_view.
