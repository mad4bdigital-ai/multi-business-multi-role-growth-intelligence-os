...
`ads_provider_preflight_surface_blueprint_policy_v1` defines the design-only blueprint after the contract passes. The blueprint returns the proposed or existing names for preflight tool, preflight ledger table, validator family, credential readiness gate/ledger, execution adapter, and execution enablement family. It never creates those surfaces; a provider-specific PR and governed migration are required later.
