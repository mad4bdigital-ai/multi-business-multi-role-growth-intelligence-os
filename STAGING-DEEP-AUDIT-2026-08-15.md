...
| Ingress 2 | `mcp-dev.mad4b.com` -> `http://app:8080` | Correct |
...
| `mcp-dev.mad4b.com` | CNAME -> same tunnel target | Staging |
...
| Remote MCP | Resource and authorization server continue to resolve from Staging env values: `mcp-dev.mad4b.com` and `dev.mad4b.com/auth/mcp`. |
...
The runtime OAuth regression now performs an HTTP round-trip through `buildRootDiscoveryRoutes()`. It confirms that `dev.mad4b.com/tenant-gpt/oauth-preset` returns the dedicated Staging client and `dev.mad4b.com` authorization/token URLs, while `mcp-dev.mad4b.com` and `activation-dev.mad4b.com` return 404 for the Tenant GPT preset endpoint.
