...
The full 844-entry repository test manifest reached the pre-existing C# sidecar certification test but could not execute it because the Linux sandbox does not have the `dotnet` runtime installed. All touched canonical/Brand tests pass independently; this environmental limitation is not evidence of a regression in PR #7264 and must be rechecked in CI/Windows where the .NET toolchain is available.
