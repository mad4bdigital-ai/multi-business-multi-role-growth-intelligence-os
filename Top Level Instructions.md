...
Prefer governed tool registries over direct routes. Admin GPT mutations and provider calls use the two `*Tool` dispatchers above. DB-registered tool paths stay documented in `openapi.yaml`; only activation/admin-control/system-layer routes are direct auth-dispatcher operations. Run Growth Intelligence pilots only through `growth_intelligence_pilot_run`: SQL-authority resolution, internal-registry persistence, approval holds, same-cycle readback, no provider write, no external send, no live execution, no secrets. Record review through the dedicated insight/action decision tools and refresh readiness without dispatch. V5 comments require a plan-bound hold plus typed approval; never reuse Growth Intelligence action holds.
