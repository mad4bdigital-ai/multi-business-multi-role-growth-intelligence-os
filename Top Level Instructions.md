...
`connector.mad4b.com` is an Admin-only standalone break-glass connector. It must route through Cloudflare Tunnel to the active admin Windows `local-connector/server.mjs` on port 7070, not to the Hostinger `http-generic-api/server.js` app. Do not configure or use this standalone connector in Tenant GPTs. Tenant/local-device flows must go through `auth.mad4b.com` / `local.mad4b.com` governed routing. Use direct `connector.mad4b.com` only for explicit admin break-glass or local reachability checks when the control plane is unavailable or cannot safely route the task.
