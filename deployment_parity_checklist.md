...
> Agent Skill coverage parity: for `1005_sprint69_agent_skill_coverage_prompt_enrichment.sql`, verify `v_skill_runtime_coverage` reports active grant, manifest, prompt, approval, coverage basis, runtime binding status, and covered/gap outcomes without provider calls, credential payload reads, raw-secret returns, external sends, or external writes; `secrets_included=false` remains mandatory.

> Agent Governance admin-tool parity: for `1004_sprint69_agent_governance_admin_tools.sql`, verify the registered tools resolve only to existing `/platform/agent-governance/*` routes, remain backend-api-key and admin-principal protected, reject secret-like fields and values, preserve execution-log readback for plan runs, and perform no credential payload reads, raw-secret returns, external sends, or ungoverned external writes.

> Governed migration reconciliation parity: for `1004_sprint69_growth_agent_migration_reconciliation_policy.sql`, verify exact-file authorization for migrations `243`, `244`, `245`, and `1004`; mandatory preflight and typed confirmation; post-apply schema/ledger readback; no auto-apply outside the governed runner; no provider calls, credential reads, external writes, destructive SQL, deploys, or secrets. Production apply remains blocked until explicit release approval.

> Automation Intelligence Guard parity: before promotion, require CI success for `automation-intelligence-guard.mjs`, tenant mutation evaluator tests, transaction rollback tests, provisioning safety tests, tenant tool surface tests, and the full test manifest on Node 22. Verify `POST /connect/device-install` preserves `400/403/404/409/500` error classes, never returns connector secrets, tunnel tokens, `.env` content, or installer bodies, and returns a short-lived signed installer URL. Verify an existing device reuse reports `provider_calls_made=false` and `provisioning_performed=false`. Confirm migration `1003_sprint69_tenant_device_install_intent_contract.sql` is present in the governed migration runner but is not applied until release approval. After deployment, read back tenant tool tags/schema, OpenAPI behavior, and one no-provider existing-device reuse path before classifying production parity as complete.
