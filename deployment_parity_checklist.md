...
> Session Insight target write readback parity note: after migration `285_sprint68_session_insight_target_write_readback.sql`, verify table `session_insight_target_write_readbacks`, views `v_session_insight_target_write_readback_issues` and `v_session_insight_target_write_readback_readiness`, policy `session_insight_target_write_readback_policy_v1`, and tools `session_insight_target_write_readback_create` and `session_insight_target_write_readback_list`. Smoke target: `node test-session-insight-target-write-readback-service.mjs` passes; live smoke should read already executed internal SQL backlog target write `session_insight_target_write_acefc7aa-4507-4765-9e50-6611105d6595`, return `target_write_readback_passed`, and keep provider calls, credential payload reads, external writes, raw transcripts, target item modifications, rollback execution, and secrets all false.
