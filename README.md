...
- **Tenant Platform** (`openapi.tenant-gpt.auth.yaml` -> `auth.mad4b.com`): 5 MCP-style meta-ops (`activateSession`, `listTools`, `callTool`, `writeSessionTurn`, `endSession`); tool_keys resolved through `tenant_platform_endpoint_tools`. Auth: tenant OAuth/user JWT. Tenant GPTs must not include the standalone `connector.mad4b.com` action.
