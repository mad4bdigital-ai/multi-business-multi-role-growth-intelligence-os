...
const shadowCandidateCount = inventory.write_route_classifications.filter((route) => route.classification === "shadow_candidate").length;
assert.equal(shadowCandidateCount, inventory.classified_write_surface_count);
assert.equal(inventory.intentionally_unmapped_write_route_count, inventory.write_route_count - shadowCandidateCount);
