...
assert(inventory.deterministic === true, "inventory must declare deterministic generation");
assert(summary.deterministic === true, "summary must declare deterministic generation");
assert(summary.totals.files === inventory.totals.files, "summary file total mismatch");
assert(summary.totals.bytes === inventory.totals.bytes, "summary byte total mismatch");
assert(summary.surfaces.testSpecFiles === inventory.surfaces.testSpecFiles.length, "summary test/spec surface mismatch");
...
for (const fixture of fixtures) {
  assert(category(fixture.path) === fixture.category, `category fixture mismatch for ${fixture.path}`);
  assert(extension(fixture.path) === fixture.extension, `extension fixture mismatch for ${fixture.path}`);
}
console.log(`repository inventory self-test passed: ${inventory.totals.files} files, ${inventory.totals.directories} directories, ${fixtures.length} classification fixtures`);
