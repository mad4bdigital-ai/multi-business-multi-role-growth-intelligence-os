...
4. resolve effective resource bindings only when the operation targets an external or governed resource;
5. enforce approval, quota, audit, readback, and rollback requirements independently;
6. block dispatch when any hard gate fails;
7. write bounded evidence and read it back before completion.

Do not infer resource authority from HTTP method, Admin exposure, Tenant exposure, maturity score, tool registration, or a historical envelope.
...
Capability source claims require resolved provenance in `platform_capability_source_links`; repo-derived candidates must remain traceable to `platform_capability_source_resolutions` and `repo_capability_candidates`.
