...
Typed gaps from `v_platform_capability_assurance_gaps` must be preserved in routing evidence. `resource_binding_missing` applies only to resource-scoped operations; Admin and Tenant tool exposure must not be converted automatically into a permanent resource-authority gap.
