...
# 1. Explicit support-ticket to resolution-case linking with pre-migration compatibility.
replace_once(
    resolution_path,
...
async function selectPlaybook(connection, classification) {''',
    "resolution schema capability probe",
)
...
replace_once(
  ...
  if (hasTicketIdColumn) {
    ...
    "resolution explicit link insert",
)
...
replace_once(
  ...
  const [rows] = await pool.query(
    ...
    "resolution explicit link read",
)

# 2. Unified Admin and Tenant request inbox routes.
replace_once(
    routes_path,
...
# 3. Durable chunk schema preflight with typed diagnostics.
replace_once(
    chunk_path,
...
''',
    "chunk required column contract",
)
...
async function assertGovernedResponseChunkSchema(deps = {}, operation = "response_chunk_store") {
  ...
  throw responseChunkError(
    ...
    503,
    {
      cause_code: schema.cause_code || "ER_BAD_FIELD_ERROR",
      operation,
...
for old, new, label in [
    ...
    (
        ...
        "chunk expiry preflight",
    ),
]:
    replace_once(chunk_path, old, new, label)

# 4. Central bounded inline fallback for large Admin/Tenant tool responses.
replace_once(
    gpt_path,
...
replace_once(
  ...
  return buildToolResponseChunk({
    ...
    if (error?.code !== "response_chunk_persistence_unavailable") throw error;
    const requestId = String(
      optionsSource?.request_id || optionsSource?.requestId || deps?.request_id || ""
    ).trim() || null;
    console.warn(
      "[responseChunk] durable persistence unavailable; returning bounded inline response",
      JSON.stringify({
    ...
    return buildBoundedInlineChunkFallback(body, error, {
      sourceToolKey: optionsSource?.source_tool_key || optionsSource?.tool_key || optionsSource?.name || "governed_tool_response",
      maxChars: MAX_TOOL_RESPONSE_MAX_CHARS,
    });
  }''',
    "gpt bounded chunk fallback",
)

print("tenant request inbox runtime hardening patch applied")
