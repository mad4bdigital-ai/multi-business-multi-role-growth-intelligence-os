...
if "  - backendApiKeyAuth: []\npaths:" not in contract:
    contract = replace_once(
        contract,
        "security:\n  - bearerAuth: []\npaths:\n",
        "security:\n  - bearerAuth: []\n  - backendApiKeyAuth: []\npaths:\n",
        "Spec 013 global bearer/API-key alternatives",
    )

observability_old = """      security:
        - bearerAuth: [admin]
      responses:
"""
observability_new = """      security:
        - bearerAuth: [admin]
        - backendApiKeyAuth: []
      responses:
"""
if observability_new not in contract:
    contract = replace_once(
        contract,
        observability_old,
        observability_new,
        "Spec 013 observability bearer/API-key alternatives",
    )

scheme_old = """    bearerAuth:
      type: http
      scheme: bearer
      bearerFormat: JWT
  schemas:
"""
scheme_new = """    bearerAuth:
      type: http
      scheme: bearer
      bearerFormat: JWT
    backendApiKeyAuth:
...
if "    backendApiKeyAuth:\n" not in contract:
    contract = replace_once(
        ...
        scheme_old,
        scheme_new,
        "Spec 013 backend API-key security scheme",
    )
...
if command not in manifest:
    anchor = '  "node test-system-tool-catalog-v2.mjs",\n'
    manifest = replace_once(
        manifest,
        anchor,
        anchor + command,
        "Catalog V2 post-merge regression manifest membership",
    )
