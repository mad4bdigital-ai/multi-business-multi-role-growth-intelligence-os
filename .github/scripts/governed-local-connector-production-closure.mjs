...
assert(Number.isInteger(targetPrNumber) && targetPrNumber > 0, 'TARGET_PR_NUMBER must be a positive integer.');
...
const evidence = {
  schema_version: 'governed-local-connector-production-closure-v1',
  ...
  direct_database_command_executed: false,
...
async function resolveReleaseContract() {
  const pull = await githubJson(`/repos/${repo}/pulls/${targetPrNumber}`);
  assert(pull?.merged === true || Boolean(pull?.merged_at), `PR #${targetPrNumber} is not merged.`);
  assert(pull?.base?.ref === 'Production', `PR #${targetPrNumber} does not target Production.`);
  const mergeSha = String(pull?.merge_commit_sha || '').toLowerCase();
  assert(/^[0-9a-f]{40}$/.test(mergeSha), `PR #${targetPrNumber} has no full merge commit SHA.`);

  const files = await githubJson(`/repos/${repo}/pulls/${targetPrNumber}/files?per_page=100`);
  const changedFiles = Array.isArray(files) ? files.map((file) => String(file?.filename || '')) : [];
  const localConnectorScope = changedFiles.some((path) => [
  ...
  assert(localConnectorScope, `PR #${targetPrNumber} does not contain a recognized Local Connector release surface.`);

  evidence.release = {
    pr_number: targetPrNumber,
    ...
    recognized_local_connector_scope: true,
    secrets_included: false,
  };
  save();
  return mergeSha;
...
async function assertReleaseContained(releaseSha, productionSha) {
  const comparison = await githubJson(`/repos/${repo}/compare/${releaseSha}...${productionSha}`);
  assert(
    ['ahead', 'identical'].includes(comparison.status),
    `Production does not contain release ${releaseSha}; compare status=${comparison.status}`,
  );
  evidence.production.release_compare = {
...
async function waitForRuntimeParity(releaseSha) {
  for (let convergence = 1; convergence <= 3; convergence += 1) {
    ...
    await assertReleaseContained(releaseSha, productionSha);
  ...
  const response = await fetchJson(
    `${baseUrl}/local-manager/device/desktop-commands`,
    {
      ...
      body: JSON.stringify({
        ...
        payload: {
          reason: 'post_production_release_acceptance',
          target_pr_number: targetPrNumber,
          expected_production_sha: evidence.production.sha,
          secrets_included: false,
        },
        requested_by: 'governed-local-connector-production-closure',
        request_context: {
          purpose: 'production_release_acceptance',
          target_pr_number: targetPrNumber,
          release_merge_sha: evidence.release.merge_sha,
          expected_production_sha: evidence.production.sha,
          secrets_included: false,
        },
      }),
...
try {
  const releaseSha = await resolveReleaseContract();
  await waitForRuntimeParity(releaseSha);
  ...
  console.log(`TARGET_PRODUCTION_PR=${targetPrNumber}`);
  ...
  console.log('GOVERNED_LOCAL_CONNECTOR_PRODUCTION_CLOSURE=PASS');
} catch (error) {
  evidence.completed_at = new Date().toISOString();
