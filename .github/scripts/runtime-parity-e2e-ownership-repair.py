...
# Parallel ownership discovery: the exact generated binding is neutral, while
# every other Spec artifact continues to activate the owning parallel contract.
...

{
  ...
  assert.equal(realSpec.ok, false);
  assert(realSpec.findings.some((row) => row.code === "parallel_work_pr_branch_not_declared"));
}
...
# Direct phase-contract discovery uses the same narrow neutral-artifact rule.
