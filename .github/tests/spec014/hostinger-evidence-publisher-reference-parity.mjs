...
for (const requiredPath of [CANONICAL_PUBLISHER, CANONICAL_CANARY, ROUTING, BOOTSTRAP_TEST]) {
  assert(fs.statSync(absolute(requiredPath)).isFile(), `missing canonical evidence dependency: ${requiredPath}`);
}
assert(!fs.existsSync(absolute(LEGACY_PUBLISHER)), "retired Hostinger-only publisher must remain absent");
...
for (const contractPath of CONTRACTS) {
  ...
  assert(Array.isArray(current.e2e_journeys) && current.e2e_journeys.length > 0, `${contractPath} must retain an executable journey`);
  for (const journey of current.e2e_journeys) {
    ...
    for (const evidencePath of journey.evidence_paths) {
      assert(fs.statSync(absolute(evidencePath)).isFile(), `${contractPath} has missing or non-file evidence: ${evidencePath}`);
    }
  }
}
...
assert(
  publisher.includes("name: hostinger-storage-tenant-canary-${{ github.event.workflow_run.id }}-summary"),
  "canonical publisher must download the run-bound Tenant Canary artifact",
);
assert(
  publisher.includes("node http-generic-api/scripts/hostinger-ci-evidence-pr-comment.mjs"),
  "canonical publisher must invoke the bounded Hostinger evidence renderer",
);
assert(publisher.includes("ref: main"), "canonical publisher must execute from trusted main");
assert(publisher.includes("persist-credentials: false"), "trusted checkout must not persist credentials");

console.log("Hostinger evidence publisher reference parity regression passed");
