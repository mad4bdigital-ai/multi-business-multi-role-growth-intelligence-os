...
For three **actually empty** local Staging role databases, use `Rebuild-EmptyStagingRoleDatabases.ps1`. Start the local database containers first. Its default mode inspects every role's tables, views, routines, triggers and events with the local root identity and validates the builder plan without changing any database. If any role contains an object, it refuses the whole rebuild; a partial database needs a separately reviewed schema-repair path. To import on the exact deployed `main` checkout, pass `-Apply`, `-RebuildConfirmation "REBUILD_EMPTY_LOCAL_STAGING_DATABASES:<full SHA>"` and `-GrantConfirmation "REPAIR_LOCAL_STAGING_DATABASE_READINESS:<full SHA>:staging_local_windows_docker"`. The importer checks all three roles again under its lock before DDL, builds from the exact checkout, checks each role's table set, applies the reviewed runtime seeds, seeds Gateway certification as **pending** in Governance, then invokes least-privilege readiness repair and Auto Pilot. If a step fails after import, do not reset or rerun blindly: inspect `staging-db-dumps\schema-import-state.json` and `staging-readiness-repair-state.json` before selecting a bounded recovery path. This operation never authorizes live Gateway apply.
