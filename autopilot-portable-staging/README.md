...
For three **actually empty** local Staging role databases, use `Rebuild-EmptyStagingRoleDatabases.ps1`. Start the local database containers first. Its default mode inspects every role's tables, views, routines, triggers and events with the local root identity and validates the builder plan without mutation. If any role contains an object, it refuses the whole rebuild; selective role reconstruction is not implemented by this command. To import on the exact deployed `main` checkout, pass `-Apply -RebuildConfirmation "REBUILD_EMPTY_LOCAL_STAGING_DATABASES:<full SHA>"`. The importer repeats the root census under its lock before DDL, verifies each role's tables and views and applies reviewed runtime seeds. It seeds Gateway certification as **pending** in Governance. This command ends after schema readback with `grants=not_applied`; run the separately governed `database.access_repair` operation before Auto Pilot or runtime certification. If a step fails after import, inspect `staging-db-dumps\schema-import-state.json` before selecting a bounded recovery path. This operation never authorizes live Gateway apply.
