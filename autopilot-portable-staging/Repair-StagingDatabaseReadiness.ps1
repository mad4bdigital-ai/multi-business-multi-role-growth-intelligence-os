...
try {
    ...
    # Recreate only the DB containers when Compose command provenance changed;
    # bind-mounted database directories are preserved and are never moved/reset.
    ...
    Write-Host "No database reset, schema replay, data-directory move, Production/provider/Hostinger/Cloudflare mutation, or backup deletion was performed."
} catch {
