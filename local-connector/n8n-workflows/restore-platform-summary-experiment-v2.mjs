...
const templatePath = path.resolve(__dirname, 'platform-summary-experiment-v2.template.json');
...
function buildWorkflowPayload(credentialId) {
  ...
  for (const node of workflow.nodes) {
    if (node.credentials?.httpHeaderAuth) {
      node.credentials.httpHeaderAuth = { id: credentialId, name: template.credential.name };
    }
  }
  return workflow;
}
...
async function smokeTest() {
  const response = await fetch(`${n8nBase}/webhook/platform-summary-experiment-v2`, {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      input: {
        text: 'Decision: restore v2 as an experiment. Blocker: do not use for canonical writes. Next action: compare v2 with v1 and model summary.',
        use_case: 'quick_preview',
        max_bullets: 5,
        max_chars: 900,
      },
    }),
...
console.log(JSON.stringify({
  ...
  policy: template.runtime_binding?.experiment_policy || null,
