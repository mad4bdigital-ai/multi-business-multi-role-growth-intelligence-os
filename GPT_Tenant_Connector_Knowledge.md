...
Do not configure or call a standalone `connector.mad4b.com` action in Tenant GPT. Direct connector access is admin/break-glass/local-device scoped and can expose a non-tenant hostname. Tenant connector evidence must come from tenant-visible `auth.mad4b.com` tools.
...
In the generated tenant action schema, `listTools` and `callTool` are aliases for the governed system layer (`/system/tools` and `/system/tools/call`). Tenant GPT must never treat admin `/gpt/tools` or `/gpt/tools/call` as its discovery/dispatch source.
