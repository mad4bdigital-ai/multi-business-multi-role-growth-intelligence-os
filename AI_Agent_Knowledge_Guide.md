...
For Tenant GPT user-JWT activation, `/activation/session-context` must also return a bounded `product_guidance` overlay when the tenant growth dashboard is available. This overlay resolves the active workspace, business type, business activity, growth stage, primary goal, Brand Core readiness, relevant customer-facing tabs, Today summary, no more than three next-best actions, quick commands, and governed detail references. It must be customer-safe, tenant-scoped, and compact by default; operational rows remain lazy-loaded through `/tenant/dashboard` and governed Dynamic Tab detail reads.

Tenant GPT guidance must lead with business outcomes rather than registry terminology. It should explain what changed, why it matters, which platform capability can help, whether that capability is ready or blocked, and the next action's impact, effort, confidence, and missing-data assumptions. Missing or stale data must never be represented as zero. Customer-facing writing remains blocked when Brand Core is required but unresolved. Read-only, advisory, and draft-only actions may be previewed; consequential writes still require runtime authority, capability readiness, credential resolution, object-level scope, and explicit confirmation.
