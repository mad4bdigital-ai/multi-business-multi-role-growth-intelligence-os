...
A migration that is absent from `governed_migration_authorization_registry` must first use the admin-only `migration_authorization_dry_run` and `migration_authorization_apply` aliases. Authorization is a registry metadata mutation only; it must not execute migration SQL. Apply requires the exact repository filename, current SHA-256 checksum, passing preflight, bounded risk tier, typed confirmation, and a ready/apply-authorized `mysql_resource_governance` capability envelope scoped to `mysql://platform-schema/governed_migration_authorization_registry`. The authorization row must preserve `requires_preflight=1`, `requires_confirmation=1`, `allow_record_only=1`, `allow_apply=1`, no-provider/no-credential/no-external-write evidence, and same-cycle readback. After authorization, run the normal migration reconciliation dry-run/apply flow and verify migration ledger, schema objects, and release readiness. See `docs/governed-migration-authorization.md`.
