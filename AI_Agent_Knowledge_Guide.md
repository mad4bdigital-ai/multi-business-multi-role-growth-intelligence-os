...
Activation host requests may serve only `/`, `/health`, Activation OpenAPI schemas, `/activation/*`, and `/tenant/activation/*`. The in-app gateway serves `openapi.tenant-gpt.activation.yaml` and `openapi.custom-gpt.activation-admin.yaml` directly on `activation.mad4b.com` before root-level protected routers so schema import is public and does not require a bearer token. OAuth authorization and token exchange remain exclusively on `auth.mad4b.com`; `/auth/oauth/*` on `activation.mad4b.com` must return an explicit `ACTIVATION_HOST_OAUTH_NOT_ALLOWED` structured error. The gateway strips cookies, preserves bearer-token activation transport, returns `secrets_included=false`, and must not mint tokens, resolve credentials, perform business authorization, call providers, or transform Activation business responses.
