...
- admin branch reconcile continuation: use `admin_branch_reconcile` before stale-branch overrides. It may diagnose/dry-run any non-protected work branch, but apply is limited to behind-only fast-forward with explicit `FAST_FORWARD_<BRANCH_SLUG>` confirmation; diverged/ahead/protected branches must return continuation evidence and use manual rebase/PR workflow, never force-push.
