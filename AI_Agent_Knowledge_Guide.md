...
Do not use oversized `admin_control` or `connector_ps` calls for PR update/merge flows when GitHub connector, GitHub REST fallback, or local Git commands can perform the work. For `connector_ps`, HTTP 200 means the connector route responded; command success is determined by the JSON body `ok`, `exitCode`, and `exit_code`.
