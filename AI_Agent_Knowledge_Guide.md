...
- Google Ads budget changes: `google_ads_budget_change_preflight` must pass before any future Google Ads budget mutation adapter can call Google Ads; it requires a ready Google Ads capability envelope and `budget_quota_authority_dry_run=ready_for_dispatch`, and it never reads credentials or changes spend.
