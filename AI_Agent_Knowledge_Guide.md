...
Migration `285_sprint68_session_insight_target_write_readback.sql` adds post-write validation for those internal SQL target writes. It reads `session_insight_backlog_target_writes` and `session_insight_backlog_target_items`, records readback evidence in `session_insight_target_write_readbacks`, and verifies target-item existence, link metadata, source payload hashes, duplicate counts, and no provider/external/credential/raw-transcript/secret flags. Agents must treat it as readback-only: it must not create target writes, modify target items, execute rollback, call providers, read credential payloads, perform external writes, return raw transcripts, or include secrets.
