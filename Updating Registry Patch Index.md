...
> 2026-06-11 Session Insight target write readback validator note: PR #1373 adds migration `285_sprint68_session_insight_target_write_readback.sql`, routes `/platform/session-insight-promotions/target-write-readbacks/create` and `/platform/session-insight-promotions/target-write-readbacks/list`, admin tools `session_insight_target_write_readback_create` and `session_insight_target_write_readback_list`, and targeted service/route/test coverage. The readback validates already-executed internal SQL backlog target writes against `session_insight_backlog_target_items`, source payload hashes, target-write/item links, duplicate counts, and no provider/external/credential/raw-transcript/secret flags. It never creates target writes, modifies target items, executes rollback, calls providers, reads credential payloads, performs external writes, returns raw transcripts, or includes secrets.
