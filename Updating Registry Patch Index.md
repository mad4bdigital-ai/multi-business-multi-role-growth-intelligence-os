...
## 2026-07-01 Activation Archive and Dynamic Admin Control Authority

`1025_sprint69_activation_archive_dynamic_control_authority.sql` registers bounded Admin and tenant-owned activation archive reads plus dynamic `admin_control` authority metadata. Because that migration may already be ledger-addressed, it remains immutable. `20260701_dynamic_admin_control_permission_hardening.sql` is the additive follow-up: read operations require `view` or stronger; mutations require `edit`, `operate`, `manage`, `admin`, or `owner` independently on the binding and any required owner grant; unknown permissions deny. Both migrations perform no provider calls, credential payload reads, raw-secret access, external sends, external writes, or deployment, and declare `secrets_included=false` through the follow-up safety contract.

Runtime validation also classifies `SELECT ... INTO @variable` as stateful and therefore mutation-gated. Apply remains a separate governed migration operation with checksum-bound authorization, preflight, ledger readback, policy JSON readback, and focused regression evidence.
