...
> 2026-06-11 Session Insight target write readback validator note: migration `285_sprint68_session_insight_target_write_readback.sql` adds table `session_insight_target_write_readbacks`, readback/readiness views, policy `session_insight_target_write_readback_policy_v1`, routes `/platform/session-insight-promotions/target-write-readbacks/create` and `/platform/session-insight-promotions/target-write-readbacks/list`, and tools `session_insight_target_write_readback_create` and `session_insight_target_write_readback_list`. It validates already executed internal SQL backlog target writes against target items, source payload hashes, link metadata, duplicate counts, and no provider/external/credential/raw-transcript/secret flags. It never creates target writes, modifies target items, executes rollback, calls providers, reads credentials, performs external writes, returns raw transcripts, or includes secrets.
