...
const toolchainProvenanceDigest = digest(provenanceCore);
...
const baseAttestationEvidence = {
  ...
  subject_name: 'hostinger-storage-plan:plan-1',
  subject_digest: h('3'),
  operation_id: 'operation-1',
  plan_id: 'plan-1',
  target_id: 'target-1',
...
function verificationFor(evidenceOverrides = {}, { preserveDigest = false } = {}) {
  const evidence = { ...baseAttestationEvidence, ...evidenceOverrides };
  return {
    ready: true,
    evidence,
    evidence_digest: preserveDigest ? digest(baseAttestationEvidence) : digest(evidence),
    blockers: [],
...
const attestationVerification = verificationFor();
...
assert.equal(bundle.bundle.toolchain_provenance_digest, toolchainProvenanceDigest);
...
assert.equal(bundle.bundle.canonical_attestation_binding.toolchain_provenance_digest, toolchainProvenanceDigest);
assert.equal(bundle.bundle.canonical_attestation_binding.toolchain_selected_tools_digest, selectedToolsDigest);
assert.equal(bundle.bundle.canonical_attestation_binding.evidence_digest, attestationVerification.evidence_digest);
...
const omittedReadback = verifyHostingerStorageExecutionAuthorizationBundle({
  authorization: bundle,
  current: {},
});
assert.equal(omittedReadback.valid, true);
assert.deepEqual(omittedReadback.blockers, []);
...
const drift = verifyHostingerStorageExecutionAuthorizationBundle({
  ...
  current: {
    ...
    attestation_toolchain_provenance_digest: h('8'),
    attestation_toolchain_selected_tools_digest: h('9'),
    lease_generation: 3,
  },
});
...
assert(drift.blockers.includes('STORAGE_ATTESTATION_RECOVERY_REQUIRED_CHANGED'));
...
assert(drift.blockers.includes('STORAGE_ATTESTATION_TOOLCHAIN_PROVENANCE_CHANGED'));
...
const clearedRecovery = verifyHostingerStorageExecutionAuthorizationBundle({
  authorization: bundle,
  current: {
    recovery_required: null,
    recovery_proof_digest: null,
    recovery_requirement_binding_digest: '',
    attestation_toolchain_provenance_digest: null,
  },
});
assert.equal(clearedRecovery.valid, false);
assert(clearedRecovery.blockers.includes('STORAGE_ATTESTATION_RECOVERY_REQUIRED_CHANGED'));
assert(clearedRecovery.blockers.includes('STORAGE_ATTESTATION_RECOVERY_PROOF_CHANGED'));
assert(clearedRecovery.blockers.includes('STORAGE_ATTESTATION_RECOVERY_REQUIREMENT_CHANGED'));
assert(clearedRecovery.blockers.includes('STORAGE_ATTESTATION_TOOLCHAIN_PROVENANCE_CHANGED'));

assert.throws(
  () => buildBundle({
    attestation_verification: verificationFor({
      operation_id: undefined,
      target_id: undefined,
      candidate_set_hash: undefined,
      authority_context_hash: undefined,
    }),
  }),
...
assert.throws(
  () => buildBundle({
    attestation_verification: verificationFor({
      ...
      toolchain_provenance_digest: undefined,
  ...
  (error) => error.code === 'STORAGE_ATTESTATION_AUTHORIZATION_BINDING_REQUIRED'
    ...
    && error.details?.missing?.includes('toolchain_provenance_digest')
    && error.details?.missing?.includes('toolchain_selected_tools_digest'),
);

assert.throws(
  () => buildBundle({
    attestation_verification: verificationFor({ plan_id: 'plan-2' }),
  }),
...
assert.throws(
  () => buildBundle({
    attestation_verification: verificationFor({ execution_lease_id: 'lease-2' }),
  }),
...
for (const [field, value] of [
  ...
  ['toolchain_provenance_digest', h('0')],
  ['toolchain_selected_tools_digest', h('0')],
  ['recovery_required', false],
]) {
  assert.throws(
    () => buildBundle({ attestation_verification: verificationFor({ [field]: value }) }),
...
assert.throws(
  () => buildBundle({
    attestation_verification: verificationFor({ approval_set_hash: h('0') }, { preserveDigest: true }),
  }),
  (error) => error.code === 'STORAGE_ATTESTATION_EVIDENCE_TAMPERED',
);
...
assert.throws(
  () => buildBundle({
    approved_tools: approvedTools.map((record) => record.tool_id === 'cosign'
      ? { ...record, release_provenance_digest: h('f') }
      : record),
  }),
  (error) => error.code === 'STORAGE_ATTESTATION_AUTHORIZATION_BINDING_MISMATCH'
    && error.details?.mismatches?.includes('toolchain_provenance_digest'),
);
...
console.log(JSON.stringify({
  ...
  attestation_evidence_digest_verified: true,
  canonical_attestation_plan_and_lease_binding: true,
  canonical_attestation_approval_recovery_toolchain_binding: true,
  signed_toolchain_release_provenance_bound: true,
  explicit_recovery_binding_clear_rejected: true,
  omitted_optional_readback_allowed: true,
