...
await import('./test-spec012-t031-oauth-code-consumption-foundation.mjs');
...
const FORBIDDEN_MODULE = 'hostingerStorageSharedCanaryCore';
const ALLOWED_IMPORTER = 'http-generic-api/hostingerStorageSharedCanary.js';
...
function findForbiddenImports({ source, filePath }) {
  ...
  function record(node, expression) {
    ...
    if (specifier === null || !specifier.replaceAll('\\', '/').includes(FORBIDDEN_MODULE)) return;
    const relativePath = normalizePath(path.relative(repositoryRoot, filePath));
    if (relativePath === ALLOWED_IMPORTER) return;
...
const coreName = 'hostingerStorageSharedCanary' + 'Core.js';
const selfTests = [
  `import('../http-generic-api/${coreName}')`,
  "import('../http-generic-api/hostingerStorageSharedCanary' + 'Core.js')",
  "const prefix = '../http-generic-api/hostingerStorageShared'; const suffix = 'CanaryCore.js'; import(prefix + suffix)",
  "require('../http-generic-api/hostingerStorageSharedCanary' + 'Core.js')",
  `import(new URL('../http-generic-api/${coreName}', import.meta.url))`,
];
for (const [index, source] of selfTests.entries()) {
  const findings = findForbiddenImports({
    source,
    filePath: path.join(repositoryRoot, `scripts/shared-core-self-test-${index}.cjs`),
  });
  assert.equal(findings.length, 1, `Shared Core import self-test ${index} must be rejected.`);
}

const findings = walk(repositoryRoot).flatMap((filePath) => findForbiddenImports({
...
if (findings.length) {
  console.error(JSON.stringify({
    ok: false,
    gate: 'hostinger_storage_shared_canary_core_import_boundary',
    findings,
