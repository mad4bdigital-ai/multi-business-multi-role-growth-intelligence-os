...
import { consumeContainerOverride } from "./dynamicContainerAuthorityRepository.js";
import {
...
export async function runDynamicContainerOverrideGovernanceSmoke({
  ...
  try {
    ...
    let staleEpochRejected=false;
    try {
      await consumeContainerOverride({
        overrideId:staleOverrideId,
        executionRef:`override-governance-smoke:${runId}:stale`,
        resolution:{ resolutionId,targetContainerId,containerPathHash,authorityEpoch,resolutionSha256 },
        targetContainerId,
        dimensionKey:"actions",
        resourceType:"smoke_resource",
        resourceRef:`smoke:${runId}:${staleOverrideId}`,
        operationKey:"execute.smoke",
        actionKey:"dynamic_container_override_governance_smoke",
        endpointKey:"internal_smoke",
        readbackRef:"override-governance-smoke-stale-readback"
      },{ executor:connection,manageTransaction:false });
    } catch(error) {
      if(String(error?.code || "") !== "override_snapshot_stale") throw error;
      staleEpochRejected=true;
    }
    if(!staleEpochRejected) throw smokeError(409,"override_governance_smoke_stale_epoch_failed","Production stale-snapshot guard did not reject the fixture.");
