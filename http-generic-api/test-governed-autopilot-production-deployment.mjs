...
const rootDiscoveryRoutes = read("http-generic-api/routes/rootDiscoveryRoutes.js");
const routesIndex = read("http-generic-api/routes/index.js");
const openApiRoot = read("http-generic-api/openapi.yaml");
const openApiRouteContracts = read("http-generic-api/openapi-route-contracts.yaml");
const stagingActAsUserSchema = read("http-generic-api/openapi/openapi.custom-gpt.staging-act-as-user-control.yaml");
...
assert.match(r7, /productionCurrent/);
assert.match(r7, /resolveR7Decision/);
...
assert.match(envExample, /^TENANT_GPT_SSO_SIGNING_SECRET=/m);
assert.match(envExample, /^TENANT_GPT_SSO_COOKIE_MODE=host_only$/m);
assert.match(envExample, /^TENANT_GPT_SSO_TRUST_BOUNDARY_ATTESTED=false$/m);
...
assert.match(routesIndex, /buildRootDiscoveryRoutes\(deps\)/);
assert.match(rootDiscoveryRoutes, /unmatchedHostResponse/);
assert.match(rootDiscoveryRoutes, /unmatched_hostname/);
assert.match(openApiRouteContracts, /POST \/admin\/act-as-user\/sessions/);
assert.match(openApiRouteContracts, /POST \/admin\/act-as-user\/sessions\/\{sessionId\}\/revoke/);
assert.match(openApiRoot, /\/admin\/act-as-user\/sessions/);
assert.match(stagingActAsUserSchema, /live_execution_allowed: false/);
assert.match(stagingActAsUserSchema, /write_activation_allowed: false/);
assert.match(stagingActAsUserSchema, /database_mutation_allowed: false/);
assert.match(stagingActAsUserSchema, /secrets_included: false/);
