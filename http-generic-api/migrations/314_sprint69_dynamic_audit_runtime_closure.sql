...
--
-- Additive and idempotent. Enables an internal runtime scheduler with MySQL
-- advisory locks, records bounded scheduler evidence, canonicalizes schedule
-- configuration, and upgrades readiness to cover all event sources, Drive
-- evidence, repo file-audit persistence, DB semantic quality, and checkpoint
-- completion. No provider calls. No raw request/response payload storage.
-- No secrets. secrets_included=false
...
(
  ...
  'active',
  'Canonical internal Dynamic Audit scheduler configuration. Runtime interval execution is protected by MySQL advisory locks.',
  UTC_TIMESTAMP(),
  UTC_TIMESTAMP()
),
(
  ...
  'active',
  'Canonical scope for Dynamic Audit checkpoint rollups. Deployed commit SHA must never be inferred.',
  UTC_TIMESTAMP(),
  UTC_TIMESTAMP()
),
(
  ...
  'active',
  'Canonical Dynamic Audit bridge schedule; executed by the internal runtime scheduler and guarded by advisory locks.',
  UTC_TIMESTAMP(),
  UTC_TIMESTAMP()
),
(
  ...
  'active',
  'Canonical Dynamic Audit rollup schedule; executed by the internal runtime scheduler and guarded by advisory locks.',
  UTC_TIMESTAMP(),
  UTC_TIMESTAMP()
)
