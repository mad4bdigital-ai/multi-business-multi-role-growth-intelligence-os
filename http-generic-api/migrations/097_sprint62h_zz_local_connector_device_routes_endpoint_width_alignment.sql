-- Staging-local additive compatibility bridge for the immutable migration 098 writer.
-- endpoint URLs may be up to the historical tunnel URL width; preserve exact values and
-- use a generated SHA-256 key so the uniqueness contract remains index-safe.
...
CREATE TABLE IF NOT EXISTS `local_connector_device_routes` (
  ...
  `endpoint_url_sha256` CHAR(64) GENERATED ALWAYS AS (SHA2(`endpoint_url`, 256)) PERSISTENT,
