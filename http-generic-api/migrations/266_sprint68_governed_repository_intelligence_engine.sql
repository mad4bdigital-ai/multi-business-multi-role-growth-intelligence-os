...
-- Adds repository development governance to the generic platform_resource_* model.
-- V1 is registry/plan-only: no comments, labels, closing PRs, merging,
-- patching, force-push, or migration apply is enabled by this migration.
-- Contract: Resource + Recipe + Authority + Policy + Evidence + Capability
-- Envelope for mutation + Same-cycle validation for sensitive mutation +
-- Readback + Audit + no secrets.
...
VALUES
  ('github.repo.pr_reconciliation.adapter', 'github_repo', 'github_api_mcp', 'endpoint_recipe', NULL,
   ...
   'metadata_only', 1, 1, 0, 'active', JSON_OBJECT('v1_execution','plan_only','mutation_allowed_v1',false)),
  ...
  ('github.file.patch.adapter', 'github_file', 'github_api_mcp', 'composite', NULL,
   ...
   'content_hash_only', 1, 0, 0, 'planned', JSON_OBJECT('delegates_to','repo_patch_apply','requires_capability_envelope',true,'apply_supported_v1',false))
ON DUPLICATE KEY UPDATE
...
VALUES
  ('repo.pr.reconciliation_sweep', 'github_repo', 'pr_reconciliation_sweep', 'github.repo.pr_reconciliation.adapter', 'diagnostic', 'reconcile',
   ...
   JSON_OBJECT('v1','dry_run_only','allowed_modes',JSON_ARRAY('dry_run','plan'),'forbidden_v1',JSON_ARRAY('comment','label','close','merge','patch','force_push','migration_apply'),'secrets_allowed',false),
   'summary_node', 'resource_authority_engine', 'active', 'Plan/read-only foundation recipe for repository PR reconciliation sweep. V1 emits no mutations.'),
  ...
  ('repo.pr.comment_recommendation', 'github_pull_request', 'comment_recommendation', 'github.pull_request.classification.adapter', 'mutation', 'apply',
   ...
   'summary_node', 'resource_authority_engine', 'planned', 'Future guarded PR comment recipe. Disabled in V1.'),
