-- Additive readiness repair for the already-applied Repository Authority and Capability Bindings V2 migration.
-- Scope: internal registry rows only. No provider call, no webhook mutation, no external write, no credential payload read.
...
ON DUPLICATE KEY UPDATE
  ...
  managed_capable=1,
  status='active',
  config_json=JSON_MERGE_PATCH(COALESCE(connected_systems.config_json, JSON_OBJECT()), VALUES(config_json)),
  updated_at=NOW();
