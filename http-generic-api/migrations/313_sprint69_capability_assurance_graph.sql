...
VALUES
  ('platform_capability_assurance_reconcile','Reconcile Platform Capability Assurance Graph',
   'Dry-run by default. Apply canonical capability, provenance, evidence, certification, and debt upserts only with a ready capability envelope. No provider calls or secrets.',
   ...
   JSON_OBJECT('type','object','required',JSON_ARRAY('tool','action','alias'),'additionalProperties',false,'properties',JSON_OBJECT(
     'tool',JSON_OBJECT('type','string','const','shell'),'action',JSON_OBJECT('type','string','const','run'),
   ...
   NULL,'admin,capability,assurance,reconcile,dry_run_default,capability_envelope,no_provider_call,no_external_write,no_secrets',
   1,735,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)
ON DUPLICATE KEY UPDATE display_name=VALUES(display_name),description=VALUES(description),http_method=VALUES(http_method),
  http_path=VALUES(http_path),path_param_keys=VALUES(path_param_keys),input_schema=VALUES(input_schema),tags=VALUES(tags),
...
VALUES
 ('platform_capability_assurance_reconcile','platform_capability_assurance_reconcile','capability_assurance',
  ...
  1,0,1,1,1,1,'migration:313_sprint69_capability_assurance_graph.sql',CURRENT_TIMESTAMP,NULL,
  'Dry-run by default; apply requires a ready capability envelope and writes SQL registry/evidence/debt rows only.')
ON DUPLICATE KEY UPDATE certification_status=VALUES(certification_status),dispatch_allowed=VALUES(dispatch_allowed),
 apply_allowed=VALUES(apply_allowed),requires_resource_authority=VALUES(requires_resource_authority),
 requires_dry_run=VALUES(requires_dry_run),requires_audit_evidence=VALUES(requires_audit_evidence),
 requires_readback=VALUES(requires_readback),last_evidence_ref=VALUES(last_evidence_ref),notes=VALUES(notes),updated_at=CURRENT_TIMESTAMP;
...
SELECT DISTINCT c.source_table,REPLACE(c.source_table,'_',' '),c.capability_family,'legacy_registry',c.exposure_scope,'governed',
...
SELECT c.capability_key,c.source_table,c.display_name,c.capability_family,c.source_table,c.source_key,c.operation_class,c.risk_class,
 c.runtime_status,c.exposure_scope,
 CASE WHEN c.resource_authority_required=0 THEN 'none'
      WHEN c.source_table IN ('admin_platform_endpoint_tools','tenant_platform_endpoint_tools') THEN 'invocation'
      WHEN c.apply_allowed=1 OR c.risk_class IN ('D','critical') THEN 'combined' ELSE 'resource' END,
...
SELECT b.binding_key,b.capability_key,b.binding_family,b.source_table,b.source_key,b.binding_status,b.exposure_scope,b.credential_source,
 b.dispatch_allowed,b.apply_allowed,JSON_OBJECT('legacy_notes',b.notes)
...
SELECT SHA2(CONCAT(c.capability_key,'|registry|',c.source_table,'|',c.source_key),256),c.capability_key,'registry',
 CONCAT(c.source_table,':',c.source_key),'resolved',1.0000,
 JSON_OBJECT('source_table',c.source_table,'source_key',c.source_key,'backfill','migration_313')
FROM `v_platform_capabilities_current` c
...
INSERT INTO `platform_capability_source_resolutions`
 (`resolution_id`,`source_type`,`source_ref`,`detected_product`,`mime_type`,`read_strategy`,`fallback_strategy`,
  `supports_all_drives`,`web_url_fetch_allowed`,`resolution_json`,`status`)
SELECT UUID(),'github_repo',CONCAT('registry://',c.source_table,'/',c.source_key),'growth_intelligence_os',
 'application/vnd.mad4b.capability+json','mysql_registry_projection','repo_inspect',1,0,
 JSON_OBJECT('capability_key',c.capability_key,'source_table',c.source_table,'source_key',c.source_key,'secrets_included',false),'resolved'
FROM `v_platform_capabilities_current` c
WHERE NOT EXISTS (SELECT 1 FROM `platform_capability_source_resolutions` r
 WHERE JSON_UNQUOTE(JSON_EXTRACT(r.resolution_json,'$.capability_key'))=c.capability_key);

INSERT INTO `repo_capability_candidates`
 (`capability_candidate_id`,`repo_source_id`,`candidate_type`,`capability_key_suggested`,`runtime_language`,`install_method_detected`,
  `requires_code_execution`,`requires_network`,`requires_credentials`,`requires_filesystem`,`requires_shell`,`risk_class`,
  `sandbox_status`,`certification_status`,`status`)
SELECT UUID(),NULL,
 CASE WHEN c.source_table IN ('admin_platform_endpoint_tools','tenant_platform_endpoint_tools') THEN 'tool'
      WHEN c.source_table='platform_engine_registry' THEN 'engine'
      WHEN c.source_table='platform_plugin_contributions' THEN 'connector'
      WHEN c.source_table='runtime_dispatch_certification_registry' THEN 'runtime' ELSE 'adapter' END,
 c.capability_key,'javascript_sql_registry','registry_projection',
 CASE WHEN c.operation_class IN ('read','diagnostic') THEN 0 ELSE 1 END,
 CASE WHEN c.source_table IN ('admin_platform_endpoint_tools','tenant_platform_endpoint_tools','platform_plugin_contributions') THEN 1 ELSE 0 END,
 c.resource_authority_required,0,0,
 CASE WHEN c.risk_class IN ('D','critical') THEN 'critical' WHEN c.risk_class IN ('C','high') THEN 'high'
      WHEN c.risk_class IN ('B','medium') THEN 'medium' ELSE 'low' END,
 CASE WHEN c.operation_class IN ('read','diagnostic') THEN 'not_required' ELSE 'required' END,
 CASE WHEN c.runtime_status IN ('read_only_certified','diagnostic_certified','certified') THEN 'certified' ELSE 'required' END,'accepted'
FROM `v_platform_capabilities_current` c
WHERE NOT EXISTS (SELECT 1 FROM `repo_capability_candidates` rc WHERE rc.capability_key_suggested=c.capability_key);
...
SELECT CONCAT('envelope:',e.envelope_id,':decision'),'capability_envelope_decision','capability_invocation',e.envelope_id,
 ...
 CASE WHEN e.envelope_status IN ('ready_for_dispatch','ready_requires_approval') THEN 'passed'
      WHEN e.envelope_status='blocked' THEN 'blocked' WHEN e.envelope_status='expired' THEN 'expired' ELSE 'observed' END,
...
SELECT CONCAT('authority-binding:',b.binding_id,':state'),'resource_binding_state','resource_binding',b.binding_id,b.binding_id,
 ...
 CONCAT('binding_',b.status),SHA2(CONCAT(b.binding_id,'|',b.status,'|',b.permission_level,'|',COALESCE(b.expires_at,'')),256),
...
SELECT CONCAT('certification:',c.certification_key,':state'),'capability_certification','runtime_surface',c.surface_key,
 ...
 c.certification_status,SHA2(CONCAT(c.certification_key,'|',c.certification_status,'|',c.dispatch_allowed,'|',c.apply_allowed),256),
...
SELECT UUID(),'user_app_connection',m.connection_id,'platform_secret',p.secret_key,'value_ciphertext',p.value_sha256,p.created_by,
...
FROM `platform_secrets` p JOIN `v_platform_secret_promotion_monitoring` m ON m.secret_key=p.secret_key
WHERE p.value_sha256 IS NOT NULL
 AND NOT EXISTS (SELECT 1 FROM `platform_secret_movement_ledger` l WHERE l.target_id=p.secret_key AND l.value_sha256=p.value_sha256);
...
SELECT c.capability_key,c.display_name,c.capability_family,c.source_table,c.source_key,c.operation_class,c.risk_class,
 ...
 1 AS discoverable,1 AS registered,
 ...
 CASE WHEN c.authority_requirement_type IN ('none','invocation') THEN 1
      WHEN c.legacy_evidence_ref IS NOT NULL THEN 1
      WHEN EXISTS (SELECT 1 FROM v_effective_platform_resource_authority_bindings eb) THEN 1 ELSE 0 END AS authority_model_ready,
 CASE WHEN c.authority_requirement_type IN ('none','invocation') THEN 1
      WHEN c.legacy_evidence_ref IS NOT NULL THEN 1
      WHEN EXISTS (SELECT 1 FROM v_effective_platform_resource_authority_bindings eb) THEN 1 ELSE 0 END AS resource_binding_ready,
 ...
 CASE WHEN c.requires_readback=0 THEN 1
      ...
      WHEN EXISTS (SELECT 1 FROM platform_capability_certifications pc WHERE pc.capability_key=c.capability_key
                    AND pc.revoked_at IS NULL AND (pc.expires_at IS NULL OR pc.expires_at>CURRENT_TIMESTAMP)) THEN 1 ELSE 0 END AS readback_contract_ready,
 CASE WHEN EXISTS (SELECT 1 FROM platform_capability_certifications pc WHERE pc.capability_key=c.capability_key
                    AND pc.revoked_at IS NULL AND pc.certification_status NOT IN ('failed','blocked','revoked','expired')
 ...
 CASE WHEN EXISTS (SELECT 1 FROM platform_capability_source_links sl WHERE sl.capability_key=c.capability_key
                    AND sl.resolution_status='resolved') THEN 1 ELSE 0 END AS provenance_ready,
 CASE WHEN EXISTS (SELECT 1 FROM platform_evidence_events ev WHERE ev.capability_key=c.capability_key
                    AND ev.evidence_status='passed' AND ev.revoked_at IS NULL
 ...
 (CASE WHEN c.dispatch_allowed=0 THEN 1 ELSE 0 END +
  CASE WHEN c.authority_requirement_type IN ('resource','combined') AND c.legacy_evidence_ref IS NULL
        AND NOT EXISTS (SELECT 1 FROM v_effective_platform_resource_authority_bindings eb) THEN 1 ELSE 0 END) AS hard_block_count
FROM `platform_plugin_capabilities` c WHERE c.status='active';
...
SELECT capability_key,'resource_binding_missing',
 ...
 'A resource-scoped capability has no effective resource binding or legacy authority evidence.'
...
UNION ALL
SELECT capability_key,'active_export_missing','low','An admin or tenant capability has no active public export.'
FROM v_platform_capability_readiness_vector WHERE exposure_scope IN ('admin','tenant') AND exported=0
UNION ALL
...
UNION ALL
SELECT capability_key,'provenance_missing','low','Capability has no resolved canonical source link.'
...
SELECT COUNT(*) AS capability_count,SUM(dispatchable=1) AS dispatchable_count,SUM(applyable=1) AS applyable_count,
 SUM(certified=1) AS certified_count,SUM(provenance_ready=1) AS provenance_ready_count,
 SUM(resource_binding_ready=1) AS resource_binding_ready_count,SUM(hard_block_count>0) AS hard_blocked_count
FROM v_platform_capability_readiness_vector;
...
SELECT SHA2(CONCAT(g.capability_key,'|',g.gap_key),256),g.capability_key,g.gap_key,g.gap_severity,
 ...
 g.gap_description,JSON_OBJECT('first_backfill','migration_313','secrets_included',false)
FROM `v_platform_capability_assurance_gaps` g
ON DUPLICATE KEY UPDATE severity=VALUES(severity),status=CASE WHEN status='resolved' THEN 'open' ELSE status END,
...
VALUES
 ('capability_assurance_evidence_normalization','validating',JSON_ARRAY('platform_evidence_events','envelope_evidence_links','typed_gap_taxonomy'),
  JSON_ARRAY('migration_313_backfill'),JSON_ARRAY(),'run platform_capability_assurance_reconcile after registry changes','resource_authority_engine'),
 ('capability_source_resolution_population','certified',JSON_ARRAY('platform_capability_source_links','platform_capability_source_resolutions','repo_capability_candidates'),
  JSON_ARRAY('migration_313_backfill'),JSON_ARRAY(),'monitor provenance_missing debt','governed_repository_intelligence_engine'),
 ('canonical_capability_graph_backfill','certified',JSON_ARRAY('platform_plugins','platform_plugin_capabilities','platform_plugin_bindings','platform_plugin_capability_exports'),
  JSON_ARRAY('migration_313_backfill'),JSON_ARRAY(),'keep compatibility views until canonical cutover is certified','platform_orchestration_engine')
ON DUPLICATE KEY UPDATE state=VALUES(state),required_evidence_json=VALUES(required_evidence_json),
 observed_evidence_json=VALUES(observed_evidence_json),blocker_json=VALUES(blocker_json),next_action=VALUES(next_action),
 owner_engine_key=VALUES(owner_engine_key),updated_at=CURRENT_TIMESTAMP;
