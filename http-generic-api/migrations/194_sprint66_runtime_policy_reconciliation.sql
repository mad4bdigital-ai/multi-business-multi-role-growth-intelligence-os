...
VALUES
  (
    ...
    'TRUE',
    'repo_mutation|github_pr_merge|branch_delete|repo_patch_apply|gpt_tools_call|tool_dispatch',
    'adminCliRoutes|github_rest_fallback|gptToolsRoutes|repo_patch_apply',
    'TRUE',
    'Runtime preflight policy for GitHub/repo mutation safety. Blocks protected branch delete, unmerged branch delete, non-mergeable PRs, risky removed-file merges, and stale existing-branch patching without an explicit reason.',
    NOW(),
    NOW()
  ),
  (
    ...
    'FALSE',
    'Advisory visibility policy for appAdapters/index.js. Restores execution_policies participation without breaking existing app actions.',
    NOW(),
    NOW()
  ),
  (
    ...
    'TRUE',
    'Blocking app-specific runtime policy. governedExecutionPreflight blocks n8n execute_workflow unless explicit execution approval and reason are supplied.',
    NOW(),
    NOW()
  ),
  (
    ...
    'FALSE',
    'Advisory visibility policy for connectorExecutor.js. Restores preflight evidence before WordPress, MCP, or content workflow dispatch without changing existing execution behavior.',
    NOW(),
    NOW()
  ),
  (
    ...
    'FALSE',
    'Advisory visibility policy for agentLoopRunner.js. Restores execution_policies participation before model/tool loops and rule-based engine dispatch.',
    NOW(),
    NOW()
  ),
  (
    ...
    'TRUE',
    'Blocking agent-loop policy. governedExecutionPreflight blocks writing-like workflows when Brand Core evidence is absent from the governed context.',
    NOW(),
    NOW()
  )
...
INSERT INTO `policy_logic_bindings`
  (`source_policy_table`, `source_policy_id`, `source_policy_group`, `source_policy_key`, `target_policy_rule_key`, `logic_key`, `binding_role`, `binding_status`, `notes`)
SELECT
  'execution_policies',
  ep.id,
  ep.policy_group,
  ...
  END,
  NULL,
  'runtime_policy_target_rule',
  'active',
  'Current execution_policies row is bound to its target platform_engine_policy_rules representation. Runtime still reads execution_policies during transition.'
FROM `execution_policies` ep
WHERE (ep.policy_group = 'Repository Mutation Governance' AND ep.policy_key = 'Stale Duplicate Branch Merge Guard')
   OR (ep.policy_group = 'External App Action Governance' AND ep.policy_key = 'External App Action Preflight Visibility')
   OR (ep.policy_group = 'External App Action Governance' AND ep.policy_key = 'n8n Workflow Execution Guard')
   OR (ep.policy_group = 'Connector Dispatch Governance' AND ep.policy_key = 'Connector Dispatch Preflight Visibility')
   OR (ep.policy_group = 'Agent Loop Governance' AND ep.policy_key = 'Agent Loop Preflight Visibility')
   OR (ep.policy_group = 'Agent Loop Governance' AND ep.policy_key = 'Brand Writing Requires Brand Core')
ON DUPLICATE KEY UPDATE
  `source_policy_group` = VALUES(`source_policy_group`),
  `source_policy_key` = VALUES(`source_policy_key`),
  `target_policy_rule_key` = VALUES(`target_policy_rule_key`),
...
SELECT
  ...
  NULL,
  ld.logic_key,
  'legacy_policy_logic_mirror',
  'active',
  'Historical exact mirror from execution_policies to logic_definitions. Not an enforcement source; retained for traceability during policy/logic separation.'
FROM `execution_policies` ep
JOIN `logic_definitions` ld
  ON CAST(JSON_UNQUOTE(JSON_EXTRACT(ld.body_json, '$.legacy_id')) AS UNSIGNED) = ep.id
