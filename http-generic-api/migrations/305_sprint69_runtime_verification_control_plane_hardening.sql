...
('deployment_commit_unknown','deployment_parity_mismatch','release_platform','critical','manual_review',0,1,'Provide independent expected and deployed commit evidence, then rerun runtime verification.',JSON_OBJECT('diagnosis','Expected or deployed commit evidence is missing.','steps',JSON_ARRAY('supply expected main commit','read deployed commit from runtime environment','rerun verification'),'success_condition','expected_commit_sha and deployed_commit_sha are known and equal'),'active'),
...
('activation_hard_run_summary_api','Read Activation Hard Run Summary API','Read compact hard activation summary without full evidence payload. No secrets.','GET','/activation/hard-run/summary',JSON_ARRAY(),JSON_OBJECT('type','object','properties',JSON_OBJECT()),NULL,'admin,activation,runtime-verification,read_only,no_secrets,summary_first',1,11994),
