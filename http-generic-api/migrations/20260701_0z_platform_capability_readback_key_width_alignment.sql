-- Staging-local additive compatibility bridge for the immutable 20260701 readback writer.
-- Later capability projection writers reuse the 255-character capability-key domain.
...
CREATE TABLE IF NOT EXISTS `platform_capability_readback_contracts` (
  ...
  `current_contract_key` VARCHAR(191) GENERATED ALWAYS AS (CASE WHEN `is_current` = 1 THEN `contract_key` ELSE NULL END) STORED,
