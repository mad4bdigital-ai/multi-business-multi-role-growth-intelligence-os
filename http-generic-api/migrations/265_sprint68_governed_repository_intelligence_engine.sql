...
--
-- This migration extends the existing platform_resource_* layer for repository
-- development work. It does not create repository_* operation tables and V1 is
-- registry/plan-only: no comments, labels, close, merge, patch, force-push, or
-- migration apply is enabled by this migration.
--
...
VALUES
  ...
  ('repo.pr.classify_staleness', 'github_pull_request', 'classify_staleness', 'github.pull_request.classification.adapter', 'diagnostic', 'inspect',
   ...
   'summary_node', 'resource_authority_engine', 'active',
   'Plan-only PR staleness classification recipe.'),
  ...
  ('repo.file.patch_apply', 'github_file', 'patch_apply', 'github.file.patch.adapter', 'mutation', 'apply',
   ...
   'summary_node', 'resource_authority_engine', 'planned', 'Future recipe wrapper over repo_patch_apply. Existing repo_patch_apply remains the only guarded mutation surface.'),
...
VALUES
  ...
  ('repo.pr.reconciliation_sweep', 20, 'classify_pr_reconciliation_state', 'classify', NULL, NULL, NULL,
  ...
  ('repo.pr.reconciliation_sweep', 30, 'emit_no_secret_pr_evidence', 'emit_evidence', NULL, NULL, NULL,
