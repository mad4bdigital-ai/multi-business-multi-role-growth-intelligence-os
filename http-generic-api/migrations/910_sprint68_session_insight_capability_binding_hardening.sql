...
--
-- Purpose:
--   Persist the app/action/tool graph rows required by the Session Insight
--   internal SQL backlog target-write executor. This prevents future actual
--   capability envelopes from falling back to blocked_requires_setup after
--   migration 284 enabled the write executor.
--
-- Safety:
--   Additive/idempotent registry inserts only. No provider credentials,
--   no credential payload reads, no external writes, no raw transcripts,
--   no destructive statements, and no secrets.
...
INSERT INTO `app_integrations` (
  ...
  'session_insight',
  'Session Insight',
  'Internal Session Insight promotion and backlog target-write capability. No external credential or provider secret is required.',
  'mcp',
  'session_memory',
  'active'
)
...
INSERT INTO `actions` (
  `action_key`, `status`, `connector_family`, `runtime_capability_class`,
  `runtime_callable`, `primary_executor`, `notes`
) VALUES
  (
    'session_insight_development_backlog_apply',
    'active',
    'session_insight_internal_sql',
    'development_backlog',
    'TRUE',
    'session_insight_backlog_target_write_execute',
    'Internal SQL development backlog target write capability. No provider, credential, external write, raw transcript, or secret surface.'
  ),
  (
    ...
    'TRUE',
    'session_insight_backlog_target_write_execute',
    'Internal SQL integration backlog target write capability. No provider, credential, external write, raw transcript, or secret surface.'
  ),
  (
    ...
    'TRUE',
    'session_insight_backlog_target_write_execute',
    'Internal SQL runtime repair backlog target write capability. No provider, credential, external write, raw transcript, or secret surface.'
  )
ON DUPLICATE KEY UPDATE
  `status` = 'active',
...
INSERT INTO `app_integration_action_bindings` (
  ...
  (
    ...
    'manual_tools',
    'active',
    'Internal SQL target write binding; no credential required.'
  ),
  (
    'sib_integration_apply',
    'session_insight',
    'session_insight_integration_backlog_apply',
    'primary_api',
    'none',
...
INSERT INTO `app_integration_tool_bindings` (
  ...
  (
    ...
    'admin',
    'active',
    'Internal SQL write executor; no external credential required.'
  ),
  (
    'sib_target_write_list',
    'session_insight',
    ...
    'admin',
    'active',
    'Internal SQL write readback; no external credential required.'
  ),
  (
    'sib_target_write_rollback',
    'session_insight',
    ...
    'admin',
    'active',
    'Internal SQL rollback marker; no external credential required.'
  )
ON DUPLICATE KEY UPDATE
  `credential_source` = 'none',
