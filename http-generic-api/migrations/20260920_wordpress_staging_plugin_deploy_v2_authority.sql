...
(UUID(), 'remote_ssh_runtime', 'wordpress_staging_plugin_deploy', 'Deploy reviewed WordPress Staging plugin artifact', 'hosting_account', 'remote_runtime:ssh:wordpress_staging_plugin_deploy',
 ...
 'Staging-only WordPress plugin deploy v2. Source artifact is resolved from the successful exact-head WordPress Control Plane package run; the Hostinger target/path/SSH credentials are server-owned and uniquely resolved. Apply requires exact capability envelope plus same-cycle exact build provenance readback.'
)
...
('wordpress_staging_plugin_deploy',
 ...
 'Dry-run by default. Deploys only the reviewed exact-head MAD4B WordPress General Distribution artifact to the unique server-resolved ETG Staging Hostinger target. Production, Breakglass, caller-selected target/artifact/path/SSH mode/credentials and File Manager are not accepted.',
 'POST',
...
('wordpress_staging_plugin_deploy_governance','wordpress_staging_plugin_deploy_exact_artifact_guard',
 JSON_OBJECT(
   ...
   'requires',JSON_ARRAY(
     ...
     'artifact_outer_digest_verification',
     'general_distribution_manifest_and_sha256_verification',
     'handoff_v2_verification',
     ...
     'same_cycle_exact_provenance_readback',
 ...
 'Exact-head Staging-only WordPress deployment authority v2. The migration creates no target and no credentials; execution resolves one exact active+validated ETG Staging target server-side and fails closed on zero or multiple matches.'
)
ON DUPLICATE KEY UPDATE
