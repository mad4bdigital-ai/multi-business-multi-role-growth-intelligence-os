...
SELECT
  ...
  ep.policy_group,
  ep.policy_key,
  CONCAT('legacy_execution_policy_rule_', c.source_policy_id, '_v1') AS target_policy_rule_key,
  c.logic_key,
  'runtime_policy_target_rule' AS binding_role,
  'active' AS binding_status,
  'Generated by Sprint 67 target-rule backfill; evidence-only until resolver cutover approval.' AS notes
FROM policy_logic_mirror_classification c
JOIN execution_policies ep ON ep.id = c.source_policy_id
WHERE c.classification IN ('migrate_to_platform_policy_rule_blocking','migrate_to_platform_policy_rule_advisory')
  AND NOT EXISTS (
...
SELECT
  ...
  'WordPress Apply Requires Explicit Reason',
  JSON_OBJECT(
  ...
  'TRUE',
  'connector_dispatch|workflow_dispatch|wordpress|wordpress_apply',
  'connectorExecutor|connectorExecutor.js|wordpress',
  'FALSE',
...
VALUES (
  'runtime_wordpress_apply_reason_policy_v1', NULL, 'global', NULL, 'diagnose_only', 'medium',
  'high', 1, 1, 0,
...
VALUES (
  'runtime_wordpress_apply_requires_reason_advisory',
  'runtime_wordpress_apply_reason_policy_v1',
  NULL,
  100,
  'connector_dispatch',
  'wordpress_apply',
  'wordpress.apply',
  JSON_OBJECT(
    'source','execution_policies',
    'execution_policy_group','Connector Dispatch Governance',
    'execution_policy_key','WordPress Apply Requires Explicit Reason',
    'requires_explicit_reason',true,
    'min_reason_chars',10,
    'enforcement_mode','advisory',
    'cutover_enabled',false
  ...
  1,
  0,
  JSON_ARRAY('wordpress_reason_presence_check'),
  JSON_ARRAY(),
  JSON_ARRAY(),
  JSON_ARRAY(),
  'active',
...
SELECT
  'execution_policies', ep.id, ep.policy_group, ep.policy_key,
  'runtime_wordpress_apply_requires_reason_advisory', NULL,
  'runtime_policy_target_rule', 'active',
  'Advisory-only WordPress target rule binding; no blocking cutover.'
...
SELECT
  ...
  COALESCE(b.target_policy_rule_key, CONCAT('legacy_execution_policy_rule_', ep.id, '_v1')),
