...
SELECT
  CONCAT('legacy_execution_policy_target_', c.source_policy_id, '_v1'), NULL, 'global', NULL, 'diagnose_only',
  CASE WHEN c.classification = 'migrate_to_platform_policy_rule_blocking' THEN 'high' ELSE 'medium' END,
  'high', 1, 1, 0,
  JSON_OBJECT('source','legacy_execution_policies','source_policy_id',c.source_policy_id),
  JSON_ARRAY(), JSON_ARRAY(), JSON_ARRAY('*'), JSON_ARRAY(), 'active',
  CONCAT('Evidence-only target policy generated from execution_policies id ', c.source_policy_id, '; enforcement remains execution_policies until resolver cutover approval.')
...
SELECT
  CONCAT('legacy_execution_policy_rule_', c.source_policy_id, '_v1'),
  CONCAT('legacy_execution_policy_target_', c.source_policy_id, '_v1'),
  NULL,
  CASE WHEN c.classification = 'migrate_to_platform_policy_rule_blocking' THEN 200 ELSE 100 END,
  COALESCE(NULLIF(SUBSTRING_INDEX(ep.execution_scope, '|', 1), ''), 'execution_policy'),
  COALESCE(NULLIF(SUBSTRING_INDEX(ep.affects_layer, '|', 1), ''), 'runtime_policy'),
  LEFT(COALESCE(NULLIF(ep.execution_scope, ''), '*'), 500),
  JSON_OBJECT('source','legacy_execution_policies','source_policy_id',c.source_policy_id,'execution_policy_group',ep.policy_group,'execution_policy_key',ep.policy_key,'classification',c.classification,'enforcement_source','execution_policies','cutover_enabled',false),
  'execution_policy_fallback',
  CASE WHEN c.classification = 'migrate_to_platform_policy_rule_blocking' THEN 'high' ELSE 'medium' END,
  0, 1,
  CASE WHEN c.classification = 'migrate_to_platform_policy_rule_blocking' THEN 1 ELSE 0 END,
  JSON_ARRAY(), JSON_ARRAY(), JSON_ARRAY(), JSON_ARRAY(), 'active',
  CONCAT('Evidence-only target rule generated from execution_policies id ', c.source_policy_id, '; runtime enforcement remains execution_policies.')
FROM policy_logic_mirror_classification c
