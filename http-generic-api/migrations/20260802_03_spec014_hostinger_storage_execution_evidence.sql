-- Spec 014: Governed Hostinger Storage Orchestration — Wave 3 Execution Evidence
-- Additive migration candidate promoted into the governed runtime migration sequence.
-- Depends on 20260802_02_spec014_hostinger_storage_control_plane.sql.
-- Requires checksum-bound authorization, dry-run, typed confirmation, apply, and same-cycle readback.
-- No provider dispatch, credential payload, raw file content, external send, or Production promotion.
...
-- dedicated run/plan-item parent writer exists. Those parent FKs remain deferred.
-- All three tool registrations remain disabled until separate runtime certification.
