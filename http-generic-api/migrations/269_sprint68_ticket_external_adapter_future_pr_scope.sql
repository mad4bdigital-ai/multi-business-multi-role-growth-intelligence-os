...
SELECT
  s.scope_id, s.decision_id, s.checklist_id, s.proposal_id, s.adapter_key, s.family_key, s.channel,
  s.scope_status, s.registry_mutation_performed, s.adapter_implementation_performed,
  s.dispatch_enabled_changed, s.provider_dispatch_enabled_changed, s.external_send_performed,
  ...
  JSON_OBJECT('scope_id', s.scope_id, 'decision_id', s.decision_id, 'adapter_key', s.adapter_key,
    'future_pr_only', true, 'registry_mutation_performed', s.registry_mutation_performed,
    ...
    'external_send_performed', s.external_send_performed, 'secrets_included', false) AS evidence_json,
...
SELECT 'Support Ticket External Delivery Governance', 'external_adapter_future_pr_scope_policy_v1',
       JSON_OBJECT('rule','external_adapter_future_pr_scope_is_planning_only','scope_only',true,
         'adapter_implementation_allowed',false,'registry_mutation_allowed',false,
         'provider_dispatch_enablement_allowed',false,'external_send_allowed',false,
         'requires_approve_for_future_pr_decision',true,'requires_future_implementation_pr',true,
         'requires_separate_dispatch_policy',true,'no_secret_payload_read',true,'secrets_included',false),
       'TRUE',
...
WHERE NOT EXISTS (
  SELECT 1 FROM `execution_policies` WHERE `policy_group`='Support Ticket External Delivery Governance' AND `policy_key`='external_adapter_future_pr_scope_policy_v1'
);
...
('support_ticket_external_adapter_future_pr_scope_plan', 'Support Ticket External Adapter Future PR Scope Plan', 'Dry-run future implementation PR scope from an approved adapter readiness decision. Does not implement adapters, mutate registry flags, enable dispatch, or send externally.', 'POST', '/admin/support/tickets/external-send/provider-adapter-future-pr-scope/plan', JSON_ARRAY(), JSON_OBJECT('type','object','properties',JSON_OBJECT('decision_id',JSON_OBJECT('type','string'),'evidence_json',JSON_OBJECT('type','object')),'required',JSON_ARRAY('decision_id'),'additionalProperties',false), NULL, 'admin,support,tickets,external_delivery,adapter_future_pr_scope,dry_run,scope_only,no_registry_mutation,no_dispatch_enablement,no_external_send,no_secrets', 1, 490),
('support_ticket_external_adapter_future_pr_scope_record', 'Support Ticket External Adapter Future PR Scope Record', 'Record future implementation PR scope from an approved adapter readiness decision. Does not implement adapters, mutate registry flags, enable dispatch, or send externally.', 'POST', '/admin/support/tickets/external-send/provider-adapter-future-pr-scope/record', JSON_ARRAY(), JSON_OBJECT('type','object','properties',JSON_OBJECT('decision_id',JSON_OBJECT('type','string'),'evidence_json',JSON_OBJECT('type','object')),'required',JSON_ARRAY('decision_id'),'additionalProperties',false), NULL, 'admin,support,tickets,external_delivery,adapter_future_pr_scope,record_only,scope_only,no_registry_mutation,no_dispatch_enablement,no_external_send,no_secrets', 1, 491)
