-- Tenant-safe route rebinding for GPT-visible tools.
-- The live readiness audit found tenant-visible tools returning admin_backend_api_key_required,
-- which means registry rows can drift toward admin-only paths even when the route itself is
-- tenant-safe. Reassert the tenant-safe bindings and fail closed on admin-path drift.
