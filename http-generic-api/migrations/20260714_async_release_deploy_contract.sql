...
VALUES
  ('release_async_deploy_apply', 'Accept Async Release Deploy', 'Accept a governed Hostinger release deploy into the existing job queue and return HTTP 202 with operation and polling references.', 'POST', '/admin/release-operations/{operationId}/async-deploy', JSON_ARRAY('operationId'), JSON_OBJECT('type','object','required',JSON_ARRAY('operationId','target_id','expected_commit_sha','capability_envelope_id'),'properties',JSON_OBJECT('operationId',JSON_OBJECT('type','string','format','uuid'),'gate_id',JSON_OBJECT('type','string','format','uuid'),'target_id',JSON_OBJECT('type','string','format','uuid'),'app_key',JSON_OBJECT('type','string'),'app_path',JSON_OBJECT('type','string'),'branch',JSON_OBJECT('type','string','enum',JSON_ARRAY('main')),'expected_commit_sha',JSON_OBJECT('type','string','pattern','^[0-9a-fA-F]{40}$'),'capability_envelope_id',JSON_OBJECT('type','string','format','uuid'),'approval_reason',JSON_OBJECT('type','string'),'force_clean',JSON_OBJECT('type','boolean'),'restart',JSON_OBJECT('type','boolean'),'dry_run',JSON_OBJECT('type','boolean','default',false),'timeout_ms',JSON_OBJECT('type','integer','minimum',1000,'maximum',300000),'ssh_auth_mode',JSON_OBJECT('type','string'),'ssh_transport_mode',JSON_OBJECT('type','string'),'idempotency_key',JSON_OBJECT('type','string')),'additionalProperties',false), NULL, 'release_intelligence,admin,async_deploy,approval_required,capability_resolution,rollback_required,readback,same_cycle_readback,external_mutation,expected_commit_required,no_secrets', 1, 6730),
