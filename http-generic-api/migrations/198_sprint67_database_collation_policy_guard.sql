...
-- Enforces one default collation policy for future DDL and exposes readback
-- views so release readiness can detect unregistered drift. Existing legacy
-- mismatches are recorded as expiring exceptions instead of silently accepted.
...
SELECT
  ...
  CASE
    ...
    WHEN ((c.COLUMN_NAME REGEXP '(^|_)json$|_json$|json_') OR c.COLUMN_NAME IN ('path_param_keys','input_schema','fixed_body','allowed_actions','allowed_args','mcp_server_info','default_action_grants')) AND c.COLLATION_NAME NOT IN (p.json_allowed_collation, p.target_collation) THEN 'wrong_json_collation'
    WHEN NOT ((c.COLUMN_NAME REGEXP '(^|_)json$|_json$|json_') OR c.COLUMN_NAME IN ('path_param_keys','input_schema','fixed_body','allowed_actions','allowed_args','mcp_server_info','default_action_grants')) AND c.COLLATION_NAME <> p.target_collation THEN 'wrong_text_collation'
    ELSE 'ok'
...
WHERE c.TABLE_SCHEMA = DATABASE()
  ...
  AND (
    ...
    OR (((c.COLUMN_NAME REGEXP '(^|_)json$|_json$|json_') OR c.COLUMN_NAME IN ('path_param_keys','input_schema','fixed_body','allowed_actions','allowed_args','mcp_server_info','default_action_grants')) AND c.COLLATION_NAME NOT IN (p.json_allowed_collation, p.target_collation))
    OR (NOT ((c.COLUMN_NAME REGEXP '(^|_)json$|_json$|json_') OR c.COLUMN_NAME IN ('path_param_keys','input_schema','fixed_body','allowed_actions','allowed_args','mcp_server_info','default_action_grants')) AND c.COLLATION_NAME <> p.target_collation)
  );
...
('database_schema_governance','unified_collation_required',
 JSON_OBJECT('rule','unified_database_collation_required','target_character_set','utf8mb4','target_collation','utf8mb4_unicode_ci','json_allowed_collation','utf8mb4_bin','readback_views',JSON_ARRAY('v_database_collation_policy_violations','v_database_collation_policy_status')),
 'true','database_schema_governance','migrations,ddl,registry,release_readiness,workspace_authority,capability_vault','true',
 'Database DDL must use one default charset/collation: utf8mb4/utf8mb4_unicode_ci. JSON-like columns may use utf8mb4_bin only by policy.')
ON DUPLICATE KEY UPDATE
