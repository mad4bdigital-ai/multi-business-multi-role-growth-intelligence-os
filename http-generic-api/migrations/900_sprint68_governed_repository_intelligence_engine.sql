...
-- High-numbered migration intentionally avoids collisions on a highly active main branch.
-- V1 is registry/plan-only: no PR close, comment, label, merge, patch,
-- force-push, or migration apply is enabled by this migration.
-- Contract: Resource + Recipe + Authority + Policy + Evidence + Capability
-- Envelope for mutation + Same-cycle validation + Readback + Audit + no secrets.
...
VALUES
  ('github_repo', 'github_repo', 'github_api_mcp', 'GitHub Repository', NULL, NULL,
   'github://{owner}/{repo}', NULL, 'repo.pr.reconciliation_sweep', 1, 1, 0, 0,
   'resource.github_repo', 'active', JSON_OBJECT('v1_policy','plan_only','mutation_default','blocked','tenant_binding_required',true)),
  ('github_pull_request', 'github_repo', 'github_api_mcp', 'GitHub Pull Request', NULL, NULL,
   'github://{owner}/{repo}/pr/{pr_number}', 'repo.pr.classify_staleness', 'repo.pr.reconciliation_sweep', 0, 1, 0, 0,
   'resource.github_pull_request', 'active', JSON_OBJECT('v1_policy','metadata_only','mutation_default','blocked')),
  ('github_file', 'github_repo', 'github_api_mcp', 'GitHub File', NULL, NULL,
   'github://{owner}/{repo}/file/{path}', NULL, NULL, 0, 1, 1, 1,
   'resource.github_file', 'planned', JSON_OBJECT('v1_policy','patch_apply_planned_only','requires_capability_envelope',true,'force_push_allowed',false)),
  ('github_migration', 'github_repo', 'github_api_mcp', 'GitHub Migration File', NULL, NULL,
   'github://{owner}/{repo}/migration/{path}', NULL, 'repo.migration.detect_conflicts', 0, 1, 1, 0,
   'resource.github_migration', 'planned', JSON_OBJECT('v1_policy','detect_conflicts_only','tenant_migration_apply_allowed',false))
...
VALUES
  ('github.repo.pr_reconciliation.adapter', 'github_repo', 'github_api_mcp', 'endpoint_recipe', NULL,
   'parse_github_repo_ref', 'normalize_github_repo_metadata', 'normalize_github_pull_requests', 'metadata_only', 1, 1, 0, 'active',
   JSON_OBJECT('v1_execution','plan_only','mutation_allowed_v1',false)),
  ('github.pull_request.classification.adapter', 'github_pull_request', 'github_api_mcp', 'endpoint_recipe', NULL,
   'parse_github_pull_request_ref', 'normalize_github_pull_request_metadata', NULL, 'metadata_only', 1, 1, 0, 'active',
   JSON_OBJECT('v1_execution','plan_only','mutation_allowed_v1',false)),
  ('github.file.patch.adapter', 'github_file', 'github_api_mcp', 'composite', NULL,
   'parse_github_file_ref', 'normalize_repo_patch_context', NULL, 'content_hash_only', 1, 0, 0, 'planned',
...
VALUES
  ('repo.pr.reconciliation_sweep', 'github_repo', 'pr_reconciliation_sweep', 'github.repo.pr_reconciliation.adapter', 'diagnostic', 'reconcile',
   1, 1, 0, 0, 0, NULL,
   JSON_OBJECT('type','object','required', JSON_ARRAY('owner','repo')),
   JSON_OBJECT('type','object','required', JSON_ARRAY('classification','summary','recommendations','secrets_included')),
   JSON_OBJECT('v1','dry_run_only','forbidden_v1',JSON_ARRAY('comment','label','close','merge','patch','force_push','migration_apply'),'secrets_allowed',false),
   'summary_node', 'resource_authority_engine', 'active', 'Plan/read-only foundation recipe for repository PR reconciliation sweep.'),
  ('repo.pr.classify_staleness', 'github_pull_request', 'classify_staleness', 'github.pull_request.classification.adapter', 'diagnostic', 'inspect',
   1, 1, 0, 0, 0, NULL,
   JSON_OBJECT('type','object','required', JSON_ARRAY('owner','repo','pr_number')),
   JSON_OBJECT('type','object','required', JSON_ARRAY('classification','confidence','recommended_action','secrets_included')),
   JSON_OBJECT('classification_model',JSON_ARRAY('merge_ready','behind_only','superseded_by_main','duplicate_migration_conflict','manual_review_required','unsafe_to_merge'),'v1_execution','plan_only','secrets_allowed',false),
   'summary_node', 'resource_authority_engine', 'active', 'Plan-only PR staleness classification recipe.'),
  ('repo.pr.close_superseded', 'github_pull_request', 'close_superseded', 'github.pull_request.classification.adapter', 'mutation', 'apply',
   ...
   'summary_node', 'resource_authority_engine', 'planned', 'Future guarded close-superseded PR recipe. Disabled in V1.'),
  ('repo.file.patch_apply', 'github_file', 'patch_apply', 'github.file.patch.adapter', 'mutation', 'apply',
   ...
   'summary_node', 'resource_authority_engine', 'planned', 'Future recipe wrapper over repo_patch_apply.'),
  ('repo.migration.detect_conflicts', 'github_migration', 'detect_conflicts', 'github.repo.pr_reconciliation.adapter', 'diagnostic', 'compare',
   ...
   JSON_OBJECT('v1','plan_only','tenant_migration_apply_allowed',false,'detect_duplicate_numbers',true,'secrets_allowed',false),
   'summary_node', 'resource_authority_engine', 'planned', 'Future migration conflict detection recipe. Tenant migration apply remains disabled.')
ON DUPLICATE KEY UPDATE
  `resource_type` = VALUES(`resource_type`),
...
VALUES
  ('Repository Intelligence Governance', 'governed_repository_intelligence_engine_policy_v1',
   JSON_OBJECT(
     ...
     'contract',JSON_ARRAY('resource','recipe','authority','policy','evidence','capability_envelope_for_mutation','same_cycle_validation','readback','audit','no_secret'),
     'v1','dry_run_plan_only',
     'required_tables',JSON_ARRAY('platform_resource_types','platform_resource_adapters','platform_resource_recipes','platform_resource_authority_requirements','capability_resolution_envelope_ledger','audit_payload_evidence'),
