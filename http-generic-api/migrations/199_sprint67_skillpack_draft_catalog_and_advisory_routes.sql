...
-- Matt Pocock skillpack path-scope review: keep deprecated/in-progress/personal
-- assets as evidence-only and allow stable assets only.
...
SET c.import_status='blocked_evidence_only',
    c.policy_json=JSON_OBJECT('content_recognition','reviewed_path_scope_blocked','path_scope_reviewed',true,'blocked_reason','deprecated_in_progress_or_personal_path_not_allowed_for_runtime_import','review_decision','blocked_evidence_only','raw_execution',false,'secret_values_returned',false)
WHERE r.full_name='mad4bdigital-ai/skills'
  AND c.source_path IN (
    'skills/deprecated/design-an-interface/SKILL.md','skills/deprecated/qa/SKILL.md','skills/deprecated/request-refactor-plan/SKILL.md','skills/deprecated/ubiquitous-language/SKILL.md',
    'skills/in-progress/review/SKILL.md','skills/in-progress/teach/SKILL.md','skills/in-progress/writing-beats/SKILL.md','skills/in-progress/writing-fragments/SKILL.md','skills/in-progress/writing-shape/SKILL.md',
    'skills/personal/edit-article/SKILL.md','skills/personal/obsidian-vault/SKILL.md'
  );
...
SET certification_status='certification_ready',
    ...
    blocked_asset_manifest_json=JSON_OBJECT('blocked_reason','deprecated_in_progress_or_personal_path_scope','runtime_import_allowed_assets',18,'blocked_assets_remaining',11,'runtime_tool_grants_created',false),
...
SELECT 'install_nagy_superpowers_v1', @tenant_id, @super_pkg_id, @super_ver_id,
 JSON_OBJECT('scope_type','tenant','runtime_enabled',false,'draft',true,'advisory_agent_skill_enabled',false,'package_key','skillpack_superpowers'), JSON_ARRAY(), JSON_ARRAY(),
...
SELECT 'install_nagy_seo_geo_claude_skills_v', @tenant_id, @seo_pkg_id, @seo_ver_id,
 JSON_OBJECT('scope_type','tenant','runtime_enabled',false,'draft',true,'advisory_agent_skill_enabled',false,'package_key','skillpack_seo_geo_claude_skills','webfetch_enabled',false), JSON_ARRAY(), JSON_ARRAY(),
 JSON_OBJECT('auto_enable',false,'requires_explicit_runtime_tool_grant',true,'source','capability_vault_manual_tenant_private_draft','forbidden_without_grant',JSON_ARRAY('file_write','deploy','shell','package_install','publish','secret_read','provider_write','webfetch')),
