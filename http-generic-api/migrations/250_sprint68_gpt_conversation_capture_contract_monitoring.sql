-- Sprint 68: GPT conversational turn capture contract monitoring.
-- After this contract, new GPT sessions with tool activity must also capture user and assistant turns.
-- Backfilled legacy archives remain exempt because they have explicit archive_backfill evidence.
...
UNION ALL
SELECT session_id, 'conversation_capture_missing_after_contract' AS issue_code, 'fail' AS severity, evidence_json
FROM `v_gpt_session_archive_monitoring` m
WHERE started_at >= '2026-06-09 00:00:00'
  AND tool_turns > 0
