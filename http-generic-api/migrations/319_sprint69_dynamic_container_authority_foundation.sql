...
CREATE TABLE IF NOT EXISTS `container_classification_type_registry` (
  ...
  `merge_strategy` ENUM('deny_wins','union','intersection','minimum','maximum','nearest_replace','priority_replace') NOT NULL DEFAULT 'nearest_replace',
...
CREATE TABLE IF NOT EXISTS `container_resource_dimension_registry` (
  ...
  `default_merge_strategy` ENUM('deny_wins','union','intersection','minimum','maximum','nearest_replace','priority_replace') NOT NULL DEFAULT 'deny_wins',
  ...
  `default_override_risk_class` VARCHAR(64) NOT NULL DEFAULT 'standard',
...
INSERT INTO `container_classification_type_registry`
  (`classification_type_key`,`display_name`,`description`,`value_schema_json`,`eligible_container_types_json`,`cardinality`,`inheritance_mode`,`merge_strategy`,`conflict_policy`,`affected_dimensions_json`,`status`,`metadata_json`)
VALUES
  ('governance_profile','Governance Profile','Describes the governance posture inherited by a container.',JSON_OBJECT('type','string','enum',JSON_ARRAY('baseline','standard','controlled','strict')),JSON_ARRAY('platform','tenant','workspace','brand','activity','workflow'),'single','inherit_down','nearest_replace','block',JSON_ARRAY('rules','policies','roles'),'active',JSON_OBJECT('seed','migration_319','grants_authority',false)),
  ('brand_maturity','Brand Maturity','Describes the maturity stage that influences defaults without granting authority.',JSON_OBJECT('type','string','enum',JSON_ARRAY('new','emerging','growing','mature','transforming')),JSON_ARRAY('brand','activity','workflow'),'single','inherit_down','nearest_replace','block',JSON_ARRAY('profiles','workflows','knowledge'),'active',JSON_OBJECT('seed','migration_319','grants_authority',false)),
  ('runtime_scope_class','Runtime Scope Class','Describes the intended operational scope of a container.',JSON_OBJECT('type','string','enum',JSON_ARRAY('isolated','tenant_shared','workspace_shared','brand_scoped','activity_scoped','workflow_scoped')),JSON_ARRAY('tenant','workspace','brand','activity','workflow'),'single','inherit_down','nearest_replace','block',JSON_ARRAY('connections','tools','actions','endpoints'),'active',JSON_OBJECT('seed','migration_319','grants_authority',false)),
  ('data_sensitivity_level','Data Sensitivity Level','Numeric sensitivity ceiling; the highest inherited value wins.',JSON_OBJECT('type','integer','minimum',0,'maximum',4),JSON_ARRAY('platform','tenant','workspace','brand','activity','workflow'),'single','inherit_down','maximum','restrict',JSON_ARRAY('knowledge','assets','connections','credentials','policies'),'active',JSON_OBJECT('seed','migration_319','scale',JSON_OBJECT('0','public','1','internal','2','confidential','3','restricted','4','critical'),'grants_authority',false)),
  ('execution_risk_level','Execution Risk Level','Numeric execution risk; the highest inherited value wins.',JSON_OBJECT('type','integer','minimum',0,'maximum',4),JSON_ARRAY('tenant','workspace','brand','activity','workflow'),'single','inherit_down','maximum','restrict',JSON_ARRAY('actions','endpoints','workflows','policies'),'active',JSON_OBJECT('seed','migration_319','scale',JSON_OBJECT('0','read_only','1','standard','2','high','3','critical','4','destructive'),'grants_authority',false)),
  ('site_governance_model','Site Governance Model','Describes the brand/site governance operating model.',JSON_OBJECT('type','string','maxLength',191),JSON_ARRAY('brand','activity','workflow'),'single','inherit_down','nearest_replace','block',JSON_ARRAY('profiles','rules','workflows'),'active',JSON_OBJECT('seed','migration_319','legacy_source','brands.site_governance_model','grants_authority',false)),
  ('starter_pack_profile','Starter Pack Profile','Selects a container-specific starter profile.',JSON_OBJECT('type','string','maxLength',191),JSON_ARRAY('brand','activity','workflow'),'single','inherit_down','nearest_replace','block',JSON_ARRAY('profiles','skills','tools','workflows'),'active',JSON_OBJECT('seed','migration_319','legacy_source','brands.starter_pack_profile','grants_authority',false)),
  ('language_scope_class','Language Scope Class','Describes language and localization scope.',JSON_OBJECT('type','string','maxLength',191),JSON_ARRAY('brand','activity','workflow'),'single','inherit_down','nearest_replace','block',JSON_ARRAY('profiles','knowledge','workflows'),'active',JSON_OBJECT('seed','migration_319','legacy_source','brands.language_scope_class','grants_authority',false))
ON DUPLICATE KEY UPDATE
  display_name=VALUES(display_name),description=VALUES(description),value_schema_json=VALUES(value_schema_json),
  eligible_container_types_json=VALUES(eligible_container_types_json),cardinality=VALUES(cardinality),inheritance_mode=VALUES(inheritance_mode),
  merge_strategy=VALUES(merge_strategy),conflict_policy=VALUES(conflict_policy),affected_dimensions_json=VALUES(affected_dimensions_json),
  metadata_json=VALUES(metadata_json),updated_at=CURRENT_TIMESTAMP;
