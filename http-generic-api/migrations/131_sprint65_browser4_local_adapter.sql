...
UPDATE `browser_runtime_registry`
   SET `status` = CASE WHEN `status` IN ('planned', 'blocked_missing_java17', 'planned_poc_partial_java17_ready') THEN 'planned_adapter_available_after_connector_upgrade' ELSE `status` END,
       `metadata_json` = JSON_SET(COALESCE(`metadata_json`, JSON_OBJECT()), '$.adapter.route', '/browser-runtime/inspect-site/run', '$.adapter.connector_path', '/browser4', '$.adapter.session_lifecycle', JSON_ARRAY('open', 'goto', 'snapshot_or_screenshot'), '$.adapter.requires_connector_upgrade', true)
