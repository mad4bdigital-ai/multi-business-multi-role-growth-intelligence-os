...
async function loadRuntimeConfig(connection) {
  ...
  const bridgeConfig = safeJsonParse(bridgeRow?.config_json, {});
  const rollupConfig = safeJsonParse(rollupRow?.config_json, {});
  const scopeConfig = safeJsonParse(byKey.get("dynamic_audit_checkpoint_scope")?.config_json, {});
...
async function currentCommitSha() {
  ...
  try {
    ...
    for (const key of ["commit_sha", "commitSha", "git_commit_sha", "head_sha"]) {
      if (/^[0-9a-f]{40}$/i.test(String(manifest[key] || ""))) {
        return String(manifest[key]).toLowerCase();
      }
    }
  } catch {
    // Runtime provenance remains explicitly unknown when no manifest is available.
  }
  return null;
}
  ...
  for (const source of sources) {
    ...
    for (const row of rows) {
      ...
      const [eventResult] = await connection.query(
        ...
        [
          eventKey,
          String(row.source_id).slice(0, 191),
          source.eventType,
          source.resourceKind,
      ...
      const [assetResult] = await connection.query(
        ...
        [
          ...
          eventKey,
          source.eventType,
          readbackVerified ? "readback_verified" : "observed",
          safeJson(evidence),
          row.created_at || null,
          eventKey,
        ]
...
async function produceReleaseReadinessEvent(connection) {
  if (!(await tableExists(connection, "release_readiness_log"))) {
    return { inserted: 0, reason: "release_readiness_log_missing" };
  }
  const [rows] = await connection.query(
  ...
  const status = Number(row.fail_count || 0) > 0
    ? "fail"
    : Number(row.warn_count || 0) > 0
      ? "warn"
      : "pass";
...
async function produceRepoFileAudit(connection, commitSha, limit) {
  ...
  const [existing] = await connection.query(
    "SELECT run_id FROM repo_file_audit_runs WHERE run_id=? LIMIT 1",
    [runId]
  );
  if (existing.length) {
    return { run_inserted: 0, findings_inserted: 0, run_id: runId, reason: "run_exists" };
  }
  ...
  await connection.query(
    ...
    [
      ...
      "main",
      commitSha,
      `repo_file_audit:${runId}`,
      safeJson({
        changed_file_observation_count: files.length,
        audit_depth: "changed_files_from_governed_patch_evidence",
        full_repo_exhaustive: false,
        secrets_included: false,
      }),
      safeJson({
        source: "audit_log.repo_patch_apply",
        commit_sha: commitSha,
        raw_patch_content_stored: false,
        secrets_included: false,
      }),
    ]
  );
...
async function writeCheckpoint(connection, config, commitSha) {
  ...
  const oldEnough = Number.isFinite(oldestAt) &&
    Date.now() - oldestAt >= maxAgeMinutes * 60_000;
  if (rows.length < minEvents && !oldEnough) {
    return { written: 0, planned_count: rows.length, reason: "checkpoint_threshold_not_met" };
  }
...
async function recordSchedulerFinish(connection, runId, status, result, error = null) {
  ...
  await connection.query(
    ...
    [
      ...
      safeJson(result),
      error?.code || null,
      boundedText(error?.message || "", 1000) || null,
      runId,
    ]
  );
}
...
export async function runDynamicAuditCycle(options = {}, dependencies = {}) {
  ...
  try {
    ...
    const bridge = await runAuditLogEventBusBridge(
      {
        apply: true,
        confirm: AUDIT_BRIDGE_CONFIRMATION,
    ...
    const rollup = await runAuditEventRollupBuilder(
      {
        apply: true,
        confirm: AUDIT_ROLLUP_CONFIRMATION,
        limit: config.batch_limit,
      },
      { pool }
    );
    ...
    if (lockAcquired) {
      await connection.query("SELECT RELEASE_LOCK(?) AS released", [CYCLE_LOCK]).catch(() => {});
    }
    connection.release();
  }
}
...
export async function startDynamicAuditScheduler(options = {}) {
  ...
  if (!config.enabled) {
    return { started: false, reason: "runtime_config_disabled", secrets_included: false };
  }
