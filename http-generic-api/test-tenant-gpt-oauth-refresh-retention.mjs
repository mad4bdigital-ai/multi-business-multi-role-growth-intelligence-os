...
const readyProbe = await tenantGptRefreshReady({ TENANT_GPT_REFRESH_TOKENS_ENABLED: "true", JWT_SECRET: "test-refresh-secret-32-characters-long" }, { query: async (sql) => sql.includes("index_count") ? [[{ index_count: 5 }]] : [[{ present: 1 }]] });
assert.equal(readyProbe.ready, true);
assert.equal(readyProbe.indexes_present, true);
...
assert.match(metadata, /indexes_present/u);
