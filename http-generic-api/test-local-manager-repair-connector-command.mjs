...
assert.ok(program.includes('ConnectorRuntimeVerificationAttempts = 18'));
assert.ok(program.includes('ConnectorRuntimeVerificationDelay = TimeSpan.FromSeconds(5)'));
assert.ok(program.includes('LocalConnectorFootprint.AssessAsync()'));
assert.ok(program.includes('waiting for connector services, tunnel, heartbeat, and route'));
assert.ok(program.includes('verification_attempts = attempts'));
assert.ok(program.includes('cloudflared_running = lastFootprint?.CloudflaredRunning'));
...
assert.ok(program.includes('Math.Max(localBackoffWithJitter, serverRetryAfterSeconds ?? 0)'), 'desktop polling must not retry before the server cooldown expires');
