...
function failure(status, details = {}) {
  return { ok: false, status, ...details, provider_apply_allowed: false, mutation_allowed: false, enforcement_cutover: false, secrets_included: false };
}

export function stableAdapterJson(value) {
  if (value === null || typeof value !== "object") return JSON.stringify(value);
  if (Array.isArray(value)) return `[${value.map((item) => stableAdapterJson(item)).join(",")}]`;
  return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${stableAdapterJson(value[key])}`).join(",")}}`;
}

export function adapterHash(value) {
  return crypto.createHash("sha256").update(stableAdapterJson(value)).digest("hex");
}
...
function adapterManifest(binding = {}) {
  return {
    ...
    boundary_key: binding.boundary_key,
    resource_type: binding.resource_type,
    operations: binding.operations,
    certification_requirements: binding.certification_requirements,
    readback_contract_hash: binding.readback_contract_hash,
    drift_policy_hash: binding.drift_policy_hash,
...
function certificationManifest(certification = {}) {
  return {
    certification_version: certification.certification_version,
    adapter_binding_id: certification.adapter_binding_id,
    execution_control_hash: certification.execution_control_hash,
    binding_hash: certification.binding_hash,
    certification_status: certification.certification_status,
    checks_hash: certification.checks_hash,
    provider_apply_allowed: false,
    mutation_allowed: false,
    enforcement_cutover: false,
    secrets_included: false,
...
export function buildAdapterBinding({ adapter_key = "", provider_key = "", capability_key = "", boundary_key = "", resource_type = "", operations = [], certification_requirements = [], readback_contract = {}, drift_policy = {}, priority = 100 } = {}) {
  const binding = {
    ok: true,
    binding_version: "platform_adapter_binding_contract_v1",
    adapter_binding_id: crypto.randomUUID(),
    adapter_key: safeText(adapter_key, 191),
    provider_key: safeText(provider_key, 191),
    capability_key: safeText(capability_key, 191),
    boundary_key: safeText(boundary_key, 191),
    resource_type: safeText(resource_type, 191),
    operations: normalizeList(operations),
    certification_requirements: normalizeList(certification_requirements),
    readback_contract,
    readback_contract_hash: adapterHash(readback_contract || {}),
    drift_policy,
    drift_policy_hash: adapterHash(drift_policy || {}),
...
export function certifyAdapterBinding({ binding = {}, execution_control = {}, execution_envelope = {}, approval_request = {}, decision_log = [], current_enforcement = {}, now = new Date() } = {}) {
  ...
  const checks = [
    { name: "binding_hash_verified", pass: true },
    { name: "execution_concurrency_ready", pass: true },
    { name: "readback_contract_present", pass: Boolean(binding.readback_contract_hash) },
    { name: "drift_policy_present", pass: Boolean(binding.drift_policy_hash) },
    { name: "provider_apply_forbidden", pass: true },
    { name: "no_mutation", pass: true },
    { name: "no_enforcement_cutover", pass: true },
    { name: "no_secrets", pass: true },
  ];
  const certification = {
    ok: true,
    certification_version: "platform_adapter_certification_v1",
    adapter_binding_id: binding.adapter_binding_id,
    adapter_key: binding.adapter_key,
    execution_control_hash: execution_control.control_hash,
    binding_hash: binding.binding_hash,
    certification_status: checks.every((check) => check.pass) ? "certified_contract_only" : "certification_failed",
    checks,
    checks_hash: adapterHash(checks),
    provider_apply_allowed: false,
    mutation_allowed: false,
    enforcement_cutover: false,
    secrets_included: false,
...
export function selectDeterministicAdapterBinding(bindings = [], { capability_key = "", boundary_key = "", resource_type = "" } = {}) {
  const filtered = (Array.isArray(bindings) ? bindings : [])
    .filter((binding) => binding?.ok === true)
    .filter((binding) => !capability_key || binding.capability_key === capability_key)
    .filter((binding) => !boundary_key || binding.boundary_key === boundary_key)
    .filter((binding) => !resource_type || binding.resource_type === resource_type)
    .filter((binding) => bool(binding.provider_apply_allowed) === false && bool(binding.mutation_allowed) === false)
    .sort((a, b) => Number(a.priority || 100) - Number(b.priority || 100)
      || String(a.adapter_key).localeCompare(String(b.adapter_key))
      || String(a.binding_hash).localeCompare(String(b.binding_hash)));
