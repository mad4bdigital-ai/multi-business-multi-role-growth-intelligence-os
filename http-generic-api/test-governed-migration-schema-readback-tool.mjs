...
const fakePool = {
  async query(query, params = []) {
    ...
    if (/FROM INFORMATION_SCHEMA\.TABLES/i.test(query)) return [[{ TABLE_NAME: "operational_alert_lifecycle_events" }]];
    if (/FROM INFORMATION_SCHEMA\.COLUMNS/i.test(query)) return [[
    ...
    if (/FROM INFORMATION_SCHEMA\.STATISTICS/i.test(query)) return [[
