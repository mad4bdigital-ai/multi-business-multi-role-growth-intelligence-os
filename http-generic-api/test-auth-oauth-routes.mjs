...
try {
  ...
  const codeResult = await postJson(baseUrl, "/auth/oauth/code", { token: userToken, redirect_uri: redirectUri, state, scope: TENANT_SCOPE, resource: ACTIVATION_RESOURCE, activation_context: activationContext });
  ...
  assert("authorization code binds the Activation resource", decodedAuthorizationCode?.resource === ACTIVATION_RESOURCE, JSON.stringify(decodedAuthorizationCode));
  ...
  assert("access JWT preserves the tenant GPT audience", Array.isArray(accessPayload.aud) && accessPayload.aud.includes("mad4b-tenant-gpt"), JSON.stringify(accessPayload));
  assert("access JWT includes the Activation resource audience", Array.isArray(accessPayload.aud) && accessPayload.aud.includes(ACTIVATION_RESOURCE), JSON.stringify(accessPayload));
