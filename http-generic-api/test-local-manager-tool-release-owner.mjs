...
const localManagerControlTemplateMigration = readFileSync('migrations/20260922_local_manager_control_templates_registry.sql', 'utf8');
...
assert(localManager.includes('LOCAL_MANAGER_WINDOWS_LATEST_VERSION = "0.2.30"'), 'public Local Manager update route must advertise Windows 0.2.30');
assert(localManager.includes('Mad4B-Local-Manager-Setup-0.2.30.exe'), 'public Local Manager download route must point at Windows 0.2.30 assets');
...
assert(localManagerProject.includes('<Version>0.2.30</Version>'), 'Windows project Version must match advertised release');
assert(localManagerProject.includes('<AssemblyVersion>0.2.30.0</AssemblyVersion>'), 'Windows project AssemblyVersion must match advertised release');
assert(localManagerProject.includes('<FileVersion>0.2.30.0</FileVersion>'), 'Windows project FileVersion must match advertised release');
assert(localManagerProject.includes('<InformationalVersion>0.2.30-update-response-recovery</InformationalVersion>'), 'Windows project InformationalVersion must identify the update response recovery build');
...
assert(localManagerDeviceLinkService.includes('local_manager_control_template_registry_unavailable'), 'registry read failures must fail closed explicitly');
assert(localManagerControlTemplateMigration.includes('CREATE TABLE IF NOT EXISTS `local_manager_control_templates`'), 'repository migration must own control-template schema');
assert(localManagerControlTemplateMigration.includes('ON DUPLICATE KEY UPDATE'), 'repository migration must own deterministic seed convergence');
for (const templateKey of ['powershell_admin','windows_control','hermes_agent_surface','auto_browser','managed_n8n_client','tenant_dedicated_n8n','edge','chrome','firefox','brave','opera','chromium','browserbase','browserless','steel_browser','playwright_adapter','puppeteer_adapter','selenium_adapter','vscode','cursor','open_claude','open_claw','notepad','git_bash']) {
  assert(localManagerControlTemplateMigration.includes(`'${templateKey}'`), `governed control-template migration must seed ${templateKey}`);
}
assert(localManagerControlTemplateMigration.includes('"app_manager_scope":"managed_mad4b_service_side"'), 'managed service-side apps must carry app_manager_scope metadata');
assert(localManagerControlTemplateMigration.includes('"app_manager_scope":"tenant_local_device_side"'), 'tenant local device-side apps must carry app_manager_scope metadata');
assert(localManagerControlTemplateMigration.includes('"current_hosting_target":"essam_local_device"'), 'managed n8n client must record current hosting target');
assert(localManagerControlTemplateMigration.includes('"future_hosting_target":"vps"'), 'managed n8n client must record future VPS target');
assert(localManagerControlTemplateMigration.includes('"integration_type":"managed_service_client"'), 'managed n8n must be classified as managed_service_client');
assert(localManagerControlTemplateMigration.includes('"integration_type":"tenant_local_service"'), 'tenant n8n must be classified as tenant_local_service');
assert(localManagerControlTemplateMigration.includes('"integration_type":"external_provider"'), 'browser providers must be classified as external providers');
assert(localManagerControlTemplateMigration.includes('"integration_type":"plugin_adapter"'), 'browser adapters must be classified as plugin adapters');
