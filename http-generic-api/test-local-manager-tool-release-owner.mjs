...
assert(localManager.includes('LOCAL_MANAGER_WINDOWS_LATEST_VERSION = "0.2.18"'), 'public Local Manager update route must advertise Windows 0.2.18');
assert(localManager.includes('Mad4B-Local-Manager-Setup-0.2.18.exe'), 'public Local Manager download route must point at Windows 0.2.18 assets');
...
assert(localManagerProject.includes('<Version>0.2.18</Version>'), 'Windows project Version must match advertised release');
assert(localManagerProject.includes('<AssemblyVersion>0.2.18.0</AssemblyVersion>'), 'Windows project AssemblyVersion must match advertised release');
assert(localManagerProject.includes('<FileVersion>0.2.18.0</FileVersion>'), 'Windows project FileVersion must match advertised release');
assert(localManagerProject.includes('<InformationalVersion>0.2.18-app-managed-connector-repair</InformationalVersion>'), 'Windows project InformationalVersion must identify the app-managed connector repair build');
