...
async function readRecordedRows({ snapshotKey, recommendationKey }) {
  ...
  return { snapshot: snapshotRows[0] || null, recommendation: recommendationRows[0] || null };
}

export async function recordSupportTicketLifecycleSnapshot(input = {}) {
  const ticketId = normalizeKey(input.ticket_id || input.ticketId, "ticket_id", 128);
  ...
  const proposal = await proposeSupportTicketLifecycleSnapshot({ ticket_id: ticketId });
  if (proposal.candidate_sha256 !== candidateSha256) {
    ...
    err.details = { ticket_id: ticketId, expected_candidate_sha256: proposal.candidate_sha256, received_candidate_sha256: candidateSha256, secrets_included: false };
    throw err;
  }
  ...
  if (Number(envelope.apply_allowed) !== 1) {
    const err = new Error("capability envelope is ready for dispatch but not apply-allowed for Support Ticket snapshot recording.");
    err.status = 403;
    err.code = "capability_envelope_apply_not_allowed";
    err.details = { envelope_id: envelope.envelope_id, envelope_status: envelope.envelope_status, decision: envelope.decision, dispatch_allowed: Boolean(envelope.dispatch_allowed), apply_allowed: Boolean(envelope.apply_allowed), secrets_included: false };
    throw err;
  }
  ...
  try {
    ...
    await conn.query(
      `INSERT INTO platform_orchestration_state_snapshots (
       ...
       ON DUPLICATE KEY UPDATE updated_at = CURRENT_TIMESTAMP, status = 'recorded'`,
      [snapshotId, snapshotKey, snapshotCandidate.plugin_key, snapshotCandidate.scope_type || "ticket", snapshotCandidate.scope_id || ticketId, snapshotCandidate.tenant_id || null, snapshotCandidate.workspace_id || null, snapshotCandidate.brand_key || null, snapshotCandidate.subject_key || `ticket:${ticketId}`, snapshotCandidate.state_classification, snapshotCandidate.maturity_score, json(snapshotCandidate.input_sources), json(snapshotCandidate.state), json(snapshotCandidate.maturity), json(snapshotCandidate.blockers), json({ ...snapshotCandidate.safety, candidate_sha256: candidateSha256, idempotency_key: idempotencyKey, capability_envelope_id: capabilityEnvelopeId }), "orchestration_intelligence_engine"]
    );
    const [snapshotRows] = await conn.query(`SELECT snapshot_id FROM platform_orchestration_state_snapshots WHERE snapshot_key = ? LIMIT 1`, [snapshotKey]);
    ...
    await conn.query(
      `INSERT INTO platform_orchestration_recommendations (
       ...
       ON DUPLICATE KEY UPDATE snapshot_id = VALUES(snapshot_id), updated_at = CURRENT_TIMESTAMP, recommendation_status = 'proposed'`,
      [recommendationId, recommendationKey, recordedSnapshotId, recommendationCandidate.plugin_key, recommendationCandidate.scope_type || "ticket", recommendationCandidate.scope_id || ticketId, recommendationCandidate.task_class, recommendationCandidate.recommendation_type, recommendationCandidate.priority, json(recommendationCandidate.decision), json(recommendationCandidate.blockers), json(recommendationCandidate.next_actions), json({ ...recommendationCandidate.safety_contract, candidate_sha256: candidateSha256, idempotency_key: idempotencyKey, capability_envelope_id: capabilityEnvelopeId }), "orchestration_intelligence_engine"]
    );
    await conn.commit();
  } catch (err) {
