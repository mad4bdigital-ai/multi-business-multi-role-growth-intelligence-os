...
const migrationName = "1031_sprint69_registry_export_orphan_cleanup.sql";
...
assert.match(migration, /LEFT JOIN endpoints endpoint_row/);
assert.match(migration, /SET export_row\.status = 'archived'/);
assert.match(migration, /export_row\.status = 'active'/);
assert.match(migration, /NOT EXISTS \(/);
assert.match(migration, /FROM platform_tool_dispatch_bindings binding_row/);
...
assert.match(migration, /archived non-callable export/);
...
console.log("platform endpoint export orphan cleanup tests passed");
