...
function toDate(value, fallback = new Date()) { const date = value instanceof Date ? value : new Date(value || fallback); return Number.isNaN(date.getTime()) ? fallback : date; }
function addSeconds(date, seconds) { return new Date(date.getTime() + seconds * 1000); }
function ttlSeconds(value) { const parsed = Number(value || DEFAULT_TTL_SECONDS); if (!Number.isFinite(parsed) || parsed <= 0) return DEFAULT_TTL_SECONDS; return Math.min(Math.floor(parsed), MAX_TTL_SECONDS); }
function failure(status, details = {}) { return { ok: false, status, ...details, secrets_included: false }; }
...
export function approvalHash(value) { return crypto.createHash("sha256").update(stableApprovalJson(value)).digest("hex"); }
...
function requestManifest(request = {}) {
  return { request_version: request.request_version, approval_request_id: request.approval_request_id, execution_envelope_id: request.execution_envelope_id, execution_envelope_manifest_hash: request.execution_envelope_manifest_hash, request_scope_hash: request.request_scope_hash, requested_by: request.requested_by, issued_at: request.issued_at, expires_at: request.expires_at, provider_apply_allowed: false, mutation_allowed: false, enforcement_cutover: false, secrets_included: false };
}
function decisionMaterial(record = {}) {
  return { decision_version: record.decision_version, approval_request_id: record.approval_request_id, request_manifest_hash: record.request_manifest_hash, decision_sequence: record.decision_sequence, previous_decision_hash: record.previous_decision_hash || null, decision: record.decision, decided_by: record.decided_by, decided_at: record.decided_at, decision_note_hash: record.decision_note_hash, provider_apply_allowed: false, mutation_allowed: false, enforcement_cutover: false, secrets_included: false };
}
...
export function buildScopedApprovalRequest({ execution_envelope = {}, requested_scope = {}, requested_by = "", reason = "", issued_at = new Date(), ttl_seconds = DEFAULT_TTL_SECONDS } = {}) {
  ...
  const request = { ok: true, request_version: "platform_scoped_approval_request_v1", approval_request_id: crypto.randomUUID(), execution_envelope_id: execution_envelope.envelope_id, execution_envelope_manifest_hash: execution_envelope.manifest_hash, request_scope: scope, request_scope_hash: approvalHash(scope), requested_by: safeText(requested_by, 191) || "system", reason: safeText(reason, 1000), status: "pending_approval", issued_at: issuedAt.toISOString(), expires_at: addSeconds(issuedAt, ttl).toISOString(), ttl_seconds: ttl, single_use: true, provider_apply_allowed: false, mutation_allowed: false, enforcement_cutover: false, secrets_included: false };
...
export function appendApprovalDecision({ approval_request = {}, decision_log = [], decision = "", decided_by = "", decision_note = "", decided_at = new Date() } = {}) {
  ...
  const record = { decision_version: "platform_scoped_approval_decision_v1", approval_request_id: approval_request.approval_request_id, request_manifest_hash: approval_request.manifest_hash, decision_sequence: decision_log.length + 1, previous_decision_hash: previous?.decision_hash || null, decision: normalizedDecision, decided_by: safeText(decided_by, 191) || "system", decided_at: toDate(decided_at, new Date()).toISOString(), decision_note_hash: approvalHash(safeText(decision_note, 1000)), provider_apply_allowed: false, mutation_allowed: false, enforcement_cutover: false, secrets_included: false };
