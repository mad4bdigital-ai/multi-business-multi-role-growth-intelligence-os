...
assert(schema.includes("brand_connections_included"), "tenant resolver must expose sanitized Brand connection inclusion evidence");
assert(mainSchema.includes("brand_connections_included"), "root resolver must expose sanitized Brand connection inclusion evidence");
assert(schema.includes("brand_authority_source"), "tenant resolver must expose the sanitized canonical Brand authority source");
assert(mainSchema.includes("brand_authority_source"), "root resolver must expose the sanitized canonical Brand authority source");
assert(schema.includes("tenant_owner_membership"), "tenant resolver must document tenant owner/admin Brand authority evidence");
assert(mainSchema.includes("tenant_owner_membership"), "root resolver must document tenant owner/admin Brand authority evidence");
assert(schema.includes("workspace_resource_grant"), "tenant resolver must document exact effective Brand grant authority evidence");
assert(mainSchema.includes("workspace_resource_grant"), "root resolver must document exact effective Brand grant authority evidence");
assert(schema.includes("BRAND_CONNECTION_AUTHORITY_REQUIRED"), "tenant resolver must document the stable Brand authority denial");
assert(mainSchema.includes("BRAND_CONNECTION_AUTHORITY_REQUIRED"), "root resolver must document the stable Brand authority denial");
assert(schema.includes("- brand"), "tenant resolver ownership scope must allow canonical Brand ownership");
assert(mainSchema.includes("- brand"), "root resolver ownership scope must allow canonical Brand ownership");
