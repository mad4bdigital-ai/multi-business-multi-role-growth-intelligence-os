...
export async function probeRemoteRuntimeTarget({
  ...
  try {
    const evidence = await writeExecutionEvidence({
      ...
      tenantId: target.tenant_id || normalizedTenantId || null,
      userId: target.user_id || normalizedUserId || null,
      actorId: target.user_id || normalizedUserId || null,
      actorType: target.user_id || normalizedUserId ? "user" : "system",
      ...
      executionContext: {
        source: "remote_runtime_probe",
        target_kind: target.target_kind,
        command_key: selectedCommand?.command_key || normalizedCommandKey,
        secrets_included: false,
      },
      outputSummary: {
        tenant_id: target.tenant_id || normalizedTenantId || null,
        user_id: target.user_id || normalizedUserId || null,
        plugin_key: "remote_ssh_runtime",
        app_key: "remote_ssh_runtime",
        action_key: "remote_runtime_probe",
...
export async function planRemoteRuntimeDispatchDryRun({
  ...
  try {
    const evidence = await writeExecutionEvidence({
      ...
      resourceType: "remote_runtime_target",
      resourceId: normalizedTargetId,
      targetType: target.target_kind,
      targetId: normalizedTargetId,
      correlationId: traceId,
      executionContext: {
        source: "remote_runtime_dispatch_dry_run",
        target_kind: target.target_kind,
        command_key: normalizedCommandKey,
        secrets_included: false,
      },
      outputSummary: {
        ...
        plugin_key: "remote_ssh_runtime",
        app_key: "remote_ssh_runtime",
        action_key: "remote_runtime_dispatch_dry_run",
        resource_type: "remote_runtime_target",
        resource_id: normalizedTargetId,
        target_type: target.target_kind,
        target_id: normalizedTargetId,
        target_kind: target.target_kind,
        command_key: normalizedCommandKey,
