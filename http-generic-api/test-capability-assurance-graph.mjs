...
const migration = readFileSync(new URL("./migrations/314_sprint69_capability_assurance_graph.sql", import.meta.url), "utf8");
...
assert.match(migration, /platform_capability_source_links/);
assert.match(migration, /source_kind[^\n]+mysql_registry|mysql_registry/);
assert.match(migration, /platform_capability_envelope_binding_links bl/);
assert.doesNotMatch(migration, /EXISTS \(SELECT 1 FROM v_effective_platform_resource_authority_bindings eb\)/);
assert.doesNotMatch(migration, /registry:\/\//);
...
assert.match(reconciler, /platform_capability_certifications/);
assert.match(reconciler, /platform_secret_movement_ledger/);
assert.match(reconciler, /status='resolved'/);
...
assert.doesNotMatch(reconciler, /fetch\(|axios|OPENAI_API_KEY|GITHUB_TOKEN/i);
