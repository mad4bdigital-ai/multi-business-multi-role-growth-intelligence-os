...
assert.match(windowsCloudflared, /\[wmiclass\]'Win32_Service'/);
assert.match(windowsCloudflared, /\$serviceClass\.Create\(/);
assert.doesNotMatch(windowsCloudflared, /sc\.exe\s+create\s+\$serviceName/);
...
assert.match(windowsCloudflared, /transport_protocol = 'http2'/);
...
assert.match(autoDeployInstaller, /\[switch\]\$EnableActivationGateway/);
assert.match(autoDeployInstaller, /if \(\$EnableActivationGateway\) \{ \$arguments \+= " -EnableActivationGateway" \}/);
...
assert.match(oneClick, /if \(\$EnableActivationGateway\) \{ \$args \+= "-EnableActivationGateway" \}/);
assert.match(launcher, /scripts\/staging-public-schema-readiness\.mjs/);
assert.match(semanticReadiness, /openapi\.tenant-gpt\.auth\.staging\.yaml/);
assert.match(semanticReadiness, /openapi\.custom-gpt\.auth-dispatcher\.staging\.yaml/);
assert.match(semanticReadiness, /openapi\.remote-mcp\.staging\.yaml/);
...
assert.match(launcher, /authenticated_remote_e2e_required = \$true/);
assert.match(launcher, /tenant_authenticated_action_state = if \(\$authenticated\.tenant_authenticated_action_ready\) \{ 'ready' \} else \{ 'blocked' \}/);
assert.doesNotMatch(launcher, /requires_end_user_oauth_runtime/);
assert.match(launcher, /mcp_access_tokens_persisted_to_env = \$false/);
assert.match(launcher, /staging_database_mutation = \[bool\]\(\$ProvisionMcpApp -or \$publicRequired\)/);
assert.match(launcher, /staging_database_transient_probe_mutation = \[bool\]\$publicRequired/);
