...
const portable = path.join(repoRoot, "autopilot-portable-staging");
...
assert.equal(policy.tunnel_modes.windows_service.origin, "http://127.0.0.1:8080");
...
assert.equal(policy.tunnel_modes.docker_sidecar.origin, "http://app:8080");
...
assert.match(launcher, /Enforce-TunnelRuntimeExclusion/);
...
assert.match(launcher, /http:\/\/app:8080/);
