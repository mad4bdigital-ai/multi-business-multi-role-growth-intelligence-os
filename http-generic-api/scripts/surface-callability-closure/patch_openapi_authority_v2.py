...
if "const OPENAPI_EVIDENCE_INDEX = buildOpenApiEvidenceIndex();" not in source:
    ...
    source = source[:start] + new_openapi + source[end:]
...
legacy_test_openapi = r'''const runtimeOpenApi = YAML.parse(fs.readFileSync(\"openapi/frontend-runtime-routes.generated.yaml\", \"utf8\"));\nconst canonicalOpenApi = YAML.parse(fs.readFileSync(\"openapi.yaml\", \"utf8\"));\nfor (const contract of manifest.contracts) {\n  const [method, route] = contract.route_signature.split(/\\s+/, 2);\n  const document = route === \"/system/tools/call\" ? canonicalOpenApi : runtimeOpenApi;\n  assert(document.paths?.[route]?.[method.toLowerCase()], `OpenAPI operation missing: ${contract.route_signature}`);\n}'''
...
elif legacy_test_openapi in source:
    source = source.replace(legacy_test_openapi, new_test_openapi, 1)
