...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (item === "--apply") out.apply = true;
      ...
      out.confirm = String(value || "");
      if (consume) i += 1;
    } else if (item.startsWith("--limit")) {
      out.limit = Math.max(1, Math.min(Number(value || 500), 5000));
      if (consume) i += 1;
    } else if (item.startsWith("--since-id")) {
      out.sinceId = Math.max(0, Number(value || 0));
      if (consume) i += 1;
    } else {
  ...
  const [rows] = await connection.query(
    `SELECT id,audit_id,tenant_id,actor_type,session_id,conversation_id,correlation_id,
      ...
      WHERE a.id > ?
        AND NOT EXISTS (
          SELECT 1
            FROM platform_audit_event_bus b
...
async function main() {
  const result = await runAuditLogEventBusBridge(parseArgs());
  ...
  main()
    ...
    .catch((error) => {
      process.stderr.write(`${JSON.stringify({
        ok: false,
        error: {
          code: error.code || "audit_log_event_bus_bridge_failed",
