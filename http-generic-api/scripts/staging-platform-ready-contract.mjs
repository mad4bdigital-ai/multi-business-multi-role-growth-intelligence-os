...
assert.match(windowsHelper, /\[wmiclass\]'Win32_Service'/);
assert.match(windowsHelper, /\$serviceClass\.Create\s*\(/);
...
assert.match(launcher, /https:\/\/activation-dev\.mad4b\.com\/health/);
assert.match(launcher, /https:\/\/activation-dev\.mad4b\.com\/tenant\/activation\/session-context/);
assert.doesNotMatch(launcher, /https:\/\/activation-dev\.mad4b\.com\/\.well-known\/oauth-authorization-server/);
