...
export function buildPersistedDiscoveryReport(report) {
  const compactMigration = (entry) => ({
    migration_file: entry.migration_file,
    legacy_backlog_closed: entry.legacy_backlog_closed === true,
    documentation_complete: entry.documentation_complete === true,
    gap_severity: entry.coverage?.gap_severity || "none",
    safety_attestation: entry.safety_attestation ? {
      attestation_status: entry.safety_attestation.attestation_status,
      evidence_mode: entry.safety_attestation.evidence_mode,
      migration_sha256: entry.safety_attestation.migration_sha256,
      checksum_canonicalization: entry.safety_attestation.checksum_canonicalization || CHECKSUM_CANONICALIZATION,
    } : null,
    surface_counts: Object.fromEntries(SURFACE_TYPES.map((type) => [type, entry.surfaces?.[type]?.length || 0])),
    safety_marker_count: entry.coverage?.safety_marker_count || 0,
    openapi_missing_route_count: entry.coverage?.route_coverage?.missing_count || 0,
  });
  return {
    ...report,
    serialization_profile: "bounded_evidence_v1",
    all_migrations_detail_level: "compact_index_v1",
    all_migrations_count: report.all_migrations.length,
    all_migrations: report.all_migrations.map(compactMigration),
  };
}
