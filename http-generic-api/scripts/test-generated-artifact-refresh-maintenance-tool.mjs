...
runCheck("tool-canonical-auth-repair", () => {
  ...
  const generateOpenApiMutationPolicyIndex = toolSource.indexOf("generate_openapi_mutation_policy");
  ...
  const stagingSmartConvergenceHashIndex = toolSource.indexOf("syncStagingSmartConvergencePolicyHash");
  ...
  assert.ok(generateOpenApiMutationPolicyIndex > customGptIndex, "mutation policy generation must follow shared Custom GPT schema generation");
  ...
  assert.ok(stagingSmartConvergenceHashIndex > convergenceHashIndex, "Staging smart convergence policy hash must follow convergence profile hash synchronization");
  ...
  assert.match(toolSource, /http-generic-api\/openapi\/openapi-mutation-policy\.generated\.json/u);
  assert.match(toolSource, /autopilot-portable-staging\/activation-gateway-smart-convergence-policy\.json/u);
  ...
  assert.match(toolSource, /verify_ci_path_guard/u);
  assert.match(toolSource, /verify_staging_smart_gateway_convergence_contract/u);
...
runCheck("repository-inventory-exact-head-verifier", () => {
  ...
  assert.match(inventoryWorkflowSource, /INVENTORY_AUTHORITY_MODE:\s*read_only_verifier/u);
  ...
  assert.match(inventoryWorkflowSource, /persist-credentials:\s*false/u);
  ...
  assert.match(inventoryGateSource, /mad4b\.repository-inventory-verification-gate\.v2/u);
  assert.match(inventoryGateSource, /git\(\["rev-parse", "HEAD"\]\)/u);
  assert.match(inventoryGateSource, /exact_head_mismatch/u);
  assert.doesNotMatch(inventoryGateSource, /ls-remote/u);
  assert.doesNotMatch(inventoryGateSource, /git\(\["push"/u);
  ...
  assert.match(inventoryGateSource, /first = hashOutputs\(\)/u);
  assert.match(inventoryGateSource, /second = hashOutputs\(\)/u);
  assert.match(inventoryGateSource, /sameHashes\(first, second\)/u);
  assert.match(inventoryGateSource, /restoreOutputs\(\)/u);
  assert.match(inventoryGateSource, /\.github\/derived-state-governance\.json/u);
  assert.match(inventoryGateSource, /artifact\.artifact_class === "observability"/u);
  assert.match(inventoryGateSource, /artifact\.merge_blocking === false/u);
  assert.match(inventoryGateSource, /observability_artifacts_publish_post_merge_only/u);
  assert.match(inventoryGateSource, /outcome: "advisory"/u);
  assert.match(inventoryGateSource, /inventory_state: "stale_observability"/u);
  assert.match(inventoryGateSource, /followup_mode: "post_merge_observability_publish"/u);
  assert.match(inventoryGateSource, /inventory_state: "stale_blocking"/u);
});
...
runCheck("repository-inventory-autofix-dispatcher", () => {
  ...
  assert.match(autofixWorkflowSource, /workflow_dispatch:/u);
  ...
  assert.match(autofixWorkflowSource, /SOURCE_HEAD_BRANCH/u);
  assert.match(autofixWorkflowSource, /main_convergence/u);
  assert.match(autofixWorkflowSource, /pull_request/u);
  assert.match(autofixWorkflowSource, /MANUAL_MODE/u);
  assert.match(autofixWorkflowSource, /MANUAL_SOURCE_MAIN_SHA/u);
  assert.match(autofixWorkflowSource, /MANUAL_CONFIRMATION/u);
  assert.match(autofixWorkflowSource, /observability_artifacts_publish_post_merge_only/u);
  assert.match(autofixWorkflowSource, /observability_premerge_mutation_forbidden/u);
  assert.match(autofixWorkflowSource, /feature-PR mutation intentionally suppressed/u);
  assert.match(autofixWorkflowSource, /Pre-merge Repository Inventory mutation is forbidden by policy/u);
  assert.match(autofixWorkflowSource, /SOURCE_EVENT[\s\S]*push[\s\S]*SOURCE_HEAD_BRANCH[\s\S]*main/u);
  assert.match(autofixWorkflowSource, /current_main_sha/u);
  assert.match(autofixWorkflowSource, /git\/ref\/heads\/main/u);
  assert.match(autofixWorkflowSource, /source_main_sha/u);
  assert.match(autofixWorkflowSource, /chore\/repository-inventory-main-sync-/u);
  assert.match(autofixWorkflowSource, /main\|Production/u);
  ...
  assert.match(autofixWorkflowSource, /create_from_main:"true"/u);
  assert.doesNotMatch(autofixWorkflowSource, /create_from_main:"false"/u);
});
