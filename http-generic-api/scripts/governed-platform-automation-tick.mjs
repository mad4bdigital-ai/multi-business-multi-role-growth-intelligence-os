...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (item === "--dry-run") args.apply = false;
      ...
      if (consume) i += 1;
    } else if (item.startsWith("--migration-limit")) {
      args.migrationLimit = Math.max(1, Math.min(Number(value || 100), 2000));
      if (consume) i += 1;
    } else if (item.startsWith("--audit-limit")) {
      args.auditLimit = Math.max(1, Math.min(Number(value || 1000), 5000));
      if (consume) i += 1;
    } else {
...
function runReconciliation(options) {
  ...
  const args = options.apply
    ? [
        "--apply",
        "--confirm=APPLY_GOVERNED_MIGRATION_RECONCILIATION",
        `--limit=${options.migrationLimit}`,
      ]
