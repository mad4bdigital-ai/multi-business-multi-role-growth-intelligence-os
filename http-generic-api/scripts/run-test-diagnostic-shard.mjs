...
function selectCommands(options) {
  const filteredCommands = options.grep
    ? testCommands.filter((command) => command.includes(options.grep))
    : testCommands;
  const indexedCommands = filteredCommands.map((command, commandIndex) => Object.freeze({ command, commandIndex }));
...
function main() {
  ...
  const report = {
    contract: "mad4b.test-diagnostic-shard-report.v1",
  ...
  if (options.list) {
    for (const item of report.commands) console.log(`[test:${item.commandIndex}] ${item.command}`);
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    console.log(`\n[${index + 1}/${selected.length}][test:${commandIndex}] ${command}`);
    let execution;
    try {
    ...
    if (execution.status !== 0) {
      ...
      console.error(`::error title=Diagnostic test command failed::${escapedAnnotation(command)} exited with status ${execution.status}`);
