...
assert.match(clone, /prepared\.canonical_semantic_snapshot\.output_file/);
assert.match(clone, /Prepared canonical semantic snapshot bundle is missing/);
assert.match(clone, /Prepared canonical semantic snapshot bundle hash mismatch/);
...
assert.match(legacyClone, /\$expectedSemanticSnapshotTableCount = \$expectedSemanticSnapshotTables\.Count/);
assert.match(legacyClone, /\$semanticSnapshotTables\.Count -eq \$expectedSemanticSnapshotTableCount/);
assert.match(legacyClone, /\[int\]\$semanticSnapshotManifest\.table_count -eq \$expectedSemanticSnapshotTableCount/);
assert.doesNotMatch(legacyClone, /\$semanticSnapshotTables\.Count -eq \d+/);
assert.equal(roleManifest.canonical_semantic_snapshot.tables.includes("execution_policies"), true);
...
assert.match(replayPlanner, /canonical semantic snapshot bundle checksum mismatch/);
assert.match(replayPlanner, /canonical semantic snapshot gzip decode failed/);
assert.match(replayPlanner, /fs\.copyFileSync\(semanticSource, semanticTarget\)/);
assert.match(replayPlanner, /publicPlan\.canonical_semantic_snapshot =/);
