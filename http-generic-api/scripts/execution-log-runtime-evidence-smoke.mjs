...
async function main() {
  ...
  const policyKeys = ["execution_log_runtime_evidence_policy_v1", "execution_log_full_context_evidence_policy_v1", "platform_resource_authority_binding_policy_v1"];
  ...
  await writeExecutionEvidence({
    ...
    outputSummary: { smoke: "execution_log_full_context_evidence", tenant_id: tenantId, workspace_id: workspaceId, user_id: userId, brand_key: "activation_smoke_brand", business_activity_type_key: "activation_smoke_activity", business_type_key: "activation_smoke_business_type", knowledge_profile_key: "activation_smoke_profile", role_keys: ["tenant_admin", "operator"], policy_keys: policyKeys, app_key: "activation_smoke_app", action_key: "activation_smoke_app.execute", secrets_included: false },
    tenantId, workspaceId, userId,
    actorId: userId, actorType: "user",
    brandId: "activation_smoke_brand_id", brandKey: "activation_smoke_brand", brandName: "Activation Smoke Brand", brandCoreStatus: "ready", brandCoreAssetKeys: ["activation_smoke_brand_core"],
    activityId: "activation_smoke_activity_id", activityType: "activation_smoke_activity", businessActivityTypeKey: "activation_smoke_activity", activityKey: "activation_smoke_activity_key", businessTypeKey: "activation_smoke_business_type", knowledgeProfileKey: "activation_smoke_profile",
    parentActionKey: "wordpress_api", endpointKey: "wordpress_publish", toolKey: "wordpress_publish_tool", appKey: "activation_smoke_app", actionKey: "activation_smoke_app.execute",
    appConnectionId: "00000000-0000-4000-a000-000000000091", pluginKey: "activation.smoke.plugin",
    agentId: "00000000-0000-4000-a000-000000000094", agentKey: "activation_smoke_agent", skillId: "00000000-0000-4000-a000-000000000093", skillKey: "activation_smoke_skill",
    workflowId: "00000000-0000-4000-a000-000000000089", workflowKey: "activation_smoke_workflow", workflowBindingKey: "activation_smoke_workflow_binding",
    connectedSystemId: "00000000-0000-4000-a000-000000000096", installationId: "00000000-0000-4000-a000-000000000097", permissionGrantId: "00000000-0000-4000-a000-000000000098", permissionKey: "wordpress_api", connectorFamily: "http_generic_api_connector", providerFamily: "wordpress",
    resourceAuthorityBindingId: "activation_smoke_resource_authority", budgetAuthorityId: "activation_smoke_budget_authority",
    engineKey: "activation_smoke_engine", enginePolicyKey: "activation_smoke_engine_policy", modelKey: "openai/gpt-4o-mini", modelProviderKey: "openrouter", modelRunId: "activation_smoke_model_run", logicKey: "activation_smoke_logic", logicPackKey: "activation_smoke_logic_pack",
    roleKeys: ["tenant_admin", "operator"], policyKeys,
    resourceType: "diagnostic_smoke", resourceId: "execution_log_full_context_evidence", targetType: "execution_log", targetId: "runtime_evidence_smoke", correlationId: traceId,
  ...
  const jsonValidityKeys = ["agent_evidence_json_valid", "skill_evidence_json_valid", "app_evidence_json_valid", "workflow_evidence_json_valid", "role_evidence_json_valid", "policy_evidence_json_valid", "authorization_evidence_json_valid", "runtime_evidence_json_valid", "brand_evidence_json_valid", "business_activity_evidence_json_valid", "business_type_evidence_json_valid", "connected_system_evidence_json_valid", "permission_evidence_json_valid", "resource_authority_evidence_json_valid", "budget_authority_evidence_json_valid", "engine_evidence_json_valid", "model_evidence_json_valid", "logic_evidence_json_valid", "knowledge_evidence_json_valid"];
  ...
  const ok = Boolean(row)
    ...
    && row.tenant_id === tenantId && row.workspace_id === workspaceId && row.user_id === userId
    && row.role_keys === "tenant_admin,operator"
    && String(row.policy_keys || "").includes("execution_log_full_context_evidence_policy_v1")
    && row.agent_key === "activation_smoke_agent" && row.skill_key === "activation_smoke_skill"
    && row.app_key === "activation_smoke_app" && row.app_connection_id === "00000000-0000-4000-a000-000000000091"
    && row.plugin_key === "activation.smoke.plugin" && row.workflow_key === "activation_smoke_workflow" && row.workflow_binding_key === "activation_smoke_workflow_binding"
    && row.brand_key === "activation_smoke_brand" && row.brand_core_status === "ready"
    && row.business_activity_type_key === "activation_smoke_activity" && row.business_type_key === "activation_smoke_business_type" && row.knowledge_profile_key === "activation_smoke_profile"
    && row.connected_system_id === "00000000-0000-4000-a000-000000000096" && row.permission_key === "wordpress_api" && row.connector_family === "http_generic_api_connector" && row.provider_family === "wordpress"
    && row.resource_authority_binding_id === "activation_smoke_resource_authority" && row.budget_authority_id === "activation_smoke_budget_authority"
    && row.engine_key === "activation_smoke_engine" && row.model_key === "openai/gpt-4o-mini" && row.logic_key === "activation_smoke_logic"
    && jsonValidityOk && blockedFieldLeakDetected === false && runtimeReadiness?.readiness_status === "pass" && fullReadiness?.readiness_status === "pass";

  console.log(JSON.stringify({ ok, smoke: "execution_log_full_context_evidence_smoke", trace_id: traceId, row_id: row?.id || null, execution_evidence_status: row?.execution_evidence_status || null, tenant_id: row?.tenant_id || null, workspace_id: row?.workspace_id || null, user_id: row?.user_id || null, role_keys: row?.role_keys || null, policy_keys: row?.policy_keys || null, brand_key: row?.brand_key || null, brand_core_status: row?.brand_core_status || null, business_activity_type_key: row?.business_activity_type_key || null, business_type_key: row?.business_type_key || null, knowledge_profile_key: row?.knowledge_profile_key || null, connected_system_id: row?.connected_system_id || null, permission_key: row?.permission_key || null, connector_family: row?.connector_family || null, provider_family: row?.provider_family || null, resource_authority_binding_id: row?.resource_authority_binding_id || null, budget_authority_id: row?.budget_authority_id || null, engine_key: row?.engine_key || null, model_key: row?.model_key || null, logic_key: row?.logic_key || null, agent_key: row?.agent_key || null, skill_key: row?.skill_key || null, app_key: row?.app_key || null, app_connection_id: row?.app_connection_id || null, plugin_key: row?.plugin_key || null, workflow_key: row?.workflow_key || null, workflow_binding_key: row?.workflow_binding_key || null, json_validity_ok: jsonValidityOk, blocked_field_leak_detected: blockedFieldLeakDetected, runtime_readiness_status: runtimeReadiness?.readiness_status || null, full_context_readiness_status: fullReadiness?.readiness_status || null, external_provider_called: false, secrets_included: false }, null, 2));
  process.exit(ok ? 0 : 2);
}
