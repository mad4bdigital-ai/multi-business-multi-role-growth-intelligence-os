...
async function main() {
  ...
  const expectedBranch = String(args.expected_branch || process.env.DEV_DEPLOYMENT_BRANCH || process.env.GOVERNED_DEV_BRANCH || "main").trim();
  const expectedCommit = String(args.expected_commit || process.env.EXPECTED_DEPLOYMENT_COMMIT || "").trim();
  const checks = [];
  ...
  if (expectedCommit) {
    assertCheck(
      checks,
      `deployment commit is ${expectedCommit}`,
      status.summary.deployment.commit_sha === expectedCommit,
