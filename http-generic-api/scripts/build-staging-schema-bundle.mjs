...
const migrationContractPolicyPath = path.join(apiRoot, "config", "staging-migration-contract-policy.json");
...
function orderedEnumSeedAudit(files) {
  ...
  const orderedFiles = files.map((file) => `http-generic-api/migrations/${file}`);
  const audit = inspectOrderedMigrationChainEnumSeeds({
    files: orderedFiles,
    baselineFile: "http-generic-api/schema.sql",
    engine: "mariadb",
    policy,
  ...
  if (audit.findings.length > 0) {
    const sample = audit.findings.slice(0, 8).map((finding) => `${path.basename(finding.file)}#${finding.statement_index ?? "?"}:${finding.table}.${finding.column}=${finding.value}`).join(", ");
    fail(`ordered MariaDB enum-seed audit found ${audit.findings.length} unsupported literal writes (${sample})`);
  }
  if (audit.database_connection_performed !== false || audit.sql_mutation_performed !== false || audit.provider_mutation_performed !== false || audit.credential_access_performed !== false || audit.data_export_performed !== false || audit.runtime_mutation_performed !== false || audit.secrets_included !== false) {
    fail("ordered MariaDB enum-seed audit violated static-only safety boundary");
  }
  return audit;
}
...
function orderedIndexKeyWidthAudit(files) {
  if (!fs.existsSync(indexKeyWidthPolicyPath)) fail(`database index-key-width policy is missing: ${indexKeyWidthPolicyPath}`);
  let policy;
  try { policy = JSON.parse(fs.readFileSync(indexKeyWidthPolicyPath, "utf8")); }
  catch (error) { fail(`database index-key-width policy is invalid: ${error.message}`); }
  const orderedFiles = files.map((file) => `http-generic-api/migrations/${file}`);
  const audit = inspectOrderedMigrationChainIndexKeyWidths({
    files: orderedFiles,
    baselineFile: "http-generic-api/schema.sql",
    engine: "mariadb",
    policy,
  ...
  if (audit.findings.length > 0) {
    const sample = audit.findings.slice(0, 8).map((finding) => `${path.basename(finding.file)}#${finding.statement_index ?? "?"}:${finding.table}.${finding.index}=${finding.estimated_key_bytes ?? finding.detail?.reason ?? "unresolved"}`).join(", ");
    fail(`ordered MariaDB index-key-width audit found ${audit.findings.length} key definitions over the configured limit (${sample})`);
  }
  if (audit.database_connection_performed !== false || audit.sql_mutation_performed !== false || audit.provider_mutation_performed !== false || audit.credential_access_performed !== false || audit.data_export_performed !== false || audit.runtime_mutation_performed !== false || audit.secrets_included !== false) {
    fail("ordered MariaDB index-key-width audit violated static-only safety boundary");
  }
  return audit;
}

function orderedForeignKeyAudit(files) {
  if (!fs.existsSync(migrationContractPolicyPath)) fail(`staging migration contract policy is missing: ${migrationContractPolicyPath}`);
  let policy;
  try { policy = JSON.parse(fs.readFileSync(migrationContractPolicyPath, "utf8")); }
  catch (error) { fail(`staging migration contract policy is invalid: ${error.message}`); }
  const orderedFiles = files.map((file) => `http-generic-api/migrations/${file}`);
  const audit = inspectOrderedMigrationChainForeignKeys({
    files: orderedFiles,
    baselineFile: "http-generic-api/schema.sql",
    engine: "mariadb",
    policy,
  ...
  if (audit.findings.length > 0) {
    const sample = audit.findings.slice(0, 8).map((finding) => `${path.basename(finding.file)}#${finding.statement_index ?? "?"}:${finding.table || "?"}.${finding.column || "?"}:${finding.code}`).join(", ");
    fail(`ordered MariaDB foreign-key compatibility audit found ${audit.findings.length} findings (${sample})`);
  }
  if (audit.database_connection_performed !== false || audit.sql_mutation_performed !== false || audit.provider_mutation_performed !== false || audit.credential_access_performed !== false || audit.data_export_performed !== false || audit.runtime_mutation_performed !== false || audit.secrets_included !== false) {
    fail("ordered MariaDB foreign-key compatibility audit violated static-only safety boundary");
  }
  return audit;
}

function orderedForeignKeyMetadata(audit) {
  return {
    ...
    foreign_keys_checked: audit.foreign_keys_checked,
    type_comparisons: audit.type_comparisons,
    type_mismatches: audit.type_mismatches,
    unresolved_type_mismatches: audit.unresolved_type_mismatches,
    missing_parent_tables: audit.missing_parent_tables,
    missing_parent_columns: audit.missing_parent_columns,
    missing_parent_indexes: audit.missing_parent_indexes,
...
function indexKeyWidthMetadata(audit) {
  return {
    contract: audit.contract,
    engine: audit.engine,
    policy_key: audit.policy_key,
    ...
    statements_checked: audit.statements_checked,
    tables_projected: audit.tables_projected,
    indexes_checked: audit.indexes_checked,
    index_columns_checked: audit.index_columns_checked,
    max_key_bytes: audit.max_key_bytes,
    ok: audit.ok,
    ready: audit.ready,
    finding_count: audit.findings.length,
    warning_count: audit.warnings.length,
...
function generatedColumnMetadata(audit) {
  return {
    ...
    migration_files_checked: audit.migration_files_checked,
    statements_checked: audit.statements_checked,
    generated_columns: audit.generated_columns,
    definitions_applied: audit.definitions_applied,
    writer_checks: audit.writer_checks,
    generated_expression_checks: audit.generated_expression_checks,
    compatibility_bridge_candidates: audit.compatibility_bridge_candidates,
...
function writeOutput(manifest, expected, baseline, migrationPlanRows, canonicalSeeds, orderedAudit, collationAudit, enumSeedAudit, textWidthAudit, indexKeyWidthAudit, requiredInsertColumnAudit, generatedColumnAudit, foreignKeyAudit, bootstrap, tableSets, bundles) {
  const output = {
    ...
    validation: {
      ...
      ordered_enum_seed_chain_checked: true,
      ordered_enum_seed_chain_findings: enumSeedAudit.findings.length,
      ...
      ordered_index_key_width_chain_checked: true,
      ordered_index_key_width_chain_findings: indexKeyWidthAudit.findings.length,
      ordered_index_key_width_chain_files_checked: indexKeyWidthAudit.files_checked,
      ...
      ordered_generated_column_chain_statements_checked: generatedColumnAudit.statements_checked,
      ordered_foreign_key_compatibility_chain_checked: true,
      ordered_foreign_key_compatibility_chain_findings: foreignKeyAudit.findings.length,
      ordered_foreign_key_compatibility_chain_files_checked: foreignKeyAudit.files_checked,
      ordered_foreign_key_compatibility_chain_statements_checked: foreignKeyAudit.statements_checked,
      ordered_foreign_key_compatibility_chain_type_mismatches: foreignKeyAudit.type_mismatches,
      ordered_foreign_key_compatibility_chain_allowed_bridges: foreignKeyAudit.allowed_compatibility_bridges,
...
function printPlan(manifest, baseline, files, rows, canonicalSeeds, orderedAudit, collationAudit, enumSeedAudit, textWidthAudit, indexKeyWidthAudit, requiredInsertColumnAudit, generatedColumnAudit, foreignKeyAudit, bootstrap) {
...
const foreignKeyAudit = orderedForeignKeyAudit(files);
const generatedColumnAudit = inspectOrderedMigrationChainGeneratedColumns({
  files: files.map((file) => `http-generic-api/migrations/${file}`),
  baselineFile: "http-generic-api/schema.sql",
  engine: "mariadb",
  policy: JSON.parse(fs.readFileSync(migrationContractPolicyPath, "utf8")),
  bootstrapEntries: tableBootstrap.entries,
  readFile: (file) => fs.readFileSync(path.join(repoRoot, file), "utf8"),
});
if (generatedColumnAudit.ok !== true || generatedColumnAudit.ready !== true || generatedColumnAudit.findings.length > 0) {
  const sample = generatedColumnAudit.findings.slice(0, 8).map((finding) => `${path.basename(finding.file)}#${finding.statement_index ?? "?"}:${finding.table}.${finding.column ?? "?"}`).join(", ");
  fail(`ordered MariaDB generated-column audit is not clean${sample ? ` (${sample})` : ""}`);
}
if (generatedColumnAudit.database_connection_performed !== false || generatedColumnAudit.sql_mutation_performed !== false || generatedColumnAudit.provider_mutation_performed !== false || generatedColumnAudit.credential_access_performed !== false || generatedColumnAudit.data_export_performed !== false || generatedColumnAudit.runtime_mutation_performed !== false || generatedColumnAudit.secrets_included !== false) {
  fail("ordered MariaDB generated-column audit violated static-only safety boundary");
}
...
if (planOnly) {
  printPlan(manifest, baseline, files, rows, canonicalSeeds, orderedAudit, collationAudit, enumSeedAudit, textWidthAudit, indexKeyWidthAudit, requiredInsertColumnAudit, generatedColumnAudit, foreignKeyAudit, tableBootstrap);
  process.exit(0);
}
