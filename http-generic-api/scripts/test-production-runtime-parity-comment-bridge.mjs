...
assert.match(workflow, /\.side_effects\.repository_mutation_performed == false/u);
assert.match(workflow, /\.side_effects\.provider_dispatch_performed == false/u);
assert.match(workflow, /\.side_effects\.credential_access_performed == false/u);
assert.match(workflow, /\.side_effects\.sql_execution_performed == false/u);
assert.match(workflow, /\.side_effects\.migration_apply_performed == false/u);
assert.match(workflow, /\.side_effects\.external_send_performed == false/u);
...
assert.deepEqual(
  `RUN_PRODUCTION_RUNTIME_PARITY expected_sha=${expectedSha} tooling_sha=${toolingSha}`.match(triggerPattern)?.slice(1),
  [expectedSha, toolingSha]
);
...
console.log(JSON.stringify({
  ok: true,
  tests: 2,
  gate: "production_runtime_parity_comment_bridge",
  provider_method: "public_https_get",
