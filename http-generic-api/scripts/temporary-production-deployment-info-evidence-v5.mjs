...
if (!/^[a-f0-9]{40}$/u.test(expectedSha || "")) {
  throw new Error("EXPECTED_PRODUCTION_SHA must be a full lowercase SHA.");
}
...
const endpointEntries = await Promise.all(
  Object.entries(endpointDefinitions).map(async ([name, url]) => [name, await fetchEndpoint(name, url)])
);
...
const observedTimestamp = deployedAt.value ? Date.parse(deployedAt.value) : Number.NaN;
const expectedTimestamp = Date.parse(expectedDeployedAfter);
const timestampCurrent = Number.isFinite(observedTimestamp) && observedTimestamp >= expectedTimestamp;
const checks = {
  ...
  deployment_timestamp_current: timestampCurrent,
...
const reportPath = path.join(reportDir, "production-deployment-info-evidence.json");
