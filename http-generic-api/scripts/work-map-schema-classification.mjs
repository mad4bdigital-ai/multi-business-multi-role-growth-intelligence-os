...
function parseObjectSection(coverageText, heading) {
  return [...section(coverageText, heading).matchAll(/^- `([^`]+)`(?: \(([^)]+)\))?/gm)]
...
export function loadClassificationInputs(options = {}) {
  ...
  const legacyUncategorized = parseObjectSection(coverageText, "Uncategorized schema objects");
  const unresolved = parseObjectSection(coverageText, "Unresolved schema objects");
  const intentional = parseObjectSection(coverageText, "Intentionally unclassified schema objects");
  return {
    root,
    policy,
    ...
    unresolved_objects: unresolved.length ? unresolved : legacyUncategorized,
    intentional_objects: intentional,
  };
}
...
export function validateSchemaClassification(options = {}) {
  ...
  for (const [index, row] of exceptions.entries()) {
    ...
    if (!Array.isArray(row.nearest_existing_map_refs) || row.nearest_existing_map_refs.length < 2) {
  ...
  for (const object of intentionalObjects) {
    ...
    if (!exception) push(findings, "intentional_object_missing_registry_exception", { object_name: object.name });
    else if (exception.object_type !== object.type) push(findings, "intentional_exception_type_mismatch", { object_name: object.name, expected: object.type, actual: exception.object_type });
  }
  for (const exception of exceptions) {
    if (exception?.object_name && !renderedIntentionalNames.has(exception.object_name)) {
      push(findings, "stale_intentional_exception", { object_name: exception.object_name });
    }
  }
...
export function applySchemaClassificationRegistry(workMapRegistry, options = {}) {
  ...
  return {
    ...workMapRegistry,
    uncategorized_objects: result.unresolved,
...
function main() {
  ...
  const payload = {
    ok: result.ok,
    gate: "fail_closed",
    source_unresolved_or_intentional_count: result.source_uncategorized_count,
    classified_from_legacy_matrix: result.classified.length,
