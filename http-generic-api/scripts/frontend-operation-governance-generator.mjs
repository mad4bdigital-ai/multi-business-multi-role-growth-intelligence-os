...
function evaluateBrandCreateRecipe(apiRoot) {
  ...
  const gates = [
    ...
    evidenceGate("container_authority_transaction_scope", routeDeclaration.includes("withContainerAuthorityMutation") && routeDeclaration.includes('mutationType: "workspace_brand_create"') && routeDeclaration.includes("rebuildClosure: false"), "route-scoped canonical withContainerAuthorityMutation transaction"),
    evidenceGate("transactional_topology_readback", routeDeclaration.includes("rebuildContainerClosure") && routeDeclaration.includes("verifyWorkspaceBrandRootTopology"), "route-scoped closure rebuild and exact Root-to-Brand readback before commit"),
...
function withSourceAuthority(plan, apiRoot) {
  ...
  return {
    ...
    generator: {
      ...plan.generator,
      id: "frontend-operation-governance-generator-v8-route-scoped-root-workspace-brand-create-topology",
