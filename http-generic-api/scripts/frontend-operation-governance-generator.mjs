...
const RESOURCE_RECIPES = [
  {
    ...
    route_guard_markers: ["requireUser"],
    membership_required: true,
    capability_markers: ["requireAssetOperation", "requireAssetInput"],
  ...
  },
  {
    ...
    route_guard_markers: ["requireUser"],
    membership_required: true,
    capability_markers: ["requireAssetOperation", "resourceCapabilities"],
    preflight_mode: "inline_capability_check",
    approval_mode: "runtime_authorization",
  ...
  {
    recipe_id: "admin-resource-create-transaction-v1",
    rule_id: "generated-admin-resource-create-governance",
    operation: "POST /admin/resources/{resourceKey}",
    route_handler: "controller.adminResourceCreate",
    service_function: "adminCreateResource",
    mutation_call: "transactionRepository.insertAsset",
    route_guard_markers: ["requireBackend", "requireAdmin"],
    membership_required: false,
    capability_markers: ["requireAssetOperation", "requireAssetInput"],
    preflight_mode: "inline_admin_capability_check",
    approval_mode: "runtime_admin_authorization",
    rationale: "Requires the backend and platform-admin guards, validates the admin asset capability, creates the asset, and verifies its exact readback before committing one SQL transaction.",
    parameter_bindings: {
  ...
  {
    recipe_id: "admin-resource-update-transaction-v1",
    rule_id: "generated-admin-resource-update-governance",
    operation: "PATCH /admin/resources/{resourceKey}/{resourceId}",
    route_handler: "controller.adminResourceUpdate",
    service_function: "adminUpdateResource",
    mutation_call: "transactionRepository.updateAssetFields",
    route_guard_markers: ["requireBackend", "requireAdmin"],
    membership_required: false,
    capability_markers: ["requireAssetOperation", "resourceCapabilities"],
    preflight_mode: "inline_admin_capability_check",
    approval_mode: "runtime_admin_authorization",
    rationale: "Requires the backend and platform-admin guards, verifies the asset update capability, mutates supported fields, and verifies the exact readback before committing one SQL transaction.",
    parameter_bindings: {
      resource_key: "path.resourceKey",
      resource_id: "path.resourceId",
    },
  ...
  {
    recipe_id: "admin-resource-archive-transaction-v1",
    rule_id: "generated-admin-resource-archive-governance",
    operation: "DELETE /admin/resources/{resourceKey}/{resourceId}",
    route_handler: "controller.adminResourceArchive",
    service_function: "adminSetResourceLifecycle",
    mutation_call: "transactionRepository.setAssetLifecycle",
    route_guard_markers: ["requireBackend", "requireAdmin"],
    membership_required: false,
    capability_markers: ["requireAssetOperation", "resourceCapabilities"],
    preflight_mode: "inline_admin_capability_check",
    approval_mode: "runtime_admin_authorization",
    rationale: "Requires the backend and platform-admin guards, verifies archive capability, changes lifecycle state, and verifies the exact archived readback before committing one SQL transaction.",
    parameter_bindings: {
      resource_key: "path.resourceKey",
      resource_id: "path.resourceId",
    },
  ...
  {
    recipe_id: "admin-resource-restore-transaction-v1",
    rule_id: "generated-admin-resource-restore-governance",
    operation: "POST /admin/resources/{resourceKey}/{resourceId}/restore",
    route_handler: "controller.adminResourceRestore",
    service_function: "adminSetResourceLifecycle",
    mutation_call: "transactionRepository.setAssetLifecycle",
    route_guard_markers: ["requireBackend", "requireAdmin"],
    membership_required: false,
    ...
    preflight_mode: "inline_admin_capability_check",
    approval_mode: "runtime_admin_authorization",
    rationale: "Requires the backend and platform-admin guards, verifies restore capability, changes lifecycle state, and verifies the exact active readback before committing one SQL transaction.",
    parameter_bindings: {
      resource_key: "path.resourceKey",
      resource_id: "path.resourceId",
    },
...
function evaluateResourceRecipe(recipe, context) {
  ...
  const claimedTests = context.testEvidence.byOperation.get(recipe.operation) || [];
  const routeGuardMarkers = recipe.route_guard_markers || ["requireUser"];
  const capabilityMarkers = recipe.capability_markers || ["requireAssetOperation", "resourceCapabilities"];
  const gates = [
    ...
    evidenceGate("route_authorization_guards", route && routeGuardMarkers.every((marker) => route.declaration.includes(marker)), routeGuardMarkers.join(", ")),
    ...
    evidenceGate("membership_preflight_in_transaction", recipe.membership_required === false || serviceBlock.includes("requireMembership(auth, tenantId, transactionRepository)"), recipe.membership_required === false ? "platform-admin authorization replaces tenant membership" : "requireMembership"),
    evidenceGate("capability_preflight_present", capabilityMarkers.every((marker) => serviceBlock.includes(marker)), capabilityMarkers.join(", ")),
  ...
  return {
    recipe: {
      ...
      owner: recipe.owner || "resource-platform",
      preflight_mode: recipe.preflight_mode || "inline_capability_check",
      approval_mode: recipe.approval_mode || "runtime_authorization",
    },
    gates,
