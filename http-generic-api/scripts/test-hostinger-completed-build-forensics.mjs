...
for (const required of [
  ...
  /Checkout immutable execution revision/,
  /ref: \$\{\{ github\.sha \}\}/,
  /fetch-depth: 0/,
  /CHECKED_OUT_SHA="\$\(git rev-parse HEAD\)"/,
  /test "\$\{CHECKED_OUT_SHA\}" = "\$\{EVENT_SHA\}"/,
  /test "\$\{INPUT_EXPECTED_HEAD_SHA\}" = "\$\{EVENT_SHA\}"/,
  /test "\$\{REMOTE_MAIN_SHA\}" = "\$\{EVENT_SHA\}"/,
