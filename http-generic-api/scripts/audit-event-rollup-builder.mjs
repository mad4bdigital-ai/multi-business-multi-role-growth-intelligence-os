...
function resolveDbSemantics(row) {
  ...
  if (row.source_family === "governed_migration_reconciliation" || row.resource_kind === "database_migration") {
    return {
...
function providerKey(row) {
  ...
  if (row.event_type === "repo_patch_apply" || resourceKind === "repo" || sourceFamily === "repo_file_audit") return "github";
  return "platform";
}
  ...
  for (const row of rows) {
    ...
    if (classification === "db_change") {
      ...
      const [result] = await connection.query(
        ...
        [row.source_family, semantics.tableName, semantics.mutationClass, row.event_key,
          safeJson({ ...evidence, semantic_resolution: semantics }), row.event_key]
      );
      ...
      const [result] = await connection.query(
        ...
        [providerKey(row), row.resource_key || null, row.resource_key || null, row.event_key,
          String(row.event_type || "audit_event").slice(0, 80),
          evidence.readback_verified ? "readback_verified" : "observed", safeJson(evidence), row.event_key]
      );
...
export async function runAuditEventRollupBuilder(options = {}, dependencies = {}) {
  ...
  try {
    ...
    if (!lockAcquired) return { ok: true, mode: normalized.apply ? "apply" : "dry_run", skipped: true, reason: "rollup_lock_busy", inserted: { db: 0, asset: 0, checkpoint: 0 }, remaining_count: null, secrets_included: false };
    ...
    if (!normalized.apply) return { ...base, inserted: { db: 0, asset: 0, checkpoint: 0 }, remaining_count: await countRemaining(connection), reason: "dry_run_only" };
    ...
    return {
      ...base,
      inserted: { db: inserted.dbInserted, asset: inserted.assetInserted, checkpoint: inserted.checkpointInserted },
...
async function main() {
  process.stdout.write(`${JSON.stringify(await runAuditEventRollupBuilder(parseArgs()), null, 2)}\n`);
}
...
if (isCli) {
  main().then(() => process.exit(0)).catch((error) => {
    process.stderr.write(`${JSON.stringify({
      ok: false,
      error: { code: error.code || "audit_event_rollup_builder_failed", message: error.message },
      raw_payload_stored: false,
      raw_before_after_stored: false,
      secrets_included: false,
    }, null, 2)}\n`);
