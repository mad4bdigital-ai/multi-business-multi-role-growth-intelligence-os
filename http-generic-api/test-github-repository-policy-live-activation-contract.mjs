...
assert.match(metadataState, /github_repository_policy_1051_metadata_diagnostic\.v5/);
assert.match(metadataState, /github_repository_policy_1051_envelope_dependency_225\.v3/);
assert.match(metadataState, /github_repository_policy_1051_governance_writer_readiness\.v2/);
...
assert.match(metadataState, /schema_objects_ready === true/);
assert.match(metadataState, /missing_required_schema_table_count\) === 0/);
assert.match(metadataState, /table_names_exposed === false/);
...
assert.match(metadataState, /governance_writer_schema_ready/);
...
assert.match(liveRunner, /function requireExecutablePolicyPlan\(planned\)/);
assert.match(liveRunner, /github_review_policy_readiness_blocked/);
assert.match(liveRunner, /activation_blockers_unreadable/);
assert.match(liveRunner, /preconditions_unreadable/);
assert.ok(liveRunner.includes('Object.entries(preconditions).filter(([, value]) => value !== true)'));
assert.ok(liveRunner.includes('["create_ruleset", "update_ruleset"].includes(operation)'));
assert.ok(liveRunner.includes('plan?.mutation_executed !== false'));
assert.ok(liveRunner.includes('expected_commit_sha_matches_target: targetBound'));
assert.match(liveRunner, /stage = "policy_plan_preconditions"/);
assert.match(liveRunner, /activation_blocker_count: 0/);
assert.match(liveRunner, /failed_precondition_count: 0/);
const readinessRunner = liveRunner.slice(liveRunner.indexOf("async function readiness()"), liveRunner.indexOf("async function apply()"));
const readinessPlanGuard = readinessRunner.indexOf("requireExecutablePolicyPlan(planned)");
const readinessReadySummary = readinessRunner.indexOf('result: "ready_for_apply"');
assert.ok(readinessPlanGuard >= 0, "Readiness must enforce executable policy-plan preconditions");
assert.ok(readinessReadySummary > readinessPlanGuard, "Readiness must validate the exact policy plan before declaring ready_for_apply");
const applyRunner = liveRunner.slice(liveRunner.indexOf("async function apply()"), liveRunner.indexOf("async function verify()"));
const applyPlanGuard = applyRunner.indexOf("requireExecutablePolicyPlan(planned)");
const applyReadinessMarker = applyRunner.indexOf('stage = "readiness_marker"');
const applyEnvelope = applyRunner.indexOf('stage = "envelope_create_approve_authorize"');
assert.ok(applyPlanGuard >= 0, "Apply must revalidate executable policy-plan preconditions");
assert.ok(applyReadinessMarker > applyPlanGuard, "Apply plan validation must happen before consuming a readiness marker");
assert.ok(applyEnvelope > applyPlanGuard, "Apply plan validation must happen before creating or authorizing an envelope");
