...
const migrationWorkflow = read('../.github/workflows/github-repository-policy-1050-governed-rollout.yml');
...
const migrationRunner = read('../.github/ops/github-repository-policy-1050-governed-rollout.mjs');
...
assert.match(migrationWorkflow, /^name: Governed Migration 1050 GitHub Repository Policy Authority Rollout/m);
assert.match(migrationWorkflow, /permissions:\n  contents: read/);
assert.doesNotMatch(migrationWorkflow, /issues:\s*write/);
assert.match(migrationWorkflow, /AUTHORIZE_GOVERNED_MIGRATION_1050_GITHUB_REPOSITORY_POLICY_LIVE_APPLY_AUTHORITY/);
assert.match(migrationWorkflow, /APPLY_1050_GITHUB_REPOSITORY_POLICY_LIVE_APPLY_AUTHORITY/);
assert.match(migrationWorkflow, /VERIFY_GOVERNED_MIGRATION_1050_GITHUB_REPOSITORY_POLICY_LIVE_APPLY_AUTHORITY/);
...
assert.match(liveRunner, /verifyMigration1050Applied/);
...
assert.match(publisher, /assert\.equal\(summary\?\.migration_1050_verified, true\)/);
