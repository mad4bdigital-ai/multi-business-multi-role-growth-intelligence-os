...
function classifyTicket({ ticket, events, workflowLinks, workflowRuns, stepRuns, approvalHolds, pendingTasks }) {
  ...
  if (failedRuns.length || failedSteps.length) {
    blockers.push({ blocker_class: "workflow_runtime", code: "workflow_or_step_failed", severity: "blocker", failed_run_count: failedRuns.length, failed_step_count: failedSteps.length });
  }
  if (status === "awaiting_approval" || lifecycleState.includes("approval") || openApprovals.length) {
    blockers.push({ blocker_class: "approval_required", code: "approval_required_or_pending", severity: "blocker", open_approval_count: openApprovals.length });
  }
  if (lifecycleState.includes("validation_pending") || lifecycleState.includes("schema") || customerStatus.includes("schema")) {
    blockers.push({ blocker_class: "runtime_validation", code: "runtime_or_schema_validation_pending", severity: "blocker", lifecycle_state: ticket.lifecycle_state, customer_status: ticket.customer_status });
  }
...
export async function proposeSupportTicketLifecycleSnapshot(input = {}) {
  ...
  const events = await allRows(
    `SELECT event_id, ticket_id, tenant_id, event_type, from_state, to_state, actor_type, visibility, summary, payload_json, created_at
       FROM ticket_lifecycle_events WHERE ticket_id = ? ORDER BY created_at DESC LIMIT 25`, [ticketId]
  );
  const workflowLinks = await allRows(
    `SELECT link_id, ticket_id, tenant_id, plan_id, run_id, approval_hold_id, relationship, status, evidence_json, created_at, updated_at
       FROM ticket_workflow_links WHERE ticket_id = ? ORDER BY updated_at DESC LIMIT 25`, [ticketId]
  );
  const executionPlans = await allRows(
    `SELECT plan_id, tenant_id, workspace_id, workspace_key, user_id, actor_type, brand_key, intent_key, target_key, workflow_key, route_key,
            service_mode, access_decision, plan_status, preview_json, validation_errors, created_at, updated_at
       FROM execution_plans
      WHERE plan_id IN (SELECT plan_id FROM ticket_workflow_links WHERE ticket_id = ? AND plan_id IS NOT NULL) OR request_id = ?
      ORDER BY updated_at DESC LIMIT 25`, [ticketId, ticketId]
  );
  const workflowRuns = await allRows(
    `SELECT run_id, tenant_id, workspace_id, workspace_key, user_id, actor_type, brand_key, workflow_key, agent_id, plan_id, service_mode,
            status, current_step, error_json, started_at, completed_at, created_at, updated_at
       FROM workflow_runs
      WHERE run_id IN (SELECT run_id FROM ticket_workflow_links WHERE ticket_id = ? AND run_id IS NOT NULL)
         ...
         OR request_id = ?
      ORDER BY updated_at DESC LIMIT 25`, [ticketId, ticketId, ticketId]
  );
  const stepRuns = await allRows(
    `SELECT step_run_id, run_id, tenant_id, workspace_id, workspace_key, user_id, actor_type, brand_key, step_key, step_type,
            assigned_to, status, attempt, error_message, started_at, completed_at, created_at
       FROM step_runs WHERE run_id IN (SELECT run_id FROM ticket_workflow_links WHERE ticket_id = ? AND run_id IS NOT NULL)
      ORDER BY created_at DESC LIMIT 25`, [ticketId]
  );
  const approvalHolds = await allRows(
    `SELECT hold_id, run_id, step_run_id, tenant_id, workspace_id, workspace_key, hold_type, requested_by, user_id, actor_type,
            brand_key, request_id, status, decision_by, expires_at, decided_at, created_at
       FROM approval_holds
      WHERE hold_id IN (SELECT approval_hold_id FROM ticket_workflow_links WHERE ticket_id = ? AND approval_hold_id IS NOT NULL) OR request_id = ?
      ORDER BY created_at DESC LIMIT 25`, [ticketId, ticketId]
  );
  const pendingTasks = await allRows(
    `SELECT task_id, task_key, title, task_type, priority, status, blocker_level, owner_scope, tenant_id, user_id, source_surface,
            source_ref, activation_visibility, due_at, completed_at, created_at, updated_at
       FROM platform_pending_tasks WHERE source_ref = ? OR task_key LIKE ? ORDER BY updated_at DESC LIMIT 25`, [ticketId, `%${ticketId}%`]
  );
  ...
  const snapshotCandidate = {
    ...
    state: {
      ...
      orchestration_readback: { readiness_status: orchestrationReadback.readiness_status, stage_count: orchestrationReadback.graph?.stage_count, edge_count: orchestrationReadback.graph?.edge_count },
    },
    maturity: { score: maturityScore, max_score: 100, parts: maturityParts },
    blockers,
    safety,
    secrets_included: false,
  };
  ...
  const recommendationCandidate = {
    ...
    decision: { state_classification: stateClassification, maturity_score: maturityScore, recommended_next_action: recommendedNextAction, execution_allowed_by_this_route: false },
    blockers,
    next_actions: blockers.length ? blockers.map((blocker) => ({ blocker_code: blocker.code, action: blocker.code })) : [{ action: recommendedNextAction }],
