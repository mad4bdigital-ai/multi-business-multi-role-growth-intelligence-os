...
function normalizeMigration(value = "") {
  ...
  if (!MIGRATION_PATTERN.test(migration) || path.basename(migration) !== migration) throw readbackError("invalid_migration_filename", "migration must be one repository migration filename ending in .sql.");
  return migration;
}
function normalizeIdentifier(value = "", field = "identifier") {
  const identifier = compact(value, 191);
  if (!IDENTIFIER_PATTERN.test(identifier)) throw readbackError("invalid_schema_readback_identifier", `${field} must contain only letters, numbers, and underscores.`, 400, { field });
  return identifier;
}
function normalizeChecksum(value = "") {
  const checksum = compact(value, 64).toLowerCase();
  if (!SHA256_PATTERN.test(checksum)) throw readbackError("invalid_expected_migration_checksum", "expected_checksum_sha256 must be a lowercase SHA-256 value.");
  return checksum;
}
function normalizeStatementCount(value) {
  const count = Number(value);
  if (!Number.isInteger(count) || count < 1 || count > 5000) throw readbackError("invalid_expected_statement_count", "expected_statement_count must be an integer from 1 to 5000.");
  return count;
}
function arrayFrom(value, fallback = []) { return Array.isArray(value) ? value : fallback; }
function normalizeExpectations(input = {}) {
  ...
  const columns = arrayFrom(input.expected_columns, defaults.columns || []).map((entry) => ({ table: normalizeIdentifier(entry?.table, "expected_columns.table"), column: normalizeIdentifier(entry?.column, "expected_columns.column") }));
  const indexes = arrayFrom(input.expected_indexes, defaults.indexes || []).map((entry) => ({ table: normalizeIdentifier(entry?.table, "expected_indexes.table"), index: normalizeIdentifier(entry?.index, "expected_indexes.index") }));
  const ruleConditions = arrayFrom(input.expected_rule_conditions, defaults.rule_conditions || []).map((entry) => ({ rule_key: compact(entry?.rule_key, 191), source_type: compact(entry?.source_type, 128), condition_key: compact(entry?.condition_key, 512) })).filter((entry) => entry.rule_key && entry.source_type && entry.condition_key);
  return { migration, tables, columns, indexes, ruleConditions };
}
function placeholders(values = []) { return values.map(() => "?").join(", "); }
function unique(values = []) { return [...new Set(values.filter(Boolean))]; }
async function queryRows(pool, sql, params = []) { const [rows] = await pool.query(sql, params); return rows || []; }
async function readMigrationFile({ migration, expectedChecksum, expectedStatementCount, readFile, migrationsDir }) {
  ...
  if (checksum !== expectedChecksum) throw readbackError("migration_checksum_mismatch", "Merged migration checksum does not match expected_checksum_sha256.", 409, { migration, expected_checksum_sha256: expectedChecksum, actual_checksum_sha256: checksum });
  if (statementCount !== expectedStatementCount) throw readbackError("migration_statement_count_mismatch", "Merged migration statement count does not match expected_statement_count.", 409, { migration, expected_statement_count: expectedStatementCount, actual_statement_count: statementCount });
...
function buildMissing({ tables, columns, indexes, ruleConditions, tableRows, columnRows, indexRows, ruleRows }) {
  ...
  return { tables: tables.filter((table) => !presentTables.has(table)), columns: columns.filter((entry) => !presentColumns.has(`${entry.table}.${entry.column}`)), indexes: indexes.filter((entry) => !presentIndexes.has(`${entry.table}.${entry.index}`)), rule_conditions: ruleConditions.filter((entry) => !presentRules.has(`${entry.rule_key}.${entry.source_type}.${entry.condition_key}`)) };
}
export async function runGovernedMigrationSchemaReadback(input = {}, deps = {}) {
  ...
  return { ok: readbackStatus === "pass", readback_status: readbackStatus, migration: expectations.migration, migration_checksum_sha256: migrationFile.checksum, statement_count: migrationFile.statementCount, ledger: ledger ? { found: true, run_id: ledger.run_id, migration_file: ledger.migration_file, migration_checksum_sha256: ledger.migration_checksum_sha256, mode: ledger.mode, applied_at: ledger.applied_at, statement_count: Number(ledger.statement_count || 0), preflight_status: ledger.preflight_status || null, preflight_risk_count: Number(ledger.preflight_risk_count || 0), secrets_included: Boolean(Number(ledger.secrets_included || 0)), capability_envelope_id: ledger.capability_envelope_id || null } : { found: false }, schema: { tables: tableRows, columns: columnRows, indexes: indexRows, rule_conditions: ruleRows }, expectations: { tables: expectations.tables, columns: expectations.columns, indexes: expectations.indexes, rule_conditions: expectations.ruleConditions, missing }, provider_call_executed: false, external_write_executed: false, row_data_read: false, freeform_sql_accepted: false, secrets_included: false };
}
