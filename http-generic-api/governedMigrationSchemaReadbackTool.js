...
function normalizeIdentifier(value = "", field = "identifier") {
  ...
  if (!IDENTIFIER_PATTERN.test(identifier)) {
    throw readbackError("invalid_schema_readback_identifier", `${field} must contain only letters, numbers, and underscores.`, 400, {
      field,
    });
  }
  return identifier;
...
function normalizeExpectations(input = {}) {
  ...
  const ruleConditions = arrayFrom(input.expected_rule_conditions, defaults.rule_conditions || []).map((entry) => ({
...
async function readMigrationFile({ migration, expectedChecksum, expectedStatementCount, readFile, migrationsDir }) {
  ...
  if (checksum !== expectedChecksum) {
    throw readbackError("migration_checksum_mismatch", "Merged migration checksum does not match expected_checksum_sha256.", 409, {
      migration,
  ...
  if (statementCount !== expectedStatementCount) {
    throw readbackError("migration_statement_count_mismatch", "Merged migration statement count does not match expected_statement_count.", 409, {
      migration,
...
export async function runGovernedMigrationSchemaReadback(input = {}, deps = {}) {
  ...
  const migrationFile = await readMigrationFile({
  ...
  const allTables = unique([
    ...expectations.tables,
    ...expectations.columns.map((entry) => entry.table),
    ...expectations.indexes.map((entry) => entry.table),
  ]);
  ...
  const tableRows = allTables.length
    ? await queryRows(
        pool,
        `SELECT TABLE_NAME
           FROM INFORMATION_SCHEMA.TABLES
          WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME IN (${placeholders(allTables)})
          ORDER BY TABLE_NAME`,
        allTables
      )
    : [];
  const columnRows = allTables.length && allColumns.length
    ? await queryRows(
        pool,
        `SELECT TABLE_NAME, COLUMN_NAME, COLUMN_TYPE, IS_NULLABLE, COLUMN_DEFAULT
          ...
          WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME IN (${placeholders(allTables)})
            AND COLUMN_NAME IN (${placeholders(allColumns)})
          ORDER BY TABLE_NAME, ORDINAL_POSITION`,
        [...allTables, ...allColumns]
      )
    : [];
  const indexRows = allTables.length && allIndexes.length
    ? await queryRows(
        pool,
        `SELECT TABLE_NAME, INDEX_NAME, COLUMN_NAME, SEQ_IN_INDEX, NON_UNIQUE
          ...
          WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME IN (${placeholders(allTables)})
            AND INDEX_NAME IN (${placeholders(allIndexes)})
          ORDER BY TABLE_NAME, INDEX_NAME, SEQ_IN_INDEX`,
        [...allTables, ...allIndexes]
      )
    : [];
  const ledgerRows = await queryRows(
    pool,
    `SELECT run_id, migration_file, migration_checksum_sha256, mode, applied_at,
            statement_count, preflight_status, preflight_risk_count, secrets_included,
            capability_envelope_id
       FROM governed_migration_ledger
      WHERE migration_file = ?
        AND migration_checksum_sha256 = ?
      ORDER BY applied_at DESC
      LIMIT 1`,
    [expectations.migration, expectedChecksum]
  );
  const ruleRows = expectations.ruleConditions.length
    ? await queryRows(
        pool,
        `SELECT rule_key, source_type, condition_key, status, updated_at
           FROM operational_alert_rule_registry
          WHERE (${expectations.ruleConditions.map(() => "(rule_key = ? AND source_type = ? AND condition_key = ?)").join(" OR ")})
          ORDER BY rule_key, source_type`,
        expectations.ruleConditions.flatMap((entry) => [entry.rule_key, entry.source_type, entry.condition_key])
      )
    : [];
  ...
  const missing = buildMissing({
    ...
    ruleConditions: expectations.ruleConditions,
    tableRows,
    columnRows,
    indexRows,
    ruleRows,
  ...
  const readbackStatus = ledger && !missing.tables.length && !missing.columns.length && !missing.indexes.length && !missing.rule_conditions.length
    ? "pass"
    : "fail";
  ...
  return {
    ...
    ledger: ledger ? {
      ...
      migration_file: ledger.migration_file,
      migration_checksum_sha256: ledger.migration_checksum_sha256,
      mode: ledger.mode,
      applied_at: ledger.applied_at,
      statement_count: Number(ledger.statement_count || 0),
      preflight_status: ledger.preflight_status || null,
      preflight_risk_count: Number(ledger.preflight_risk_count || 0),
      secrets_included: Boolean(Number(ledger.secrets_included || 0)),
      capability_envelope_id: ledger.capability_envelope_id || null,
    } : { found: false },
    schema: {
      tables: tableRows,
      columns: columnRows,
      indexes: indexRows,
      rule_conditions: ruleRows,
    },
    expectations: {
      tables: expectations.tables,
      columns: expectations.columns,
      indexes: expectations.indexes,
      rule_conditions: expectations.ruleConditions,
      missing,
    },
