...
assert.match(connectorProvisioning, /connector_tunnel_hostname_owner_conflict/);
assert.match(connectorProvisioning, /connector_tunnel_hostname_ownership_unverified/);
assert.match(connectorProvisioning, /remove_duplicate_hostname_route_then_retry/);
assert.match(connectorProvisioning, /cfd_tunnel\?is_deleted=false&per_page=100/);
assert.match(connectorProvisioning, /cfd_tunnel\/\$\(\$candidate\.id\)\/configurations/);
assert.match(connectorProvisioning, /hostname_ownership_verified/);
...
assert.match(liveCertification, /assertActivationGatewayProfilePolicy\("staging", gatewayPolicy, convergenceRegistry\)/);
