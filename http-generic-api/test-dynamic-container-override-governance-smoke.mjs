// Rebuilt on current main; generated artifacts remain CI-owned.
...
const authorityRepository=readFileSync(new URL("./dynamicContainerAuthorityRepository.js",import.meta.url),"utf8");
...
assert.match(service,/consumeContainerOverride/);
assert.match(service,/capability_resolution_envelope_tenant_required/);
assert.match(service,/BINARY tenant_id = BINARY \?/);
assert.match(service,/override_governance_smoke_tenant_mismatch/);
assert.doesNotMatch(service,/envelope\.tenant_id \|\| ""\)\.trim/);
assert.match(service,/override_snapshot_stale/);
assert.doesNotMatch(service,/UPDATE container_override_requests SET status='stale'/);
...
assert.match(service,/override_governance_smoke_closure_thread_readback_failed/);
const closureReadbackIndex=service.indexOf("SELECT thread_key,state,observed_evidence_json");
const commitIndex=service.indexOf("await connection.commit();");
assert.ok(closureReadbackIndex >= 0 && commitIndex >= 0 && closureReadbackIndex < commitIndex,"closure evidence readback must complete before commit");
assert.match(authorityRepository,/executor = null, manageTransaction = true/);
assert.match(authorityRepository,/override_executor_required/);
assert.match(authorityRepository,/container_override_consumption_rollback_failed/);
...
assert.match(openapiFragment,/OverrideGovernanceSmokeRequest:/);
...
assert.match(openapiRouteContracts,/route_file: 'routes\/dynamicContainerOverrideGovernanceSmokeRoutes\.js'/);
...
assert.doesNotMatch(migration,/`metadata_json`=VALUES\(`metadata_json`\)/);
