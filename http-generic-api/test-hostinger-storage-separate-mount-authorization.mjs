...
function dispatchPacket(overrides={}) {
  ...
  return buildHostingerStorageFixedDispatchCertification({worker_certification:worker,evidence,expected:{...expected,...(overrides.expected||{})},now:NOW});
}
...
function bridgePacket({ready=true}={}) {
  ...
  const core={contract:'spec014.hostinger-storage-durable-tenant-runtime-bridge-readiness.v1',version:'spec014-hostinger-storage-durable-tenant-runtime-bridge-readiness-v1',
    ...
    evidence:{contract:'spec014.hostinger-storage-durable-tenant-runtime-bridge-evidence.v1',capabilities,secrets_included:false},blockers,ready_for_separate_mount_authorization:ready,
...
const verified=verifyHostingerStorageSeparateMountAuthorization({packet,expected_digest:packet.authorization_digest});assert.equal(verified.valid,true);assert.equal(verified.ready_for_authorized_mount_execution,true);
...
const tampered=structuredClone(packet);tampered.evidence.authorization.consumed=true;expectCode(()=>verifyHostingerStorageSeparateMountAuthorization({packet:tampered}),'STORAGE_MOUNT_AUTH_PACKET_TAMPERED');

console.log(JSON.stringify({ok:true,version:HOSTINGER_STORAGE_SEPARATE_MOUNT_AUTHORIZATION_VERSION,ready_packet_verified:verified.valid,regressions:12,authorization_persisted:false,mount_performed:false,provider_dispatch_allowed:false,production_ready:false,secrets_included:false},null,2));
