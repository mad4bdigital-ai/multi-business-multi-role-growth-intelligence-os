...
function patchIntent() {
  return {
    ...
    files: [
      ...
      { path: "docs/example.md", action: "create_file", content_sha256: HASH_3 },
    ],
  };
}
...
function approvalFor(head = HEAD_B, base = BASE_A) {
  return createMergeApprovalBinding({
    pull_number: 6000,
    final_head_sha: head,
    final_base_sha: base,
...
function receiptsFor(head = HEAD_B, base = BASE_A) {
  return {
    merge: { merged: true, expected_head_sha: head, expected_base_sha: base, merge_sha: MERGE_C },
    branch_delete: { verified_absent: true, expected_head_sha: head },
    deployment: { receipt_id: "deploy-receipt-1", deployed_sha: MERGE_C },
...
assert.throws(
  () => normalizeSemanticPatchIntent({ files: [{ path: "../escape.js", action: "create_file", content_sha256: HASH_1 }] }),
...
assert(verifyMergeApprovalBinding(approval, {
  ...
  final_base_sha: BASE_A,
  now: "2026-08-03T08:30:00.000Z",
}).reasons.includes("final_head_sha_mismatch"));
assert(verifyMergeApprovalBinding(approval, {
  pull_number: 6000,
  final_head_sha: HEAD_B,
  final_base_sha: BASE_A,
...
const receiptValidation = validateManagedDeliveryReceipts(receiptsFor(), {
  final_head_sha: HEAD_B,
  final_base_sha: BASE_A,
});
...
const executeCalls = [];
const completed = await executeManagedDeliveryOperation({ ...baseInput(), approval_binding: approval }, {
  now: "2026-08-03T08:30:00.000Z",
  ...
  readLifecycleReceipts: async () => receiptsFor(),
});
...
const movedApproval = approvalFor(HEAD_E, BASE_D);
let stateReadCount = 0;
const cancelledRuns = [];
const movedExecuteCalls = [];
const moved = await executeManagedDeliveryOperation({ ...baseInput(), approval_binding: movedApproval }, {
  now: "2026-08-03T08:30:00.000Z",
  ...
  readLifecycleReceipts: async () => receiptsFor(HEAD_E, BASE_D),
});
...
const staleApproval = approvalFor(HEAD_E, BASE_D);
const approvalPending = await executeManagedDeliveryOperation({ ...baseInput(), approval_binding: staleApproval }, {
  now: "2026-08-03T08:30:00.000Z",
  readRepositoryState: async () => ({ base_sha: BASE_A, head_sha: HEAD_B, active_runs: [] }),
  readCiDiagnosis: async () => ({ status: "success", missing_checks: [], secrets_included: false }),
