...
export const RESOURCE_DESCRIPTORS = Object.freeze({
  ...
  assets: {
    ...
    fields: `r.asset_id,r.tenant_id,r.vault_id,r.asset_type,r.asset_ref,r.display_name,r.brand_ref,
      ...
      CASE WHEN JSON_VALID(r.metadata_json) THEN JSON_UNQUOTE(JSON_EXTRACT(r.metadata_json,'$.schema_version')) ELSE NULL END AS provenance_schema_version,
      CASE WHEN JSON_VALID(r.metadata_json) THEN JSON_UNQUOTE(JSON_EXTRACT(r.metadata_json,'$.source_type')) ELSE NULL END AS source_type,
      CASE WHEN JSON_VALID(r.metadata_json) THEN JSON_UNQUOTE(JSON_EXTRACT(r.metadata_json,'$.source_provider')) ELSE NULL END AS source_provider,
      CASE WHEN JSON_VALID(r.metadata_json) THEN JSON_UNQUOTE(JSON_EXTRACT(r.metadata_json,'$.source_revision')) ELSE NULL END AS source_revision,
      CASE WHEN JSON_VALID(r.metadata_json) THEN JSON_UNQUOTE(JSON_EXTRACT(r.metadata_json,'$.content_sha256')) ELSE NULL END AS content_sha256,
      CASE WHEN JSON_VALID(r.metadata_json) THEN JSON_UNQUOTE(JSON_EXTRACT(r.metadata_json,'$.content_identity')) ELSE NULL END AS content_identity,
      CASE WHEN JSON_VALID(r.metadata_json) THEN JSON_UNQUOTE(JSON_EXTRACT(r.metadata_json,'$.ingestion_mode')) ELSE NULL END AS ingestion_mode,
      CASE WHEN JSON_VALID(r.metadata_json) THEN CASE WHEN JSON_EXTRACT(r.metadata_json,'$.source_uri') IS NOT NULL THEN 1 ELSE 0 END ELSE 0 END AS source_locator_present`,
