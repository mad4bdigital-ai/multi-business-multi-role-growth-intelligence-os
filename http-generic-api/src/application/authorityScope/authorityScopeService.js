...
function requireRepository(repository) {
  if (
    !repository ||
    typeof repository.findByKey !== "function" ||
    typeof repository.findByTenantId !== "function"
  ) {
    throw new TypeError(
      "Authority scope service requires findByKey and findByTenantId repository methods."
    );
  }
}

export function createAuthorityScopeService({ repository }) {
  requireRepository(repository);

  async function resolve({ auth = {}, tenantId = null, scopeKey = null } = {}) {
    ...
    if (explicitScopeKey) {
      ...
      throw new AuthorityScopeError(
        ...
        "tenantId or scopeKey is required for a tenant principal.",
        400
      );
    }
    ...
    if (!row) {
      throw new AuthorityScopeError(
        "AUTHORITY_SCOPE_NOT_REGISTERED",
        "No registered authority scope matches the request.",
        404
      );
    }
    ...
    if (scope.scopeType === AUTHORITY_SCOPE_TYPES.PLATFORM) {
      if (!platformPrincipal) {
        throw new AuthorityScopeError(
          ...
          "Only a platform principal may resolve the platform authority scope.",
          403
        );
      }
      if (explicitTenantId) {
        throw new AuthorityScopeError(
          "PLATFORM_SCOPE_TENANT_MISMATCH",
          "Platform authority scope cannot be resolved with a tenant target.",
          409
        );
      }
    } else {
      ...
      if (platformPrincipal) {
        if (!explicitTenantId) {
          throw new AuthorityScopeError(
            ...
            "Platform access to a tenant authority scope requires an explicit tenantId.",
            400
          );
        }
        if (explicitTenantId !== scope.tenantId) {
          throw new AuthorityScopeError(
            "AUTHORITY_SCOPE_TENANT_MISMATCH",
            "Requested tenant does not match the resolved authority scope.",
            409
          );
        }
      } else {
        if (!principalTenantId || principalTenantId !== scope.tenantId) {
          throw new AuthorityScopeError(
            ...
            "Principal cannot resolve an authority scope owned by another tenant.",
            403
          );
        }
...
export const _testingAuthorityScopeService = Object.freeze({
  cleanString,
});
