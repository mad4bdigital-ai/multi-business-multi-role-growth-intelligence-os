...
assert(
  routes.includes('router.post("/connect/api/integration-policy"'),
  "connect integration policy update route must exist"
);
assert(
  routes.includes("pool.getConnection") &&
    routes.includes("beginTransaction") &&
    routes.includes("commit()") &&
    routes.includes("rollback()"),
...
assert(
  routes.includes("readiness_unavailable") &&
    routes.includes("hybrid_integration_readiness_unavailable"),
  "post-write readiness failures must be reported as degraded readback, not as a failed mutation response"
);
assert(
  policy.includes("db = null") &&
    policy.includes("const executor = db || getPool();") &&
    policy.includes("await executor.query("),
