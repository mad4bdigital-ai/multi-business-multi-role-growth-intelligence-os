...
assert.deepEqual(policy.environments.staging.active_tunnel_hostnames, ["dev.mad4b.com"]);
assert.deepEqual(policy.environments.staging.reserved_disabled_hostnames, ["mcp_dev.mad4b.com", "activation_dev.mad4b.com"]);
...
assert.match(env, /CLOUDFLARE_TUNNEL_HOSTNAMES=dev\.mad4b\.com\s*$/m);
assert.doesNotMatch(env, /CLOUDFLARE_TUNNEL_HOSTNAMES=.*(mcp_dev|activation_dev)/);
...
assert.equal(String(tunnel.environment.TUNNEL_HOSTNAMES), "${CLOUDFLARE_TUNNEL_HOSTNAMES:-dev.mad4b.com}");
