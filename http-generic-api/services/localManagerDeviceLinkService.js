...
async function upsertConnectorAlias({ aliasDeviceId, canonical, principal, reason }) {
  const pool = getPool();
...
async function ensureLocalConnectorAliasForDeviceLink({ session, principal }) {
  try {
    ...
    for (const aliasDeviceId of aliasInputs) {
      const alias = await upsertConnectorAlias({
        aliasDeviceId,
        canonical,
        principal,
        reason: "Auto-created from Local Manager device-link approval so app hostname/device id resolves to the executable local connector device.",
      });
