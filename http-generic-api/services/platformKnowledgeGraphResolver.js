...
export async function projectPlatformKnowledgeGraph({ projectionKey = "runtime_projection", dryRun = false } = {}) {
  ...
  try {
    ...
    for (const r of devices) {
      ...
      if (r.user_id) addEdge(edges, { source_node_id: device, edge_type: "linked_to", target_node_id: nodeId("user", r.user_id), scope_type: "user", source_table: "local_manager_device_link_sessions", source_pk: r.session_id, authority_status: "authoritative", runtime_role: "resolver_input", runtime_enforced: true });
      if (r.tenant_id) addEdge(edges, { source_node_id: device, edge_type: "linked_to", target_node_id: nodeId("tenant", r.tenant_id), scope_type: "tenant", source_table: "local_manager_device_link_sessions", source_pk: r.session_id, authority_status: "authoritative", runtime_role: "resolver_input", runtime_enforced: true });
    }
