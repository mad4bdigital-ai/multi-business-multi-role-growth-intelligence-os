...
export async function projectPlatformKnowledgeGraph({ projectionKey = "runtime_projection", dryRun = false } = {}) {
  ...
  try {
    ...
    for (const r of actionGrants) { const grant = nodeId("action_grant", r.grant_id); const conn = nodeId("connection", r.connection_id); const action = nodeId("action", r.action_key); addNode(nodes, { node_id: grant, node_type: "action_grant", node_label: `${r.app_key}:${r.action_key}`, source_table: "app_action_grants", source_pk: r.grant_id, authority_status: "authoritative", lifecycle_status: lifecycle(r.status), runtime_role: "authority", metadata_json: { workspace_id: r.workspace_id, agent_id: r.agent_id, app_key: r.app_key, grant_mode: r.grant_mode, expires_at: r.expires_at } }); addNode(nodes, { node_id: conn, node_type: "connection", node_label: r.connection_id, source_table: "app_action_grants", source_pk: r.grant_id, authority_status: "candidate", sensitivity: "secret_reference" }); addNode(nodes, { node_id: action, node_type: "action", node_label: r.action_key, source_table: "app_action_grants", source_pk: r.grant_id, authority_status: "candidate" }); addEdge(edges, { source_node_id: conn, edge_type: "grants_action", target_node_id: grant, source_table: "app_action_grants", source_pk: r.grant_id, authority_status: "authoritative", runtime_role: "authority", runtime_enforced: r.status === "active" }); addEdge(edges, { source_node_id: grant, edge_type: "grants_action", target_node_id: action, source_table: "app_action_grants", source_pk: r.grant_id, authority_status: "authoritative", runtime_role: "authority", runtime_enforced: r.status === "active" }); }
