...
const failedSession = buildHardActivationEvidenceMatrix({
  sessionContext: { ok: true, activation_layer: 'session_context', session_id: 'session-123' },
  providerBootstrap: activeProviderBootstrap,
});
assert.equal(failedSession.activation_complete, false);
assert.equal(failedSession.reason_code, 'degraded_session_context_failed');
assert.equal(canReportSessionContextLoaded(failedSession.session_context), false);
