...
const migration = readFileSync("migrations/216_sprint68_connection_capability_repair_before_fallback_policy.sql", "utf8");
...
assert(migration.includes("fallback_missing_operation_mapping"), "policy must trigger on missing fallback mappings");
...
assert(migration.includes("run_targeted_regression_test"), "policy must require regression coverage");
...
assert.match(migration, /'TRUE',\r?\n\s+'connector_dispatch/, "policy must be active and dispatch-scoped");
assert.match(migration, /'TRUE',\r?\n\s+'Do not accept fallback gaps/, "policy must be blocking governance");
