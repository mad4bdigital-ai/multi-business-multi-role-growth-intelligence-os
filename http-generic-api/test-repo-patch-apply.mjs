...
// ── Protected branch policy static contract ───────────────────────────────────
{
  ...
  const lifecycleCoreSource = readFileSync(new URL("./githubRepositoryLifecycleCore.js", import.meta.url), "utf8");
  const lifecycleContractSource = `${lifecycleSource}\n${lifecycleCoreSource}`;
  ...
  assert.ok(lifecycleSource.includes('from "./githubRepositoryLifecycleCore.js"'), "public repository lifecycle wrapper must remain bound to the canonical core implementation");
  assert.ok(lifecycleContractSource.includes("github_change_set_default_branch_overlap"), "repo patch batch must stop when moved main overlaps requested patch paths");
  assert.ok(lifecycleContractSource.includes("github_change_set_default_branch_compare_truncated"), "repo patch batch must fail closed if moved-main changed-file evidence is truncated");
  assert.ok(lifecycleContractSource.includes("expected_branch_sha"), "repo patch batch must validate existing work branch head for continuation");
  assert.ok(lifecycleContractSource.includes("same_branch_continuation_used"), "repo patch batch readback must disclose continuation mode");
  assert.ok(lifecycleContractSource.includes("default_branch_drift"), "repo patch batch readback must disclose moved-main drift evidence");
  assert.ok(lifecycleContractSource.includes("github_change_set_patch_noop"), "repo patch batch must reject unified diffs that produce no content change");
  assert.ok(lifecycleContractSource.includes("patchedContent === current.content"), "repo patch batch must compare patched content with source content before committing");
  assert.ok(lifecycleContractSource.includes("github_change_set_empty_tree"), "repo patch batch must reject empty tree commits instead of reporting success with zero file edits");
  assert.ok(lifecycleContractSource.includes("newTreeSha === parentTreeSha"), "repo patch batch must compare the created tree with the parent tree before committing");
  assert.ok(lifecycleContractSource.includes("github_change_set_tree_readback_missing"), "repo patch batch must fail closed when GitHub does not return a valid tree SHA");
