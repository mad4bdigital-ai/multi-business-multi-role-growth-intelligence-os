...
export async function tenantRepositoryMutationReadbackV6(args = {}, { auth = {}, authorityStorePool = null } = {}) {
  ...
  const authorityPool = resolvePlatformResourceAuthorityPool({ authorityStorePool });
  const [[binding]] = await authorityPool.query(`SELECT * FROM platform_resource_authority_bindings WHERE binding_id=? AND status='active' AND (expires_at IS NULL OR expires_at>NOW())`, [run.binding_id]);
...
export async function tenantRepositoryGovernanceV6ReadinessSmoke(args = {}, { auth = {}, runGovernedResource, readinessRunGovernedResource } = {}) {
  ...
  if(!isAdmin(auth)) return {ok:false,tool:'tenant_repository_governance_v6_readiness_smoke',status:'authorization_gated',classification:'repository_governance_v6_admin_required',reason_code:'repository_governance_v6_readiness_admin_required',checks:[{name:'admin_principal_required',pass:true},{name:'no_mutation',pass:true},{name:'no_secrets',pass:true}],apply_allowed:false,mutations_executed:false,secrets_included:false}; if(typeof readinessRunGovernedResource!=='function') return {ok:false,tool:'tenant_repository_governance_v6_readiness_smoke',status:'fail',classification:'repository_governance_v6_readiness_executor_missing',reason_code:'repository_governance_v6_readiness_executor_missing',checks:[{name:'admin_readiness_executor_present',pass:false},{name:'no_mutation',pass:true},{name:'no_secrets',pass:true}],apply_allowed:false,mutations_executed:false,secrets_included:false}; const providerBinding=await findRepositoryGovernanceV6ReadinessBinding(repoRef);
