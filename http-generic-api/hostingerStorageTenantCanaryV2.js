...
export const HOSTINGER_STORAGE_TENANT_CANARY_VERSION = 'spec014-hostinger-storage-tenant-canary-v2';

const SAFE_ID_RE = /^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,255}$/;
const SAFE_REF_RE = /^[A-Za-z0-9][A-Za-z0-9._:/-]{0,510}$/;
const SHA256_RE = /^[0-9a-f]{64}$/i;
const EXECUTOR_REPOSITORY_METHODS = Object.freeze([
  'readAggregate',
  'transitionOperation',
  'consumePlan',
  'appendJournalEvent',
...
function positiveInteger(value, field) {
  const normalized = Number(value);
  if (!Number.isSafeInteger(normalized) || normalized < 1) throw fail(400, 'STORAGE_TENANT_CANARY_INTEGER_INVALID', 'A positive integer is required.', { field });
  return normalized;
}
...
function normalizeAllowlist(record = {}, generation = 1) {
  return {
    allowlist_id: safeId(record.allowlist_id, 'allowlist.allowlist_id'),
    revision: safeId(record.revision, 'allowlist.revision'),
    authority_generation: positiveInteger(generation, 'allowlist.authority_generation'),
    ...
    max_items: positiveInteger(record.max_items, 'allowlist.max_items'),
    max_bytes: positiveInteger(record.max_bytes, 'allowlist.max_bytes'),
    evidence_digest: hash(record.evidence_digest, 'allowlist.evidence_digest'),
    secrets_included: false,
  };
}

function normalizeApproval(record = {}, generation = 1) {
  return {
    approval_id: safeId(record.approval_id, 'approval.approval_id'),
    authority_generation: positiveInteger(generation, 'approval.authority_generation'),
...
export function createMemoryHostingerStorageTenantCanaryAuthorityStore() {
  ...
  const issuedAllowlistTokens = new Map();
  const issuedApprovalTokens = new Map();

  function tokenSet(map, id) {
    if (!map.has(id)) map.set(id, new Set());
    return map.get(id);
  }

  return Object.freeze({
    adapter_key: 'memory_hostinger_storage_tenant_canary_authority_v2',
    synthetic_only: true,
    production_ready: false,
    registerAllowlist(record) {
      const normalized = deepFreeze(normalizeAllowlist(record, 1));
      ...
      if (!existing) {
        allowlists.set(normalized.allowlist_id, normalized);
        tokenSet(issuedAllowlistTokens, normalized.allowlist_id).add(normalized.revision);
      }
      return clone(allowlists.get(normalized.allowlist_id));
    },
    updateAllowlist({ allowlist_id, expected_revision, expected_generation, record } = {}) {
      ...
      if (current.authority_generation !== positiveInteger(expected_generation, 'expected_generation')) throw fail(409, 'STORAGE_TENANT_CANARY_ALLOWLIST_GENERATION_CONFLICT', 'Allowlist generation changed before update.', { current_generation: current.authority_generation });
      const next = deepFreeze(normalizeAllowlist({ ...record, allowlist_id: id }, current.authority_generation + 1));
      if (next.revision === current.revision) throw fail(409, 'STORAGE_TENANT_CANARY_ALLOWLIST_REVISION_NOT_ADVANCED', 'Allowlist updates must advance the revision token.');
      const issued = tokenSet(issuedAllowlistTokens, id);
      if (issued.has(next.revision)) throw fail(409, 'STORAGE_TENANT_CANARY_ALLOWLIST_TOKEN_REUSED', 'Previously issued allowlist revision tokens cannot be reused.', { revision: next.revision });
      issued.add(next.revision);
      allowlists.set(id, next);
      return clone(next);
    },
    readAllowlist(id) {
      return clone(allowlists.get(safeId(id, 'allowlist_id')) || null);
    },
    registerApproval(record) {
      const normalized = deepFreeze(normalizeApproval(record, 1));
      ...
      if (!existing) {
        approvals.set(normalized.approval_id, normalized);
        tokenSet(issuedApprovalTokens, normalized.approval_id).add(normalized.evidence_digest);
      }
      return clone(approvals.get(normalized.approval_id));
    },
    updateApproval({ approval_id, expected_evidence_digest, expected_generation, record } = {}) {
      ...
      if (current.authority_generation !== positiveInteger(expected_generation, 'expected_generation')) throw fail(409, 'STORAGE_TENANT_CANARY_APPROVAL_GENERATION_CONFLICT', 'Approval generation changed before update.', { current_generation: current.authority_generation });
      const next = deepFreeze(normalizeApproval({ ...record, approval_id: id }, current.authority_generation + 1));
      if (next.evidence_digest === current.evidence_digest) throw fail(409, 'STORAGE_TENANT_CANARY_APPROVAL_EVIDENCE_NOT_ADVANCED', 'Approval updates must advance the evidence token.');
      const issued = tokenSet(issuedApprovalTokens, id);
      if (issued.has(next.evidence_digest)) throw fail(409, 'STORAGE_TENANT_CANARY_APPROVAL_TOKEN_REUSED', 'Previously issued approval evidence tokens cannot be reused.', { evidence_digest: next.evidence_digest });
      issued.add(next.evidence_digest);
      approvals.set(id, next);
      return clone(next);
    },
    readApproval(id) {
      return clone(approvals.get(safeId(id, 'approval_id')) || null);
    },
    exportState() {
      return clone({
        allowlists: [...allowlists.values()].sort((a, b) => a.allowlist_id.localeCompare(b.allowlist_id)),
        approvals: [...approvals.values()].sort((a, b) => a.approval_id.localeCompare(b.approval_id)),
      });
    },
  });
}
...
export function createMemoryHostingerStorageTenantCanaryEnablementRegistry() {
  ...
  return Object.freeze({
    adapter_key: 'memory_hostinger_storage_tenant_canary_enablement_v2',
    synthetic_only: true,
    production_ready: false,
    register(record) {
      const normalized = deepFreeze({
        ...
        generation: positiveInteger(record?.generation, 'enablement.generation'),
    ...
    consume({ enablement_id, authorization_digest, operation_id, run_id, expected_generation, now_epoch }) {
      ...
      if (current.authorization_digest !== hash(authorization_digest, 'authorization_digest')
        ...
        || current.run_id !== safeId(run_id, 'run_id')) throw fail(409, 'STORAGE_TENANT_CANARY_ENABLEMENT_BINDING_MISMATCH', 'Manual enablement is not bound to this authorization and run.');
      if (current.generation !== positiveInteger(expected_generation, 'expected_generation')) throw fail(409, 'STORAGE_TENANT_CANARY_ENABLEMENT_GENERATION_MISMATCH', 'Manual enablement generation changed.', { current_generation: current.generation });
    ...
    exportState() {
      return clone([...records.values()].sort((a, b) => a.enablement_id.localeCompare(b.enablement_id)));
    },
  });
}
...
function requireAuthorityStore(store) {
  if (!store || store.synthetic_only !== true || store.production_ready !== false
    || typeof store.readAllowlist !== 'function' || typeof store.readApproval !== 'function') throw fail(409, 'STORAGE_TENANT_CANARY_AUTHORITY_STORE_INVALID', 'A non-production authoritative canary store is required.');
}

function requireRegistry(registry) {
  if (!registry || registry.synthetic_only !== true || registry.production_ready !== false
    || typeof registry.read !== 'function' || typeof registry.consume !== 'function') throw fail(409, 'STORAGE_TENANT_CANARY_ENABLEMENT_REGISTRY_INVALID', 'A non-production one-shot enablement registry is required.');
}

function requireControlPlaneRepository(repository) {
  const missing = EXECUTOR_REPOSITORY_METHODS.filter((method) => typeof repository?.[method] !== 'function');
  if (!repository || repository.production_ready === true || missing.length) throw fail(409, 'STORAGE_TENANT_CANARY_CONTROL_PLANE_INVALID', 'Tenant canary requires the complete non-production executor repository contract.', { missing_methods: missing });
}

function revalidateCurrentAuthority({ authorization, store, now }) {
  const signedAllowlist = authorization.allowlist;
  const currentAllowlist = store.readAllowlist(signedAllowlist.allowlist_id);
  ...
  for (const field of ['revision', 'status', 'environment', 'target_scope', 'tenant_id', 'workspace_id', 'resource_id', 'target_id', 'root_ref', 'path_ref_prefix', 'shared_target', 'platform_target', 'valid_from_epoch', 'expires_at_epoch', 'max_items', 'max_bytes', 'evidence_digest']) {
    if (currentAllowlist[field] !== signedAllowlist[field]) allowlistMismatches.push(field);
  }
  if (currentAllowlist.status !== 'active' || currentAllowlist.valid_from_epoch > now || currentAllowlist.expires_at_epoch <= now) allowlistMismatches.push('current_authority');
  if (allowlistMismatches.length) throw fail(409, 'STORAGE_TENANT_CANARY_ALLOWLIST_CURRENT_STATE_INVALID', 'Current allowlist no longer matches the signed Tenant-exclusive authority.', { mismatches: [...new Set(allowlistMismatches)].sort() });

  const signedApproval = authorization.workspace_owner_approval;
  const currentApproval = store.readApproval(signedApproval.approval_id);
  ...
  for (const field of ['slot', 'status', 'tenant_id', 'workspace_id', 'operation_id', 'target_id', 'plan_hash', 'authority_context_hash', 'approver_role', 'approved_at_epoch', 'expires_at_epoch', 'evidence_digest']) {
    if (currentApproval[field] !== signedApproval[field]) approvalMismatches.push(field);
  }
  if (currentApproval.status !== 'approved' || currentApproval.expires_at_epoch <= now) approvalMismatches.push('current_authority');
...
function preflight({ protocol, protocolDigest, repository, adapter, now }) {
  ...
  if (!adapter || adapter.synthetic_only !== true || adapter.production_ready !== false || adapter.live_provider !== false
    ...
    || typeof adapter.readMutationReceipt !== 'function') throw fail(409, 'STORAGE_TENANT_CANARY_EXECUTOR_ADAPTER_INVALID', 'Canary execution requires the canonical in-memory synthetic adapter contract.');
  const aggregate = repository.readAggregate(protocol.operation_id);
  if (!aggregate?.operation) throw fail(404, 'STORAGE_TENANT_CANARY_OPERATION_NOT_FOUND', 'Canary operation aggregate was not found during preflight.');
  if (!['lease_acquired', 'executing'].includes(aggregate.operation.state)) throw fail(409, 'STORAGE_TENANT_CANARY_EXECUTOR_OPERATION_STATE_INVALID', 'Synthetic executor state is not eligible for a new canary attempt.', { state: aggregate.operation.state });
...
export function executeHostingerStorageTenantCanary({
  ...
  const mismatches = [];
  for (const field of ['tenant_id', 'workspace_id', 'resource_id', 'target_id', 'authority_context_hash', 'ownership_revision', 'policy_revision']) if (current[field] !== authorization.operation[field]) mismatches.push(field);
  ...
  revalidateCurrentAuthority({ authorization, store: authority_store, now });
  preflight({ protocol, protocolDigest: protocol_digest, repository, adapter, now });

  const enablement = authorization.manual_enablement;
  if (!enablement_registry.read(enablement.enablement_id)) throw fail(404, 'STORAGE_TENANT_CANARY_ENABLEMENT_NOT_FOUND', 'Manual canary enablement was not registered.');
  ...
  const projection = {
    schema_version: 1,
    projection_key: 'hostinger_storage_tenant_canary_result_v2',
