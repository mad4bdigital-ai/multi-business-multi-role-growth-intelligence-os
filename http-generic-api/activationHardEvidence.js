...
export function classifyHardActivationEvidence(matrix = {}) {
  ...
  if (!session.attempted) {
    return {
      ...
      reason_code: HARD_ACTIVATION_REASON_CODES.MISSING_SESSION_CONTEXT,
    };
  }
  if (!session.ok) {
    return {
      ...
      activation_complete: false,
      status_authority: "runtime_canonical",
      reason_code: HARD_ACTIVATION_REASON_CODES.SESSION_CONTEXT_FAILED,
    };
  }
  if (!provider.attempted) {
    return {
      ...
      activation_complete: false,
      status_authority: "runtime_canonical",
      reason_code: HARD_ACTIVATION_REASON_CODES.MISSING_PROVIDER_BOOTSTRAP,
    };
  }
  if (!provider.ok) {
    return {
      activation_status: provider.activation_status || "validating",
      activation_complete: false,
      status_authority: "runtime_canonical",
      reason_code: provider.reason_code || HARD_ACTIVATION_REASON_CODES.PROVIDER_BOOTSTRAP_FAILED,
    };
  }
  return {
    ...
    status_authority: "runtime_canonical",
    reason_code: HARD_ACTIVATION_REASON_CODES.ACTIVE,
  };
}
