...
assert.equal(inventory.contract_authority.canonical_source, "http-generic-api/openapi.yaml");
assert.equal(inventory.contract_authority.surface_registry, "canonicals/openapi/custom-gpt-surfaces.yaml");
assert.equal(inventory.contract_authority.surface_key, "tenant_activation");
assert.equal(inventory.contract_authority.generator, "http-generic-api/scripts/split-openapi.mjs");
assert.equal(inventory.contract_authority.orchestrator, "http-generic-api/scripts/generate-custom-gpt-schemas.mjs");
assert.equal(inventory.contract_authority.generated_artifact, "http-generic-api/openapi/openapi.tenant-gpt.activation.yaml");
assert.equal(inventory.contract_authority.canonical_generation_status, "designated_and_ci_verified");
assert.equal(surfaceRegistry.source_openapi, inventory.contract_authority.canonical_source);
assert.equal(surfaceRegistry.surfaces.tenant_activation.mode, "generated_from_openapi");
assert.equal(surfaceRegistry.surfaces.tenant_activation.output_file, "openapi/openapi.tenant-gpt.activation.yaml");
assert(splitGeneratorSource.includes("SURFACE_REGISTRY_FILE"));
assert(splitGeneratorSource.includes("validateGeneratedDoc"));
assert(schemaOrchestratorSource.includes("generatedSchemaArtifacts"));
assert(schemaOrchestratorSource.includes("validateOpenApiFiles"));
