...
export function verifyTenantGptAccessToken(token, {
  ...
  let payload;
  try {
    payload = jwt.verify(String(token || ""), JWT_SECRET, { issuer: TENANT_GPT_AUTHORIZATION_SERVER });
  } catch {
...
export function requireActivationTenantGptAccessToken(req, res, next) {
  ...
  if (!token || token === authorization) {
    return res.status(401).json({
      ok: false,
      error: { code: "user_jwt_required", message: "A Tenant GPT bearer access token is required.", requestId: res.locals?.request_id || null },
      secrets_included: false,
    });
  }
  ...
  try {
    ...
    return res.status(error.status || 401).json({
      ok: false,
      error: { code: error.code || "invalid_user_jwt", message: error.message || "User token is invalid or expired.", requestId: res.locals?.request_id || null },
      secrets_included: false,
    });
  }
}
