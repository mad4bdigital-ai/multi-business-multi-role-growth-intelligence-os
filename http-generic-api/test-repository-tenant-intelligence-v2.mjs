...
assert.match(runtimeSource, /selectRepositoryNegativeSmokeMembership/);
assert.match(runtimeSource, /FROM memberships m/);
assert.match(runtimeSource, /m\.status='active'/);
assert.match(runtimeSource, /user_id:negativeUserId/);
assert.match(runtimeSource, /active_unbound_tenant_membership_required/);
assert.doesNotMatch(runtimeSource, /repository_intelligence_v2_missing_\$\{randomUUID\(\)\}/);
