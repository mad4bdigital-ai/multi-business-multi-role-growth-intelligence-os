...
assert.equal(record.status, "consumption_foundation_complete_runtime_wiring_required");
...
assert.doesNotMatch(authRoutes, /classifyTenantGptOAuthTokenExchangeOutcome/u,
  "foundation must not claim live route wiring");
assert.doesNotMatch(authRoutes, /buildTenantGptOAuthTokenErrorResponse/u,
  "foundation must not claim live response wiring");

for (const value of Object.values(record.non_effects)) {
  assert.equal(value, false);
}

console.log("Spec 012 T031 OAuth code-consumption foundation tests passed");
