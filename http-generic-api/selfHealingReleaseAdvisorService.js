...
async function loadAdvisorEvidence(input, pool) {
  ...
  if (input.release_operation_id) {
    const [rows] = await pool.query(`SELECT * FROM release_operations WHERE operation_id = ? LIMIT 2`, [input.release_operation_id]);
...
export async function getReleaseAdvisorRun(advisorRunId, deps = {}) {
  ...
  const [rows] = await pool.query(`SELECT * FROM release_advisor_runs WHERE advisor_run_id = ? LIMIT 2`, [id]);
...
export async function createReleaseAdvisorRun(rawInput = {}, deps = {}) {
  ...
  const [existingRows] = await pool.query(`SELECT advisor_run_id FROM release_advisor_runs WHERE plan_fingerprint_sha256 = ? LIMIT 2`, [plan.plan_fingerprint_sha256]);
  ...
  try {
    ...
    if (error?.code === "ER_DUP_ENTRY") {
      const [rows] = await pool.query(`SELECT advisor_run_id FROM release_advisor_runs WHERE plan_fingerprint_sha256 = ? LIMIT 2`, [plan.plan_fingerprint_sha256]);
