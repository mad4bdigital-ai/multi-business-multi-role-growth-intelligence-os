...
export function resolveTenantGptOAuthResourceProfile({
  ...
  if (normalizedClientId !== TENANT_GPT_OAUTH_CLIENT_ID) {
    return {
      ...
      message: "OAuth client_id is not allowed for the Tenant GPT client.",
    };
  }
  ...
  if (!resource) {
    return {
      ...
      message: "The OAuth request host is not registered as a Tenant GPT protected resource.",
    };
  }
  ...
  if (explicitResource && (!normalizedExplicitResource || normalizedExplicitResource !== resource)) {
    return {
      ok: false,
      error: "invalid_target",
      message: "The requested OAuth resource does not match the registered Action server resource.",
    };
  }
