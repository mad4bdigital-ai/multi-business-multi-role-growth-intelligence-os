...
export async function decideSequentialPlanApproval({
  pool, holdId, decision, decisionBy = null, decisionNote = null,
}) {
  ...
  return withTransaction(pool, async (connection) => {
    ...
    const [stepRows] = await connection.query(
      "SELECT * FROM execution_plan_steps WHERE plan_step_id = ? LIMIT 1 FOR UPDATE",
      [context.plan_step_id]
    );
    const step = stepRows[0];
    if (!step || step.plan_id !== context.plan_id || step.status !== "awaiting_approval") {
    ...
    await connection.query(
      ...
      [stepStatus, decision === "approved", stepStatus, step.plan_step_id]
    );
    await connection.query("UPDATE execution_plans SET plan_status = ? WHERE plan_id = ?", [planStatus, step.plan_id]);
    await connection.query(
