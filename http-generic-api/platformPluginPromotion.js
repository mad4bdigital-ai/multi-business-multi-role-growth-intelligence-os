...
const VALID_ACTION_CREDENTIAL_SOURCES = new Set(["user_connection", "tenant_connection", "platform_managed", "target_resolved", "none", "mixed"]);
...
async function writeExecutionLog({ pool, traceId, entryType, status, payload }) {
  ...
  await pool.query(
    ...
    [
      sqlDate(now), now, now,
      entryType,
      "platform_plugin_promotion",
      "platformPluginPromotion",
      payload?.plugin_key ? `platform plugin promotion ${payload.plugin_key}` : "platform plugin promotion",
      entryType,
      "platform_plugin_certification_promotion",
      payload?.execution_mode || "promotion_preview",
      "admin_tool",
      status,
...
function inferAuthType(contribution) {
  const protocols = Array.isArray(contribution.protocol_bindings_json) ? contribution.protocol_bindings_json : [];
  const credentialPolicy = contribution.credential_policy_json || {};
  const explicit = normalizeKey(credentialPolicy.auth_type || credentialPolicy.authType || "");
...
function validateContributionForCertification(contribution) {
  ...
  const warnings = [];
  const protocols = Array.isArray(contribution.protocol_bindings_json) ? contribution.protocol_bindings_json : [];
  const actions = Array.isArray(contribution.action_bindings_json) ? contribution.action_bindings_json : [];
  const credentialPolicy = contribution.credential_policy_json || {};
  ...
  if (contribution.target === "tenant_private" || contribution.target === "user_private") {
    warnings.push("private_target_promotes_to_beta_platform_base_only_when_admin_confirms");
  }
  for (const action of actions) {
    ...
    const credentialSource = normalizeKey(action.credential_source || credentialPolicy.default_credential_source || "tenant_connection");
    if (!VALID_ACTION_CREDENTIAL_SOURCES.has(credentialSource)) errors.push(`invalid_credential_source:${action.action_key || "unknown"}`);
  }
  return {
    ok: errors.length === 0,
    errors,
...
export async function certifyPlatformPluginContribution({
  pool = getPool(),
  contributionId,
  adminUserId = null,
  notes = "",
  ...
  const contribution = await loadContribution(pool, normalizedId);
  if (!contribution) {
  ...
  const existingBase = await safeQuery(pool, `SELECT app_key FROM app_integrations WHERE app_key = ? LIMIT 1`, [contribution.plugin_key]);
  const report = validateContributionForCertification(contribution);
  if (existingBase[0]) {
    report.ok = false;
    report.errors.push("plugin_key_already_exists_in_platform_base");
  }
  const certificationStatus = report.ok ? "certified" : "validation_failed";
  const contributionStatus = report.ok ? "certified" : "validation_failed";

  await pool.query(
    `UPDATE platform_plugin_contributions
        SET certification_status = ?,
            status = ?,
            validation_report_json = JSON_SET(COALESCE(validation_report_json, JSON_OBJECT()), '$.certification', CAST(? AS JSON)),
            notes = CASE WHEN ? = '' THEN notes ELSE CONCAT(COALESCE(notes,''), '\n', ?) END,
            updated_by = COALESCE(?, updated_by)
      WHERE contribution_id = ?`,
    [certificationStatus, contributionStatus, JSON.stringify(report), compactString(notes, 1000), compactString(notes, 1000), adminUserId || null, normalizedId]
  );

  const readback = await loadContribution(pool, normalizedId);
  const traceId = `platform_plugin_certify_${normalizedId}_${randomUUID()}`;
  const executionLog = await writeExecutionLog({
    pool,
    traceId,
    entryType: "platform_plugin_contribution_certify",
    ...
    payload: {
      ...
      execution_mode: "certification_preview",
    },
  });
  ...
  return {
    ...
    execution_log: executionLog ? { ok: true, id: executionLog.id, execution_status: executionLog.execution_status, trace_id: executionLog.execution_trace_id_writeback } : { ok: false, trace_id: traceId },
    promoted: false,
...
function normalizeActionBinding(contribution, action) {
  ...
  return {
    ...
    credential_source: VALID_ACTION_CREDENTIAL_SOURCES.has(credentialSource) ? credentialSource : "tenant_connection",
...
export async function promotePlatformPluginContribution({
  pool = getPool(),
  contributionId,
  adminUserId = null,
  status = "beta",
  ...
  if (!PROMOTABLE_STATUSES.has(contribution.status) || contribution.certification_status !== "certified") {
    const err = new Error("Contribution must be certified before promotion.");
    err.code = "contribution_not_certified";
    err.status = 409;
    throw err;
  }
  const existingBase = await safeQuery(pool, `SELECT app_key FROM app_integrations WHERE app_key = ? LIMIT 1`, [contribution.plugin_key]);
  if (existingBase[0]) {
    const err = new Error("Platform Base already contains this plugin_key.");
    err.code = "platform_base_conflict";
    err.status = 409;
    throw err;
  }
  const report = validateContributionForCertification(contribution);
  if (!report.ok) {
    const err = new Error("Contribution no longer passes certification checks.");
    err.code = "certification_recheck_failed";
    err.status = 409;
    err.details = report;
    throw err;
  }
  ...
  await pool.query(
    `INSERT INTO app_integrations
       (app_key, display_name, description, auth_type, oauth_scopes_default,
        mcp_server_info, docs_url, category, default_action_grants, status)
     VALUES (?,?,?,?,?,?,?,?,?,?)`,
    [
      contribution.plugin_key,
      compactString(contribution.display_name, 128),
      compactString(manifest.description || `Promoted from contribution ${normalizedId}`, 4000),
      report.auth_type,
      JSON.stringify((contribution.credential_policy_json || {}).oauth_scopes || []),
      report.auth_type === "mcp" ? JSON.stringify(contribution.protocol_bindings_json || []) : null,
      compactString(manifest.docs_url || manifest.docsUrl || "", 512) || null,
      compactString(contribution.plugin_type || "contribution", 64),
      JSON.stringify(defaultActionGrants),
      baseStatus,
    ]
  );
  ...
  const promotionReport = {
    ...
    app_key: contribution.plugin_key,
    status: baseStatus,
    action_bindings_created: actions.filter((action) => action.action_key).length,
    secrets_included: false,
  };
  await pool.query(
    `UPDATE platform_plugin_contributions
        SET status = 'certified',
            validation_report_json = JSON_SET(COALESCE(validation_report_json, JSON_OBJECT()), '$.promotion', CAST(? AS JSON)),
            notes = CASE WHEN ? = '' THEN notes ELSE CONCAT(COALESCE(notes,''), '\n', ?) END,
            updated_by = COALESCE(?, updated_by)
      WHERE contribution_id = ?`,
    [JSON.stringify(promotionReport), compactString(notes, 1000), compactString(notes, 1000), adminUserId || null, normalizedId]
  );

  const readbackBase = await safeQuery(pool, `SELECT app_key, display_name, auth_type, category, status FROM app_integrations WHERE app_key = ? LIMIT 1`, [contribution.plugin_key]);
  const readbackBindings = await safeQuery(pool, `SELECT action_key, binding_role, credential_source, exposure_default, status FROM app_integration_action_bindings WHERE app_key = ? ORDER BY action_key ASC`, [contribution.plugin_key]);
  const traceId = `platform_plugin_promote_${normalizedId}_${randomUUID()}`;
  const executionLog = await writeExecutionLog({
    pool,
    traceId,
    entryType: "platform_plugin_contribution_promote",
    status: readbackBase[0] ? "success" : "failed",
    payload: {
      contribution_id: normalizedId,
      plugin_key: contribution.plugin_key,
      platform_base_mutated: Boolean(readbackBase[0]),
      action_bindings_created: readbackBindings.length,
      execution_mode: "admin_platform_base_promotion",
    },
  });

  return {
    ok: Boolean(readbackBase[0]),
    promoted: Boolean(readbackBase[0]),
    platform_base_mutated: Boolean(readbackBase[0]),
    app_integration: readbackBase[0] || null,
    action_bindings: readbackBindings,
    contribution: await loadContribution(pool, normalizedId),
    execution_log: executionLog ? { ok: true, id: executionLog.id, execution_status: executionLog.execution_status, trace_id: executionLog.execution_trace_id_writeback } : { ok: false, trace_id: traceId },
    secrets_included: false,
  };
}
