...
const MAX_DYNAMIC_EVIDENCE_KEYS = 32;
...
function requireMutationOperationContract(operationContract) {
  ...
  const dynamicEvidenceKeys = [...new Set(operationContract.dynamicEvidenceKeys
    .map((entry, index) => cleanToken(entry, `operationContract.dynamicEvidenceKeys[${index}]`)))]
    .sort();
  if (
    dynamicEvidenceKeys.length === 0 ||
    dynamicEvidenceKeys.length > MAX_DYNAMIC_EVIDENCE_KEYS
  ) {
    throw new TypeError(
      `operationContract.dynamicEvidenceKeys must contain 1-${MAX_DYNAMIC_EVIDENCE_KEYS} unique keys.`,
    );
  }
  return deepFreeze({
...
function requireDynamicEvidence(value, operationContract, capsule) {
  ...
  const missing = operationContract.dynamicEvidenceKeys.filter((key) => !byKey.has(key));
  const unexpected = [...byKey.keys()].filter(
    (key) => !operationContract.dynamicEvidenceKeys.includes(key),
  );
  ...
  return {
    ...
    now: value.now,
    selectedTarget,
    refreshedEvidence: deepFreeze(operationContract.dynamicEvidenceKeys.map((key) => byKey.get(key))),
  };
}
  ...
  return Object.freeze({
    ...
    async execute({
      ...
      try {
        ...
        const governance = requireGovernanceDecision(
          governanceDecision,
          normalizedOperation,
          canonicalCapsule,
        );
        ...
        await emitSafely(emitTelemetry, {
          eventType: "execution_capsule_mutation_validation",
          operationKey: normalizedOperation?.operationKey || safeTelemetryToken(operationContract?.operationKey),
          validationStatus: validation?.status || null,
          governanceStatus: null,
          approvalStatus: null,
          dynamicRefreshComplete: validation?.status === ExecutionCapsuleValidationStatus.VALID,
