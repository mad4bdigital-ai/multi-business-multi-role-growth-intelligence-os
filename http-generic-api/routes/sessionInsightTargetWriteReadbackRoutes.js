...
function errorResponse(res, err, fallbackCode) {
  return res.status(err.status || 500).json({ ok: false, error: { code: err.code || fallbackCode, message: String(err.message || "Session insight target write readback failed.").slice(0, 300), details: err.details || null }, secrets_included: false });
}
