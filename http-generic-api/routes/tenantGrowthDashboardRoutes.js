...
function requireTenantPrincipal(req, res, next) {
  if (req.auth?.mode !== "user_jwt" || req.auth?.is_admin === true || !req.auth?.tenant_id || !req.auth?.user_id) {
    return res.status(401).json({ ok: false, error: { code: "tenant_user_jwt_required", message: "A signed tenant user JWT with tenant_id and user_id is required." }, secrets_included: false });
  }
  return next();
}
...
function requireMatchingTenantParam(req, res, next) {
  ...
  if (requestedTenantId && requestedTenantId !== req.auth?.tenant_id) {
    return res.status(403).json({ ok: false, error: { code: "tenant_dashboard_scope_mismatch", message: "The requested dashboard scope is outside the signed-in tenant." }, secrets_included: false });
  }
  return next();
}
...
function errorResponse(res, error, fallback = "tenant_growth_dashboard_failed") {
  return res.status(error?.status || 500).json({ ok: false, error: { code: error?.code || fallback, message: error?.message || String(error || fallback) }, secrets_included: false });
}
...
async function preferencesReadResponse(req, res) {
  try {
    return res.status(200).json({ ...(await readTenantDashboardPreferences({ tenantId: req.auth.tenant_id, userId: req.auth.user_id })), secrets_included: false });
  } catch (error) {
...
async function preferencesWriteResponse(req, res) {
  try {
    return res.status(200).json(await upsertTenantDashboardPreferences({ tenantId: req.auth.tenant_id, userId: req.auth.user_id, preferences: req.body || {} }));
  } catch (error) {
...
async function digestResponse(req, res) {
  try {
    return res.status(200).json(await buildTenantDashboardDigest({ tenantId: req.auth.tenant_id, userId: req.auth.user_id, containerKey: req.query.container_key || null }));
  } catch (error) {
...
async function actionPreviewResponse(req, res) {
  try {
    return res.status(200).json(await buildTenantDashboardActionPreview({ tenantId: req.auth.tenant_id, userId: req.auth.user_id, actionRefKey: req.params.actionRefKey, containerKey: req.query.container_key || null }));
  } catch (error) {
...
export const _testingTenantGrowthDashboardRoutes = { requireMatchingTenantParam, dashboardArgs };
