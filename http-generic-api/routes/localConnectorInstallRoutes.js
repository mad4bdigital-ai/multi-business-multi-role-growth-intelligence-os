...
function buildInstallPowerShell({ cfToken, connectorSecret, tunnelUrl, aliases, port }) {
  ...
  return [
    ...
    "# This script is self-contained: it installs Node.js, cloudflared, downloads the agent, and starts it.",
    ...
    "Invoke-WebRequest -Uri 'https://auth.mad4b.com/connector-agent/server.mjs' -OutFile (Join-Path $Root 'server.mjs') -UseBasicParsing",
    "",
    "# 3. Write .env",
    `$EnvText = @'`,
    envText,
    ...
    "}",
    "",
    "# 6. Register tunnel (uninstall old service first if present)",
    "try { cloudflared service uninstall 2>$null } catch {}",
    `cloudflared service install ${cfToken}`,
    "",
    "# 7. Start connector agent",
