...
function issueTenantGptAccessToken(payload, {
  ...
  if (!resourceProfile?.ok || !resourceProfile?.audience || !resourceProfile?.resource) {
    const err = new Error("Cannot issue tenant GPT token without a valid protected-resource profile.");
      ...
      const code = jwt.sign(
        {
          ...
          resource_profile_key: resourceProfile.profile_key,
          activation_context,
        },
        JWT_SECRET,
  ...
  router.post("/oauth/token", express.urlencoded({ extended: false }), async (req, res) => {
    ...
    try {
      ...
      const accessToken = issueTenantGptAccessToken(
        ...
        { clientId: clientValidation.client_id, jwtid: accessJti, compact: true, resourceProfile }
      );
