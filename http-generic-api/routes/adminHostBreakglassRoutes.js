...
export function buildAdminHostBreakglassRoutes({ requireBackendApiKey, requireAdminPrincipal, broker = {} } = {}) {
  ...
  router.post("/admin/runtime-bootstrap/plan", (req, res) => { try { return res.status(200).json({ ok: true, ...buildHostBreakglassPlan(req.body || {}, planDependencies), secrets_included: false }); } catch (error) { return errorResponse(res, error); } });
  router.post("/admin/runtime-bootstrap/runs", async (req, res) => { try { const plan = buildHostBreakglassPlan(req.body || {}, planDependencies); if (plan.action === "plan") return res.status(200).json({ ok: true, ...plan }); return res.status(202).json(await dispatchHostBreakglassPlan(plan, broker)); } catch (error) { return errorResponse(res, error); } });
