...
function attachVerifiedStagingLocalRequest(plan, receipt) {
  if (
  ...
  const verifiedRequest = buildVerifiedHostBreakglassLocalRequest(plan);
...
export function buildAdminHostBreakglassRoutes({ requireBackendApiKey, requireAdminPrincipal, broker = {} } = {}) {
  ...
  router.post("/admin/runtime-bootstrap/runs", async (req, res) => {
    try {
      ...
      return res.status(202).json(attachVerifiedStagingLocalRequest(plan, receipt));
    } catch (error) {
...
export const __adminHostBreakglassRoutesTest = Object.freeze({ isRoleSelectiveProductionApply, attachVerifiedStagingLocalRequest });
