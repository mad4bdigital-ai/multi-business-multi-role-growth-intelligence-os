...
export function buildPlatformGraphRoutes({ requireBackendApiKey, requireAdminPrincipal }) {
  ...
  router.post("/platform/graph/memory", ...requireAdmin, async (req, res) => {
    try {
      ...
      await logGraphQuery({ queryType: "memory", input, result: { node_count: graphContext.node_count, edge_count: graphContext.edge_count, memory_asset_count: memory.asset_count, validation_state: graphContext.validation_state } });
