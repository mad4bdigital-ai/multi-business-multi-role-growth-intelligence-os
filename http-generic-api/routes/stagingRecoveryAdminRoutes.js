...
export function buildStagingRecoveryAdminContract() {
  return {
    ...
    operation_policy: {
      ...
      bootstrap_ticket_authority_modes: ["verify_existing_ticket", "verify_role_bundle_binding", "mark_executing", "prepare_access_repair", "approve_access_repair"],
      approval_to_ticket_flow: "recovery_system_tool_plan_challenge_typed_approval_server_issued_ticket",
      caller_selected_operation_on_issuance: false,
      bootstrap_ticket_reservation: "server_managed_single_use_signed_receipt",
      bootstrap_execution_start: "server_signed_receipt_required_before_local_mutation",
...
export async function buildStagingRecoveryAdminReadiness({ recoveryComposition = null, stagingCertificationReader = null, deploymentAttestationReader = null, targetFingerprintReader = null, recoveryReadinessEvidenceReader = null, ingressBuildIdentity = null, expectedSha = null, expectedTargetFingerprint = null } = {}) {
  ...
  return {
    ...
    live_certification: { status: "not_run_by_readiness_surface", separate_workflow_required: true, consequential_provider_execution_performed: false },
    database_mutation_performed: false,
    provider_accessed: false,
    provider_mutation_performed: false,
...
export function buildStagingRecoveryAdminRoutes({
  ...
  router.get("/admin/recovery/staging/readiness", ...guards, async (req, res) => {
    try {
      const result = await buildStagingRecoveryAdminReadiness({ recoveryComposition, stagingCertificationReader, deploymentAttestationReader, targetFingerprintReader, recoveryReadinessEvidenceReader, ingressBuildIdentity: req.activationHostGateway?.ingress_build_identity });
  ...
  router.post("/admin/recovery/staging/bootstrap-ticket/verify", ...guards, async (req, res) => {
    try {
      ...
      if (authorityAction === "prepare_access_repair") {
        const authority = stagingAccessRepairTicketAuthorityFactory({ env });
        const result = await authority.prepare(exactAccessRepairPrepare(req.body || {}));
        return res.status(201).json({ ...result, environment: "staging", production_authority: false, secrets_included: false });
      }
      if (authorityAction === "approve_access_repair") {
        const authority = stagingAccessRepairTicketAuthorityFactory({ env });
      ...
      if (authorityAction === "mark_executing") {
        ...
        return res.status(result.executing === true ? 200 : 409).json({ ok: result.executing === true, ...result, environment: "staging", production_authority: false, database_mutation_performed: false, secrets_included: false });
      }
  ...
  router.post("/admin/recovery/staging/bootstrap-ticket/finalize", ...guards, async (req, res) => {
    try {
      ...
      const executionReceipt = input.execution_receipt;
      const readbackEvidence = input.readback_evidence;
      delete input.readback_receipt;
      delete input.reservation_receipt;
      delete input.execution_receipt;
      delete input.readback_evidence;
      const binding = exactBootstrapBinding(input);
      const authority = stagingBootstrapExecutionAuthorityFactory({ env });
      if (!readbackReceipt) {
        if (!isObject(reservationReceipt) || !isObject(executionReceipt) || !isObject(readbackEvidence) || hasSensitiveReceiptKey(reservationReceipt) || hasSensitiveReceiptKey(executionReceipt) || hasSensitiveReceiptKey(readbackEvidence)) return errorResponse(res, req, 409, "RECOVERY_READBACK_UNVERIFIED", "Finalization requires signed reservation and execution-start receipts plus canonical, non-sensitive same-cycle readback evidence.");
        const attested = await authority.attestReadbackForBootstrap({ ...binding, reservation_receipt: reservationReceipt, execution_receipt: executionReceipt, evidence: readbackEvidence });
