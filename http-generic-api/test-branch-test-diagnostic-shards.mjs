...
try {
  ...
  assert.equal(completeMatrixResult.status, 0, completeMatrixResult.stderr || completeMatrixResult.stdout);
  const completeMatrix = JSON.parse(completeMatrixResult.stdout);
  assert.ok(completeMatrix.family_count > 5);
  assert.equal(completeMatrix.command_count, testCommands.length);
  assert.ok(completeMatrix.include.some((row) => row.family === "growth-control"));
  assert.ok(completeMatrix.include.some((row) => row.family === "context-kernel"));
  assert.equal(completeMatrix.include.every((row) => row.command_count >= 1), true);
  assert.equal(completeMatrix.include.every((row) => row.family_slug.length > 0), true);
  assert.ok(completeMatrix.include.length <= 64);

  const growthMatrixResult = run([
    "--emit-matrix",
  ...
  assert.equal(growthMatrixResult.status, 0, growthMatrixResult.stderr || growthMatrixResult.stdout);
  const growthMatrix = JSON.parse(growthMatrixResult.stdout);
  assert.equal(growthMatrix.family_count, 1);
  assert.equal(growthMatrix.include.every((row) => row.family === "growth-control"), true);
  assert.equal(growthMatrix.include.every((row) => row.command_count <= 2), true);
  ...
  assert.equal(report.contract, "mad4b.test-diagnostic-shard-report.v2");
  ...
  assert.equal(report.family, "growth-control");
  ...
  assert.equal(report.rerun.family, "growth-control");
  assert.equal(report.rerun.parentIndex, 2);
  assert.equal(report.rerun.parentCount, 4);
  assert.equal(report.commands.every((item) => item.status === "listed"), true);
  assert.equal(report.commands.every((item) => item.family === "growth-control"), true);
  ...
  assert.deepEqual(
    sameReport.commands.map((item) => [item.commandIndex, item.command, item.family]),
    report.commands.map((item) => [item.commandIndex, item.command, item.family]),
  );
  assert.equal(sameReport.catalogSha256, report.catalogSha256);

  const invalidParentCoordinates = run([
    "--parent-index", "2",
    "--parent-count", "2",
    "--shard-index", "0",
    "--shard-count", "4",
  ...
  assert.equal(invalidParentCoordinates.status, 1);
  assert.match(invalidParentCoordinates.stderr, /parent coordinates are out of range/);

  const invalidShardCoordinates = run([
    "--parent-index", "0",
    "--parent-count", "1",
    "--shard-index", "4",
    "--shard-count", "4",
  ...
  assert.equal(invalidShardCoordinates.status, 1);
  assert.match(invalidShardCoordinates.stderr, /shard coordinates are out of range/);
  ...
  assert.match(missingFamily.stderr, /No test commands matched/);
} finally {
  rmSync(temporaryDirectory, { recursive: true, force: true });
}

console.log("branch-agnostic family diagnostic shard automation tests passed");
