...
const directGapQueries = [];
const directGapPool = {
  async query(sql, params = []) {
    directGapQueries.push({ sql, params });
    if (sql === "SELECT DATABASE() AS database_name") return [[{ database_name: "direct-gap-database" }]];
    ...
    if (/INFORMATION_SCHEMA\.COLUMNS/u.test(sql)) {
      return [params.slice(1).map((table, index) => ({
        TABLE_NAME: table,
        COLUMN_NAME: "id",
        ORDINAL_POSITION: index + 1,
        COLUMN_TYPE: "int",
    ...
    if (/INFORMATION_SCHEMA\.STATISTICS/u.test(sql)) {
      return [params.slice(1).map((table) => ({
        TABLE_NAME: table,
        INDEX_NAME: "PRIMARY",
        COLUMN_NAME: "id",
        SEQ_IN_INDEX: 1,
    ...
    if (/FROM governed_migration_ledger/u.test(sql)) {
      ...
      const source = fixtureMigrations.get(migration);
      return [[{
        run_id: `direct-${migration}`,
        ...
        preflight_risk_count: 0,
        secrets_included: 0,
        capability_envelope_id: "direct-envelope-must-not-be-returned",
      }]];
    }
...
const directGapResult = await collectRetailCommerceProductionSchemaBaseline({
  pool: directGapPool,
  readFile: fixtureReadFile,
  now: () => new Date("2026-08-03T12:20:00.000Z"),
});

assert.equal(directGapResult.authoritative_database_connection_succeeded, true);
assert.equal(directGapResult.baseline_collected, true);
assert.equal(directGapResult.parity_status, "gaps_detected");
assert.equal(directGapResult.gaps.some((entry) => entry.code === "missing_expected_tables"), false);
assert(directGapResult.gaps.some((entry) => entry.code === "missing_expected_columns"));
assert(directGapResult.gaps.some((entry) => entry.code === "missing_expected_indexes"));
assert(directGapResult.migrations.every((entry) => entry.checks.all_expected_tables_present === true));
assert(directGapResult.migrations.every((entry) => entry.checks.all_expected_columns_present === false));
assert(directGapResult.migrations.every((entry) => entry.checks.all_expected_indexes_present === false));
assert.equal(JSON.stringify(directGapResult).includes("direct-envelope-must-not-be-returned"), false);
assert(directGapQueries.every(({ sql }) => /^SELECT\b/iu.test(sql.trim())));
assert(directGapQueries.every(({ sql }) => !/SELECT\s+\*/iu.test(sql)));
assert(directGapQueries.every(({ sql }) => !/\b(INSERT|UPDATE|DELETE|ALTER|DROP|CREATE|REPLACE|TRUNCATE|CALL|GRANT|REVOKE|LOCK|UNLOCK|SET)\b/iu.test(sql)));
...
console.log(JSON.stringify({
  ...
  direct_column_and_index_gaps_detected: true,
  runtime_tool_calls: realMigrationRequests.length,
  direct_query_count: directGapQueries.length,
  row_data_read: false,
