...
function boundedTimeout(value) { const n = Number(value); if (!Number.isFinite(n) || n <= 0) return DEFAULT_TIMEOUT_MS; return Math.min(Math.max(Math.floor(n), 1000), MAX_TIMEOUT_MS); }
function safeJson(value, fallback = {}) { if (value && typeof value === "object") return value; if (!value) return fallback; try { return JSON.parse(value); } catch { return fallback; } }
...
async function upsertInspectionRun(pool, input, status) {
  await pool.query(
    `INSERT INTO \`browser_site_inspection_runs\`
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE binding_key = VALUES(binding_key), tenant_id = VALUES(tenant_id), user_id = VALUES(user_id), target_url = VALUES(target_url), checks_json = VALUES(checks_json), policy_json = VALUES(policy_json), status = VALUES(status), updated_at = CURRENT_TIMESTAMP`,
...
async function updateInspectionResult(pool, inspectionKey, status, result, error = null) {
  await pool.query(`UPDATE \`browser_site_inspection_runs\` SET status = ?, result_json = ?, error_json = ?, updated_at = CURRENT_TIMESTAMP WHERE inspection_key = ?`, [status, result ? JSON.stringify(result) : null, error ? JSON.stringify(error) : null, inspectionKey]);
}
async function recordBrowserRuntimeEvent(pool, event) {
  await pool.query(
    `INSERT INTO \`browser_runtime_events\` (event_id, session_id, runtime_key, binding_key, tenant_id, user_id, event_type, url_host, actor, policy_result, event_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
function internalHeaders() { return { "Content-Type": "application/json", ...(process.env.BACKEND_API_KEY ? { Authorization: `Bearer ${process.env.BACKEND_API_KEY}` } : {}) }; }
function connectorEndpoint(deviceId, baseUrl) { const base = String(baseUrl || process.env.INTERNAL_BASE_URL || `http://localhost:${process.env.PORT || 8080}`).replace(/\/$/, ""); return `${base}/connector/${encodeURIComponent(deviceId)}/browser4`; }
function sanitizeConnectorResult(data) {
  ...
  return { ok: data.ok !== false, action: data.action || null, run_key: data.run_key || null, target_host: data.target_host || null, checks: Array.isArray(data.checks) ? data.checks : [], exit_code: data.exit_code ?? null, status: data.status || null, status_json: safeJson(data.status_json, null), artifacts: safeJson(data.artifacts, {}), stdout_preview: typeof data.stdout_preview === "string" ? data.stdout_preview.slice(0, 2000) : "", stderr_preview: typeof data.stderr_preview === "string" ? data.stderr_preview.slice(0, 2000) : "", connector_route: data.connector_route || null, secrets_included: false };
}
...
export async function runBrowser4InspectionAdapter({ pool = getPool(), fetchImpl = fetch, input = {}, internalBaseUrl = null } = {}) {
  ...
  if (!bindingKey || !targetUrl) { const err = new Error("binding_key and url are required."); err.status = 400; err.code = "browser4_inspection_missing_fields"; throw err; }
  ...
  if (runtime.provider !== "browser4") { const err = new Error("The selected binding does not point to a Browser4 runtime."); err.status = 409; err.code = "browser4_runtime_required"; err.details = { runtime_key: runtime.runtime_key, provider: runtime.provider }; throw err; }
  if (!runtime.device_id) { const err = new Error("Browser4 runtime is missing device_id."); err.status = 409; err.code = "browser4_device_required"; throw err; }
  const policy = await checkBrowserRuntimePolicyFromDb({ pool, binding_key: bindingKey, input: { ...input, target_url: targetUrl, action: "inspect_site", use_case: input.use_case || "site_diagnostics" } });
  if (!policy.ok) {
  ...
  const response = await fetchImpl(connectorEndpoint(runtime.device_id, internalBaseUrl), { method: "POST", headers: internalHeaders(), signal: AbortSignal.timeout(boundedTimeout(input.timeout_ms || input.timeoutMs)), body: JSON.stringify({ action: "inspect_site", url: targetUrl, checks, inspection_key: inspectionKey, timeout_ms: boundedTimeout(input.timeout_ms || input.timeoutMs) }) });
