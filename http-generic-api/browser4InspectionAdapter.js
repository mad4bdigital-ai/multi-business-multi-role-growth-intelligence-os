...
async function loadInspectionRun(pool, inspectionKey) {
  const [rows] = await pool.query(
    "SELECT * FROM `browser_site_inspection_runs` WHERE inspection_key = ? LIMIT 1",
    [inspectionKey],
  );
  return rows[0] || null;
}
...
export async function runBrowser4InspectionAdapter({
  ...
  const policy = await checkBrowserRuntimePolicyFromDb({
    ...
    input: {
      ...
      action: "inspect_site",
      use_case: input.use_case || "site_diagnostics",
    },
  });
  ...
  const endpoint = connectorEndpoint(runtime.device_id, internalBaseUrl);
  const response = await fetchImpl(endpoint, {
    ...
    body: JSON.stringify({
      ...
      checks,
      inspection_key: inspectionKey,
      timeout_ms: boundedTimeout(input.timeout_ms || input.timeoutMs),
    }),
  });
