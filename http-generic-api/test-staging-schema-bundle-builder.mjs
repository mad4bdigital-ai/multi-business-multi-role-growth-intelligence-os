...
test("baseline actions schema covers the full migration column contract", () => {
  ...
  const requiredColumns = [
    ...
    "allowed_actor_roles", "allowed_governance_levels", "review_required", "provider_agnostic", "request_envelope_required",
    "structured_api_supported", "writeback_scope", "secret_store_ref", "openai_schema_file_name", "openai_schema_ref",
    "openai_schema_storage_surface", "oauth_config_ref", "oauth_client_id_ref", "oauth_client_secret_ref",
    "oauth_secret_storage_type", "oauth_binding_status", "oauth_last_validated_at", "runtime_binding_profile", "schema_json",
    "import_job_id", "schema_imported_at",
  ];
...
test("generator plan-only mode inventories the exact migration chain", () => {
  ...
  assert.equal(plan.ordered_text_width_chain.bounded_text_columns, 5239);
  ...
  assert.equal(plan.ordered_index_key_width_chain.tables_projected, 585);
  assert.equal(plan.ordered_index_key_width_chain.indexes_checked, 2855);
  assert.equal(plan.ordered_index_key_width_chain.index_columns_checked, 4845);
  ...
  assert.equal(plan.ordered_required_insert_column_chain.tables_projected, 588);
  ...
  assert.equal(plan.ordered_foreign_key_compatibility_chain.tables_projected, 586);
