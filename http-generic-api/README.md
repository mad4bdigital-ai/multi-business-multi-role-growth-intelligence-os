...
- `modelAdapterRouter.js` — `buildCallModel`: normalizes Anthropic / OpenAI / OpenRouter / Gemini / local Ollama shapes
- `POST /connector/{device_id}/agent-runtime` — proxies capability discovery, model recommendation, safe persisted settings, approved Ollama installation, and explicitly approved local multi-agent jobs
