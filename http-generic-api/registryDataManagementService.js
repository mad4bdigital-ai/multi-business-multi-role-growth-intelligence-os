...
function quoteIdentifier(identifier) {
  ...
  if (!IDENTIFIER_RE.test(value)) {
    throw new DataManagementError("invalid_registry_identifier", "Registry contains an unsafe SQL identifier.", 500, { identifier: value });
  }
  return `\`${value}\``;
}
...
export function validateRegisteredColumns(registry) {
  ...
  for (const column of groups.flat()) {
    ...
    if (containsSecretLikeColumn(column)) {
      throw new DataManagementError("secret_like_column_not_allowed", "Registry-managed data tables must not expose secret-like columns.", 500, { table_key: registry.table_key, column });
    }
  }
}
