...
function authorityGraph(env, authorityGraph, adapters) {
...
async function attestExactDeployment(graph, expectedSha, targetFingerprint = null) {
  ...
  if (attestation.environment !== "staging" || attestation.branch !== "main" || deploymentSha !== expectedSha || !SHA256.test(target)) {
    fail(deploymentSha !== expectedSha ? "STAGING_SHA_MISMATCH" : "RECOVERY_TICKET_BINDING_MISMATCH", "Staging rebuild-empty authority is not bound to the exact main deployment and target.", { deployment_sha: deploymentSha || null }, 412);
  }
  if (targetFingerprint && target !== targetFingerprint) fail("RECOVERY_TICKET_BINDING_MISMATCH", "Staging target changed after the durable inspection.", { target_match: false }, 412);
  ...
  return { ...attestation, sha: deploymentSha, target_fingerprint: target, recovery_manifest_hash: manifestHash, attestation_hash: attestationHash };
}
...
function normalizeBundleBindings(value, selectedRoles) {
  ...
  for (const role of selectedRoles) {
    ...
    if (!binding || typeof binding !== "object" || Array.isArray(binding)) fail("RECOVERY_ROLE_BUNDLE_BINDING_MISSING", "Every selected empty role requires an exact canonical schema-bundle binding before approval.", { role }, 409);
...
function normalizeInspection(input, expectedSha, targetFingerprint) {
  ...
  const normalized = {
    contract: "mad4b.staging-durable-full-inspection.v1",
...
function buildPlan({ expectedSha, idempotencyKey, run, proof, attestation }) {
  ...
  const planIdentity = { expected_sha: expectedSha, inspection_run_id: run.run_id, inspection_evidence_hash: run.inspection_evidence_hash, role_selection_hash: proof.selection_hash, target_fingerprint: attestation.target_fingerprint, step_hash: stepHash, nonce: randomUUID() };
  ...
  const base = {
    ...
    target_key: "staging-runtime",
    target_fingerprint: attestation.target_fingerprint,
    target_fingerprint_at_creation: attestation.target_fingerprint,
    target_fingerprints: { composite: attestation.target_fingerprint },
...
export function createStagingRebuildEmptyAuthority({ env = process.env, adapters = null, authorityGraph: injectedGraph = null } = {}) {
  ...
  if (!store?.putRun || !store?.getRun || !store?.putFinding || !store?.putPlan || !store?.getPlan || !store?.getApprovalByPlanStep || !store?.putExecutionTicket || !store?.reserveApproval || !store?.releaseApprovalReservation || !store?.markApprovalUsed || !graph.approvalIssuer?.createChallenge || !graph.approvalVerifier?.verify || !graph.executionTicketSigner?.sign) {
  ...
  const resolveProof = async (input = {}) => {
    ...
    await attestExactDeployment(graph, expectedSha, run.target_fingerprint);
  ...
  return Object.freeze({
    ...
    async recordInspection(input = {}) {
      ...
      const inspection = normalizeInspection(input, expectedSha, attestation.target_fingerprint);
      const inspectionEvidenceHash = digest(inspection);
      const runId = `run:${digest({ expected_sha: expectedSha, target_key: targetKey, target_fingerprint: attestation.target_fingerprint, inspection_evidence_hash: inspectionEvidenceHash, correlation_id: correlationId }).slice(0, 32)}`;
      ...
      const run = {
        contract: "mad4b.staging-durable-full-inspection-run.v1",
        run_id: runId,
        status: "classified",
        phase: "classified",
        expected_sha: expectedSha,
    ...
    async prepare(input = {}) {
      ...
      const proof = proofFromRun(run);
      const attestation = await attestExactDeployment(graph, expectedSha, run.target_fingerprint);
    ...
    async approveAndIssue(input = {}) {
      ...
      if (plan.execution_ticket_id || plan.status === "approved") fail("RECOVERY_APPROVAL_INVALID", "This rebuild-empty approval already issued its single-use ticket.", {}, 409);
      ...
      if (reservation?.reserved !== true) fail("RECOVERY_APPROVAL_INVALID", "The rebuild-empty approval challenge is already reserved or consumed.", { reconciliation_required: true }, 409);
      ...
      try {
        ...
        await store.putPlan({ ...plan, status: "approved", execution_ticket_id: ticket.ticket_id, execution_ticket_hash: ticket.ticket_hash, approved_at: new Date().toISOString() });
...
export const _testingStagingRebuildEmptyAuthority = Object.freeze({ normalizeInspection, proofFromRun, buildPlan, approvalConfirmation, digest });
