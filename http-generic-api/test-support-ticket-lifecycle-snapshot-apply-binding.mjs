...
const migration = readFileSync("migrations/904_sprint68_support_ticket_lifecycle_snapshot_apply_binding.sql", "utf8");
...
assert(migration.includes("'admin_platform_tool', 'state_changing', 'none'"), "tool binding must be state-changing but no-credential");
...
assert(releaseReadiness.includes("904_sprint68_support_ticket_lifecycle_snapshot_apply_binding.sql"), "release readiness must track migration 904");
...
assert(runner.includes("904_sprint68_support_ticket_lifecycle_snapshot_apply_binding.sql"), "governed migration runner must allowlist migration 904");
...
for (const forbidden of ["runtime_endpoint_call", "callTool.name", "/system/tools/call", "credential_payload_read_allowed", "provider_dispatch_enabled_changed = 1", "external_send_performed = 1"]) {
