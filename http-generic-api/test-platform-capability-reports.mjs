    ...
    async query(sql, params = []) {
      ...
      if (normalized.includes("FROM admin_platform_endpoint_tools")) {
        return [[
          ...
          { tool_key: "platform_capability_assurance_reconcile" },
        ]];
      }
      if (normalized === "SELECT UTC_TIMESTAMP() AS evaluated_at") {
        return [[{ evaluated_at: "2026-06-15 00:00:00" }]];
      }
      ...
      if (normalized.includes("FROM v_platform_capability_readiness_vector") && normalized.includes("COUNT(*) AS capability_count")) {
        return [[{
          capability_count: 607,
          authority_required_count: 31,
          dispatch_allowed_count: 550,
          apply_allowed_count: 14,
          certified_count: 45,
          provenance_ready_count: 607,
          resource_binding_ready_count: 590,
          hard_blocked_count: 17,
        }]];
      }
      ...
      if (normalized.includes("FROM `v_platform_capability_assurance_gaps`") && normalized.includes("GROUP BY")) {
        return [[
      ...
      if (normalized.includes("FROM `platform_capability_certifications`")) {
        return [[{ certification_status: "certified", count_rows: 45 }]];
      }
      if (normalized.includes("FROM `platform_capability_source_links`")) {
        return [[{ source_kind: "mysql_registry", resolution_status: "resolved", count_rows: 607 }]];
      }
      if (normalized.includes("FROM `platform_capability_debt`")) {
        return [[{ status: "open", severity: "high", count_rows: 7 }]];
      }
      if (normalized.includes("FROM v_platform_capability_assurance_gaps") && normalized.includes("LIMIT ?")) {
        return [[{
          capability_key: "resource_authority_route_family.github_write",
          gap_key: "resource_binding_missing",
          gap_severity: "high",
          gap_description: "missing capability-specific effective resource binding",
        }]];
      }
...
assert.equal(contract.checks.find((item) => item.contract_key === "capability_provenance_contract")?.status, "implemented");
...
assert.equal(live.source_resolution_distribution[0].source_kind, "mysql_registry");
