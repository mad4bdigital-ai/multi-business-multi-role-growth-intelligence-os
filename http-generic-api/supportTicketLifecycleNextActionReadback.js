...
function normalizeRowJson(row = {}, jsonFields = []) {
  const out = { ...row };
  for (const field of jsonFields) if (Object.prototype.hasOwnProperty.call(out, field)) out[field] = parseJson(out[field], null);
  return out;
}

function buildGuidance({ ticket, snapshot, recommendation }) {
  const decision = recommendation?.decision_json || {};
  const stateClassification = decision.state_classification || snapshot?.state_classification || ticket?.lifecycle_state || "unknown";
  const recommendedNextAction = decision.recommended_next_action || "review_support_ticket_state";
  const guidance = {
    state_classification: stateClassification,
    recommended_next_action: recommendedNextAction,
    admin_next_action: recommendedNextAction,
    customer_safe_summary: "Support Ticket lifecycle evidence has been reviewed. No customer-facing execution was performed.",
    customer_safe_next_step: "Continue only after the operator confirms the recommendation.",
    requires_operator_review: true,
    requires_runtime_link_review: recommendedNextAction === "evaluate_whether_runtime_or_approval_link_is_required",
    requires_approval_review: recommendedNextAction === "request_or_decide_approval_hold",
  };
  if (stateClassification === "ticket_ready_for_customer_update") {
...
export async function readSupportTicketLifecycleNextAction(input = {}) {
  ...
  const [[ticketRows], [snapshotRows], [recommendationRows]] = await Promise.all([
    pool.query(`SELECT ticket_id, tenant_id, user_id, title, category, priority, severity, status, lifecycle_state, customer_status, queue_key, assignment_status, service_mode, metadata_json, sla_status, created_at, updated_at FROM tickets WHERE ticket_id = ? LIMIT 1`, [ticketId]),
    pool.query(`SELECT snapshot_id, snapshot_key, plugin_key, scope_type, scope_id, tenant_id, subject_key, state_classification, maturity_score, input_sources_json, state_json, maturity_json, blockers_json, safety_json, status, secrets_included, created_at, updated_at FROM platform_orchestration_state_snapshots WHERE plugin_key = ? AND scope_type = 'ticket' AND scope_id = ? ORDER BY created_at DESC LIMIT 1`, [pluginKey, ticketId]),
    pool.query(`SELECT recommendation_id, recommendation_key, snapshot_id, plugin_key, scope_type, scope_id, task_class, recommendation_type, priority, recommendation_status, decision_json, blockers_json, next_actions_json, safety_contract_json, secrets_included, created_at, updated_at FROM platform_orchestration_recommendations WHERE plugin_key = ? AND scope_type = 'ticket' AND scope_id = ? ORDER BY created_at DESC LIMIT 1`, [pluginKey, ticketId]),
  ]);
  ...
  return {
    ...
    snapshot,
    recommendation,
    guidance: buildGuidance({ ticket, snapshot, recommendation }),
    execution: {
