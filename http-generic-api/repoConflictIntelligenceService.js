...
function classifyFiles(files = [], policies = DEFAULT_REPO_CONFLICT_PATH_POLICIES) {
  return files.map((file) => {
    ...
    return sanitize({
      path,
      status: safeString(file.status || (conflict ? "conflicting" : "modified"), 64),
      conflict,
      generated,
      path_class: policy.path_class,
      strategy: policy.default_strategy,
      risk: policy.risk,
      auto_resolve: Boolean(policy.auto_resolve),
      requires_test: Boolean(policy.requires_test),
...
export function analyzeRepoConflict(input = {}) {
  ...
  return sanitize({
    ...
    classification,
    safe_to_auto_resolve: safeToAutoResolve,
    recommended_path: safeToAutoResolve ? (generatedFiles.length || botCommits.length ? "clean_branch_replay" : "semantic_resolution") : "manual_required",
    base: safeString(input.base || input.base_branch || "main", 128),
    head: safeString(input.head || input.head_branch || "", 256),
    summary: { file_count: files.length, conflict_count: files.filter((file) => file.conflict).length, generated_count: generatedFiles.length, manual_required_count: manualRequired.length, bot_commit_count: botCommits.length, strategies: unique(files.map((file) => file.strategy)) },
    files,
...
export function buildRepoConflictPlan(input = {}) {
  const analysis = input.analysis?.classification ? sanitize(input.analysis) : analyzeRepoConflict(input);
  const steps = [];
