...
assert(routeSource.includes("content_preview"));
assert(!routeSource.includes("SELECT * FROM"));
assert.equal(_testingResourceApiRoutes.descriptor("sessions").table, "customer_sessions");
assert.equal(_testingResourceApiRoutes.decodeToken(_testingResourceApiRoutes.encodeToken({ session_id: "s1", created_at: "2026-01-01" }, _testingResourceApiRoutes.RESOURCES.sessions)).id, "s1");
