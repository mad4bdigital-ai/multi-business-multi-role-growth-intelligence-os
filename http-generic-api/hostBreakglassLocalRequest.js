...
export const HOST_BREAKGLASS_LOCAL_REQUEST_CONTRACT = "mad4b.host-breakglass-local-request.v1";
const SHA40_RE = /^[0-9a-f]{40}$/u;
const SHA256_RE = /^[0-9a-f]{64}$/u;
const SAFE_ID_RE = /^[A-Za-z0-9._:-]{1,160}$/u;
const ENVIRONMENT_KEY = "staging_local_windows_docker";
const OPERATION_KEY = "database.repair";
const RUNBOOK_KEY = "database.access_repair";
const TARGET_SOURCE = "staging_local_role_env";
...
function baseRequestFromPlan(plan = {}) {
  if (plan.environment_key !== ENVIRONMENT_KEY) fail("host_breakglass_local_request_environment_denied", "Verified local requests are restricted to Staging Windows/Docker.");
  if (plan.operation_key !== OPERATION_KEY || plan.runbook_key !== RUNBOOK_KEY) fail("host_breakglass_local_request_runbook_denied", "Verified local requests are restricted to database.access_repair.");
  ...
  if (plan.target_source !== TARGET_SOURCE) fail("host_breakglass_local_request_target_source_denied", "Verified local access repair must use staging_local_role_env.");
  ...
  return {
    contract: HOST_BREAKGLASS_LOCAL_REQUEST_CONTRACT,
...
export function verifyHostBreakglassLocalRequest(request = {}) {
  ...
  if (request.contract !== HOST_BREAKGLASS_LOCAL_REQUEST_CONTRACT || request.secrets_included !== false) {
    fail("host_breakglass_local_request_contract_invalid", "Verified local request contract is invalid.");
  }
  if (request.environment_key !== ENVIRONMENT_KEY || request.operation_key !== OPERATION_KEY || request.runbook_key !== RUNBOOK_KEY || request.target_source !== TARGET_SOURCE) {
    fail("host_breakglass_local_request_scope_invalid", "Verified local request escaped the fixed Staging access-repair scope.");
  }
...
export const __hostBreakglassLocalRequestTest = Object.freeze({ digest });
