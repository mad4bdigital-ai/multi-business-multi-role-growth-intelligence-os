...
function newReadbackId() { return `session_insight_target_write_readback_${randomUUID()}`; }
...
export async function createSessionInsightTargetWriteReadback({ pool = getPool(), input = {} } = {}) {
  ...
  const readbackId = newReadbackId();
  await pool.query(
    ...
    [
      ...
      ctx.target_surface,
      ctx.promotion_type,
      result.valid_target_write_readback ? "target_write_readback_passed" : "target_write_readback_failed",
      result.checks.target_item_exists ? 1 : 0,
      result.checks.target_link_matches ? 1 : 0,
      result.checks.source_payload_matches ? 1 : 0,
      result.checks.target_write_executed ? 1 : 0,
      asNumber(ctx.duplicate_target_write_count),
      asNumber(ctx.duplicate_target_item_count),
      JSON.stringify(result),
      JSON.stringify(safety),
      clean(input.created_by || input.createdBy, "session_insight_target_write_readback_tool"),
    ]
  );
...
export async function listSessionInsightTargetWriteReadbacks({ pool = getPool(), filters = {} } = {}) {
  ...
  return {
    ok: true,
    count: rows.length,
    readbacks: rows.map(sanitize),
    issues: issues.map((row) => ({ issue_code: row.issue_code, severity: row.severity, count: asNumber(row.count) })),
    policy: {
      readback_only: true,
      target_item_modified_by_readback: false,
      rollback_executed: false,
