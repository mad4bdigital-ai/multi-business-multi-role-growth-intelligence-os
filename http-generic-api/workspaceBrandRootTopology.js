...
async function bindOperationalWorkspaceToRoot(connection, { tenantId, brandWorkspaceId, rootWorkspaceId, actorUserId }) {
  ...
  if (Number(updateResult?.affectedRows || 0) !== 1) {
    throw topologyError(409, "workspace_brand_operational_workspace_update_conflict", "Brand operational workspace Root binding could not be updated exactly once.");
  }
  return nextConfig;
}
...
async function archiveLegacyOperationalParent(connection, row) {
  const [relationshipResult] = await connection.query(
  ...
  if (Number(relationshipResult?.affectedRows || 0) !== 1) {
    throw topologyError(409, "workspace_brand_legacy_parent_archive_conflict", "Legacy operational Brand parent could not be retired exactly once.", [{ relationship_id: row.relationship_id }]);
  }
  await connection.query(
...
async function loadRootBrandPairRows(connection, { tenantId, rootContainerId, brandContainerId }) {
  const [rows] = await connection.query(
    `SELECT relationship_id, tenant_id, status, from_container_id, to_container_id, relationship_type_key,
      ...
      FOR UPDATE`,
    [tenantId, rootContainerId, brandContainerId]
  );
  return Array.isArray(rows) ? rows : [];
}
...
async function ensureRootBrandRelationship(connection, { tenantId, rootWorkspace, brandWorkspace, rootContainer, brandContainer, actorUserId }) {
  ...
  const pairRows = await loadRootBrandPairRows(connection, {
    tenantId,
    rootContainerId: rootContainer.container_id,
    brandContainerId: brandContainer.container_id,
  });
  const nonCanonicalPairRows = pairRows.filter((row) => row.relationship_id !== relationshipId);
  if (nonCanonicalPairRows.length > 0) {
    throw topologyError(409, "workspace_brand_root_relationship_noncanonical", "Root-to-Brand relationship exists with a non-canonical identity and must be reconciled before Brand creation.", nonCanonicalPairRows.map((row) => ({ relationship_id: row.relationship_id, status: row.status, created_by: row.created_by || null })));
  }
  ...
  if (desiredParents.length === 1 && desiredParents[0].relationship_id !== relationshipId) {
    throw topologyError(409, "workspace_brand_root_relationship_noncanonical", "Active Root-to-Brand relationship does not use the canonical deterministic identity.", [{ relationship_id: desiredParents[0].relationship_id, expected_relationship_id: relationshipId }]);
  }
  ...
  const readback = await readCanonicalRootBrandRelationship(connection, relationshipId);
  if (!readback || Number(readback.currently_effective) !== 1) {
    throw topologyError(409, "workspace_brand_root_relationship_readback_invalid", "Created canonical Root-to-Brand relationship failed same-transaction readback.", [{ relationship_id: relationshipId }]);
  }
  return readback;
}
