...
const core = readFileSync(new URL("./capabilityResolutionEnvelopeGuardCore.js", import.meta.url), "utf8");
...
assert.match(core, /export function extractCapabilityEnvelopeId/);
assert.match(core, /export async function resolveCapabilityExecutionEnvelope/);
assert.match(core, /capability_resolution_envelope_ledger/);
assert.match(core, /ready_for_dispatch/);
assert.match(core, /dispatch_allowed/);
assert.match(core, /approval_required/);
assert.match(core, /blocking_gap_count/);
assert.match(core, /capability_resolution_envelope_commit_mismatch/);
assert.match(core, /capability_resolution_envelope_workspace_mismatch/);
assert.match(core, /capability_resolution_envelope_brand_mismatch/);
assert.match(core, /capability_resolution_envelope_resource_uri_mismatch/);
assert.match(core, /capability_resolution_envelope_binding_sha256_mismatch/);
assert.match(core, /capability_resolution_envelope_capability_sha256_mismatch/);
assert.match(core, /export async function markCapabilityEnvelopeReferenced/);
assert.match(core, /secrets_included: false/);
assert.doesNotMatch(helper + core, /decryptToken|value_ciphertext|encrypted_credentials|oauth_token|private_key/i);
assert.doesNotMatch(helper + core, /fetch\(|axios|child_process|exec\(|spawn\(/);
