...
const migration = readFileSync("migrations/264_sprint68_governed_repository_intelligence_engine.sql", "utf8");
...
includesAll(migration, [
  ...
  "repo.pr.comment_recommendation",
...
includesAll(migration, [
  ...
  "same_cycle_validation_for_sensitive_mutation",
  "dry_run_plan_only",
  "auto_merge",
  "force_push",
  "migration_apply",
...
assert.equal(repoResolved.resource_ref.owner, "mad4bdigital-ai");
assert.equal(repoResolved.resource_ref.repo, "multi-business-multi-role-growth-intelligence-os");
...
const objectPrResolved = resolveResourceRefInput({
  resource_type: "github_pull_request",
  resource_ref: { owner: "mad4bdigital-ai", repo: "multi-business-multi-role-growth-intelligence-os", pr_number: 1061 },
});
assert.equal(objectPrResolved.resource_type, "github_pull_request");
assert.equal(objectPrResolved.resource_uri, "github://mad4bdigital-ai/multi-business-multi-role-growth-intelligence-os/pr/1061");

includesAll(releaseReadiness, [
  "264_sprint68_governed_repository_intelligence_engine.sql",
  "governed_repository_intelligence_engine_policy_v1",
], "release readiness must enforce repository intelligence policy and migration");

assert(
  manifest.includes("node test-governed-repository-intelligence-engine.mjs"),
  "test manifest must include governed repository intelligence engine test"
);
