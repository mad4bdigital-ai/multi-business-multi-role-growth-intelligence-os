...
export async function readSupportTicketExternalDeliveryOrchestrationReadiness(input = {}) {
  ...
  let readiness = null;
  try {
    const [rows] = await pool.query(`SELECT * FROM v_platform_orchestration_support_ticket_external_delivery_readiness LIMIT 1`);
    readiness = rows[0] || null;
  } catch { readiness = null; }

  const [recentLinks] = await pool.query(
    `SELECT link_id, ticket_id, tenant_id, plan_id, run_id, approval_hold_id, relationship, status, evidence_json, created_at, updated_at FROM ticket_workflow_links WHERE relationship LIKE 'external_%'${tenantClause} ORDER BY updated_at DESC LIMIT ?`,
    [...tenantParams, limit]
  );
  const [recentApprovals] = await pool.query(
    `SELECT hold_id, tenant_id, workspace_id, workspace_key, hold_type, requested_by, user_id, actor_type, brand_key, request_id, status, decision_by, expires_at, decided_at, created_at FROM approval_holds WHERE (hold_type LIKE '%external%' OR request_id LIKE '%external%')${tenantClause} ORDER BY created_at DESC LIMIT ?`,
    [...tenantParams, limit]
  );
  const [recentCredentialIntake] = await pool.query(
    `SELECT intake_id, tenant_id, channel, audience, status, credential_ref, approval_hold_id, expires_at, created_at, updated_at FROM credential_intake_sessions WHERE 1=1${tenantClause} ORDER BY updated_at DESC LIMIT ?`,
    [...tenantParams, limit]
  ).catch(() => [[]]);
