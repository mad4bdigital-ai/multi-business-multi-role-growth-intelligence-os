...
write(apiRoot, "scripts/test-manifest.mjs", `export const tests = ["node test-tenant-dashboard.mjs", "node test-admin-runtime-verification.mjs"];`);
...
assert.equal(parseOpenApiOperations(fs.readFileSync(path.join(apiRoot, "openapi.yaml"), "utf8")).size, 5);
assert.equal(parseMountedRouteFiles(fs.readFileSync(path.join(apiRoot, "routes/index.js"), "utf8")).length, 2);
...
assert.equal(plan.coverage.mounted_family_count, 2);
assert.equal(plan.coverage.operation_count, 5);
