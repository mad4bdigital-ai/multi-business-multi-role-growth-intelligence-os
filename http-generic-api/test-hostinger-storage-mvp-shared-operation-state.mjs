...
function createAuthorization(fixture, nowEpoch) {
...
const nowEpoch = 1100;
const { operation, authorization } = createAuthorization(fixture, nowEpoch);
...
const capsule = createExecutionCapsule({
  ...
  contextRevision: 'mvp-shared-state-context-r1',
  ...
  connectionRef: 'mvp-shared-state-synthetic-connection',
  authorityPathRef: 'mvp-shared-state-authority-path',
  capabilityKey: OPERATION_KEY,
  authorityRevision: operation.ownership_revision,
  capabilityRevision: operation.policy_revision,
  registryRevision: 'mvp-shared-state-registry-r1',
  credentialReadinessRevision: 'mvp-shared-state-credential-r1',
  issuedAt: new Date(validationDate.getTime() - 60_000).toISOString(),
  expiresAt: new Date(validationDate.getTime() + 600_000).toISOString(),
...
const governanceDecision = Object.freeze({
  decisionRef: 'mvp-shared-state-governance-decision',
  decisionRevision: 'mvp-shared-state-governance-r1',
  operationKey: OPERATION_KEY,
  contextHash: capsule.contextHash,
  status: 'allowed',
...
for (const key of REQUIRED_DYNAMIC_EVIDENCE) {
  dynamicEvidence[key] = {
    ...
    revision: `${key}-mvp-shared-state-r1`,
    evidenceRef: `${key}-mvp-shared-state-evidence`,
    operationKey: OPERATION_KEY,
    contextHash: capsule.contextHash,
    tenantRef: capsule.tenantRef,
    workspaceRef: capsule.workspaceRef,
...
const beforeAggregate = fixture.repository.readAggregate(operation.operation_id);
assert(beforeAggregate);
const beforePlan = beforeAggregate.plans.find((row) => row.plan_id === fixture.plan.plan_id);
assert(beforePlan);
...
const runtime = createHostingerStorageTenantRuntime({
  ...
  async resolveExecutionContext({ operationId }) {
    assert.equal(operationId, operation.operation_id);
    return {
      capsule,
      governanceDecision,
      currentContext,
...
let response;
try {
  response = await fetch(`${app.url}/tenant/storage-operations/apply-plan`, {
    method: 'POST',
...
assert.equal(response.operation_id, operation.operation_id);
assert.equal(response.readback.operation_id, operation.operation_id);
assert.equal(response.readback.run_id, fixture.run_id);
assert.equal(response.readback.plan_id, fixture.plan.plan_id);
assert.equal(response.readback.synthetic_only, true);
assert.equal(response.readback.tenant_exclusive, true);
assert.equal(response.readback.provider_dispatch_allowed, false);
assert.equal(response.readback.production_ready, false);
assert.equal(enablementRegistry.exportState()[0].consumed, true);

const afterAggregate = fixture.repository.readAggregate(operation.operation_id);
assert(afterAggregate);
assert.equal(afterAggregate.operation.operation_id, operation.operation_id);
...
const afterPlan = afterAggregate.plans.find((row) => row.plan_id === fixture.plan.plan_id);
assert(afterPlan);
assert.equal(afterPlan.consumed, true);
assert.equal(afterPlan.consumed_run_id, fixture.run_id);
assert.deepEqual({
...
assert(afterAggregate.journals.every((row) => (
  ...
  && row.run_id === fixture.run_id
  && row.plan_id === fixture.plan.plan_id
)));
assert.equal(afterAggregate.journals.find((row) => row.phase === 'result').result, 'deleted');

assert.equal(afterAggregate.reconciliations.length, 1);
const reconciliation = afterAggregate.reconciliations[0];
assert.equal(reconciliation.operation_id, operation.operation_id);
assert.equal(reconciliation.run_id, fixture.run_id);
assert.equal(reconciliation.outcome, response.outcome);
assert.equal(reconciliation.result_digest, response.readback.result_digest);
assert.equal(response.readback.outcome, reconciliation.outcome);
assert.equal(response.readback.result_digest, reconciliation.result_digest);
assert.equal(fixture.adapter.exportState().items[0].exists, false);
...
console.log(JSON.stringify({
  ...
  same_repository_identity_preserved: executionPackage.repository === fixture.repository,
