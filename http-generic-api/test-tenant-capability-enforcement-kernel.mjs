...
for (const boundary of ["activation.skills.read", "platform.output-artifact.write", "content.wordpress.publish"]) {
  assert(kernelSource.includes(`"${boundary}"`), `${boundary} must be registered in the enforcement kernel`);
  assert(loop.includes(boundary), `${boundary} must be documented in the task loop evidence`);
}

for (const phrase of ["shadow_only", "provider_apply_allowed: false", "mutations_executed: false", "no_enforcement_cutover", "resolveTenantEffectiveCapability", "tenantEffectiveCapabilityReadinessSmoke"]) {
...
assert.deepEqual(
  TENANT_CAPABILITY_ENFORCEMENT_SYSTEM_TOOLS.map((tool) => tool.name),
  ["tenant_capability_enforcement_preview", "tenant_capability_enforcement_readiness_smoke"]
);

{
  const result = await tenantCapabilityEnforcementPreview({ capability_key: "files.object.read", boundary_key: "unknown.boundary" }, {});
  assert.equal(result.ok, false);
  assert.equal(result.enforcement_status, "blocked");
  assert.equal(result.provider_apply_allowed, false);
