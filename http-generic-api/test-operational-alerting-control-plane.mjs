...
function testStableKeysAndExecutionGrouping() {
  ...
  const grouped = _testingOperationalAlerts.groupExecutionRows([
    { id: 1, tenant_id: "tenant-1", workspace_id: "workspace-1", execution_status: "failed", entry_type: "workflow", app_key: "github", workflow_key: "ci", route_status: "failed", execution_trace_id_writeback: "trace-1", created_at: "2026-06-14T10:00:00.000Z" },
    { id: 2, tenant_id: "tenant-1", workspace_id: "workspace-1", execution_status: "failed", entry_type: "workflow", app_key: "github", workflow_key: "ci", route_status: "failed", execution_trace_id_writeback: "trace-2", created_at: "2026-06-14T11:00:00.000Z" },
  ]);
...
function testDedupeAndLifecyclePrecedence() {
  const live = _testingOperationalAlerts.candidate({ alertKey: "alert.test", sourceType: "execution_log", title: "Failure", reasonCode: "execution_failed", severity: "critical", occurrenceCount: 3, lifecycleStatus: "open", verificationState: "verified", lastSeenAt: "2026-06-14T12:00:00.000Z" });
  const persisted = _testingOperationalAlerts.candidate({ alertId: "alert-id-1", alertKey: "alert.test", sourceType: "execution_log", title: "Failure", reasonCode: "execution_failed", severity: "critical", occurrenceCount: 2, lifecycleStatus: "investigating", verificationState: "observed", persisted: true, lastSeenAt: "2026-06-14T11:00:00.000Z" });
  ...
  const resolvedPersisted = _testingOperationalAlerts.candidate({ alertId: "alert-id-2", alertKey: "alert.test", sourceType: "execution_log", title: "Failure", reasonCode: "execution_failed", severity: "critical", lifecycleStatus: "resolved", verificationState: "verified", persisted: true, lastSeenAt: "2026-06-14T11:00:00.000Z" });
...
function testRepositoryContracts() {
  ...
  for (const key of _testingOperationalAlerts.KNOWN_ISSUE_KEYS) assert.ok(migration.includes(key), `migration must seed ${key}`);
  for (const path of [
