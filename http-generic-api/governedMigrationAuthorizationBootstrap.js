...
function verifyMigrationExecutorApplyPolicy(row) {
  ...
  const exact =
    ...
    policy?.governed_runner_only === true &&
    policy?.checksum_bound_authorization_required === true &&
    policy?.typed_confirmation_required === true &&
    policy?.same_cycle_dry_run_required === true &&
    policy?.same_cycle_ledger_readback_required === true &&
    policy?.arbitrary_sql_forbidden === true &&
    policy?.provider_call_forbidden === true &&
    policy?.external_write_forbidden === true &&
    policy?.credential_payload_read_forbidden === true &&
