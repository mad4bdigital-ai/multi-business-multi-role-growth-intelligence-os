...
assert.deepEqual(classifyPath("http-generic-api/openapi-route-contracts.d/remote-runtime-target-catalog-readonly.yaml", classes).map((entry) => entry.id), ["shared_runtime"]);
...
assert.deepEqual(classifyPath("http-generic-api/scripts/maintenance-tools/repository-tool-lifecycle-guard.mjs", classes).map((entry) => entry.id), ["repository_governance"]);
assert.deepEqual(classifyPath("http-generic-api/scripts/test-repository-tool-lifecycle-guard.mjs", classes).map((entry) => entry.id), ["repository_governance"]);
