...
const STATES=new Set(["reserved","executing","known_not_applied","unknown_outcome","reconciled_no_mutation","succeeded"]);
const TRANSITIONS=Object.freeze({
  reserved:new Set(["executing"]),
  executing:new Set(["known_not_applied","unknown_outcome","succeeded"]),
  unknown_outcome:new Set(["reconciled_no_mutation","succeeded"]),
  known_not_applied:new Set(),reconciled_no_mutation:new Set(),succeeded:new Set(),
});
...
export function createFileCanonicalSemanticRepairLedger({directory,now=()=>new Date()}={}){
  ...
  return Object.freeze({
    async reserve({plan_sha256,expected_commit,artifact_sha256,precondition_fingerprint}={}){
      ...
      const record={contract:"mad4b.staging.canonical-semantic-repair-ledger.v2",plan_sha256:plan,expected_commit:String(expected_commit||""),artifact_sha256:String(artifact_sha256||""),precondition_fingerprint:String(precondition_fingerprint||""),state:"reserved",created_at:now().toISOString(),updated_at:now().toISOString(),mutation_retry_allowed:false,secrets_included:false};
    ...
    markKnownNotApplied:(plan,details)=>transition(plan,"known_not_applied",details),
    markReconciledNoMutation:(plan,details)=>transition(plan,"reconciled_no_mutation",details),
    read,
  });
}
