...
assert.match(dualModeCore, /Set-StagingEnvValue \\$envFile 'CLOUDFLARE_TUNNEL_ORIGIN_APP' 'http:\\/\\/127\\.0\\.0\\.1:8080'/);
assert.doesNotMatch(dualModeCore, /Set-StagingEnvValue \\$envFile 'CLOUDFLARE_TUNNEL_ORIGIN_APP' 'http:\\/\\/app:8080'/);
...
assert.match(launcher, /\$missing = @\(\$required \| Where-Object \{ -not \(Test-Path \(Join-Path \$dumpDir \$_\)\) \}\)/);
assert.match(launcher, /\$available = \(Test-Path \$dumpDir\) -and \(\$missing\.Count -eq 0\)/);
assert.doesNotMatch(launcher, /\(\$required \| Where-Object \{[^}]+\}\)\.Count/);
assert.ok(launcher.includes('$bundleManifestPath = Join-Path $dumpDir "staging-schema-bundle-manifest.json"'));
assert.ok(launcher.includes('return "skipped_invalid_schema_bundle"'));
assert.ok(launcher.includes('return "skipped_incompatible_schema_bundle"'));
assert.ok(launcher.includes('return "skipped_stale_schema_bundle"'));
assert.ok(launcher.includes('Schema bundle manifest is not bound to ExpectedCommit'));
assert.ok(launcher.includes('if ($RequireSchemaBundle -or $ApplySchemaBundle) { Fail "Schema bundle manifest is not bound to ExpectedCommit:'));
assert.ok(launcher.indexOf('return "skipped_stale_schema_bundle"') < launcher.indexOf('$clone = Join-Path $ScriptRoot "Clone-StagingDatabases.ps1"'));
