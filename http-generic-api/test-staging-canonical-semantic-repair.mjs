...
function executorFor(state){return{async query(sql){const source=String(sql);if(source.includes("information_schema.tables"))return[[{count:state.tableExists?1:0}]];
  if(source==="SELECT COUNT(*) AS count FROM workspace_registry")return[[{count:state.totalRows??state.candidateRows.length}]];
  if(source.includes("FROM workspace_registry"))return[state.candidateRows];throw new Error(`Unexpected query: ${sql}`);}};}
...
const result=await applyStagingCanonicalSemanticRepair({executor,plan,confirmation:plan.required_confirmation,actual_commit:commit,async apply_artifact(artifact){appliedArtifact=artifact;missingState.totalRows+=1;missingState.candidateRows=[canonical];}});
assert.equal(appliedArtifact.sha256,STAGING_CANONICAL_SEMANTIC_REPAIR_ARTIFACT.seed_sha256);assert.equal(result.status,"repaired");assert.equal(result.exact_row_count,1);
...
await assert.rejects(applyStagingCanonicalSemanticRepair({executor:staleExecutor,plan:stalePlan,confirmation:stalePlan.required_confirmation,actual_commit:commit,async apply_artifact(){throw new Error("must not execute");}}),
  (error)=>error?.code==="STAGING_CANONICAL_REPAIR_PRECONDITION_CHANGED");

const unknownState={tableExists:true,totalRows:7,candidateRows:[]};const unknownExecutor=executorFor(unknownState);
...
await assert.rejects(applyStagingCanonicalSemanticRepair({executor:unknownExecutor,plan:unknownPlan,confirmation:unknownPlan.required_confirmation,actual_commit:commit,async apply_artifact(){throw new Error("transport interrupted after dispatch");}}),
