...
const result=await applyStagingCanonicalSemanticRepair({executor,plan,confirmation:plan.required_confirmation,actual_commit:commit});
assert.deepEqual(missingState.transactionLog,["START TRANSACTION","COMMIT"]);assert.equal(result.artifact_sha256,STAGING_CANONICAL_SEMANTIC_REPAIR_ARTIFACT.seed_sha256);assert.equal(result.status,"repaired");assert.equal(result.exact_row_count,1);
...
await assert.rejects(applyStagingCanonicalSemanticRepair({executor:staleExecutor,plan:stalePlan,confirmation:stalePlan.required_confirmation,actual_commit:commit}),
...
await assert.rejects(applyStagingCanonicalSemanticRepair({executor:unknownExecutor,plan:unknownPlan,confirmation:unknownPlan.required_confirmation,actual_commit:commit}),
