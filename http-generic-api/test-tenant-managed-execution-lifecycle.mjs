...
import {
  assertManagedExecutionApprovalDecisionAuthority,
...
assert.throws(
  () => normalizeManagedExecutionEnvelope({ ...base, input_json: { api_token: "value" } }),
  (error) => error.code === "managed_execution_secret_field_rejected",
);
assert.throws(
  () => assertManagedExecutionPayloadSecretFree({ value: "Bearer abc.def.ghi" }),
  (error) => error.code === "managed_execution_secret_value_rejected",
);

const readGate = resolveManagedExecutionGate({ access_decision: "ALLOW_SELF_SERVE", effect_class: "read_only" });
assert.equal(readGate.requires_approval, false);
assert.equal(
  resolveManagedExecutionGate({ access_decision: "ALLOW_SELF_SERVE", effect_class: "destructive" }).hold_type,
  "supervisor_approval",
);
assert.equal(
  resolveManagedExecutionGate({ access_decision: "ROUTE_TO_MANAGED_SERVICE", effect_class: "read_only" }).hold_type,
  "managed_handoff",
);

function authorityConnection({ permission = "operate", dispatchAllowed = 1, runtimeStatus = "certified" } = {}) {
  return {
    async query(sql) {
      if (sql.includes("v_platform_capabilities_effective_evidence")) {
        return [[{
          ...
          requires_readback: 1,
          evidence_ref: "tool_dispatch_binding:binding-1:readback:wordpress-health",
        }]];
      }
...
const authority = await resolveManagedExecutionAuthority({ connection: authorityConnection(), envelope });
assert.equal(authority.resource_grant.grant_id, "grant-1");
assert.equal(authority.resource_grant.exact_resource, true);
await assert.rejects(
  resolveManagedExecutionAuthority({ connection: authorityConnection({ dispatchAllowed: 0 }), envelope }),
  (error) => error.code === "managed_execution_capability_dispatch_blocked",
);
await assert.rejects(
  resolveManagedExecutionAuthority({
    connection: authorityConnection({ permission: "view" }),
    envelope: normalizeManagedExecutionEnvelope({ ...base, effect_class: "destructive" }),
  }),
...
assert.equal(assertManagedExecutionStepEligibility({ run: managedRun, holds: [] }).allowed, true);

const approvalSnapshot = buildManagedAuthoritySnapshot({
  envelope: normalizeManagedExecutionEnvelope({ ...base, effect_class: "destructive" }),
  access: { decision: "ALLOW_SELF_SERVE", service_mode: "self_serve" },
  gate: resolveManagedExecutionGate({ access_decision: "ALLOW_SELF_SERVE", effect_class: "destructive" }),
  authority,
});
assert.throws(
  () => assertManagedExecutionStepEligibility({
    run: {
      ...managedRun,
      status: "running",
      execution_context_json: JSON.stringify({ contract: "tenant-managed-execution-v1", authority_snapshot: approvalSnapshot }),
    },
    holds: [],
  }),
  (error) => error.code === "managed_execution_approval_evidence_missing",
);
assert.equal(assertManagedExecutionStepEligibility({
  run: {
    ...managedRun,
    status: "running",
    execution_context_json: JSON.stringify({ contract: "tenant-managed-execution-v1", authority_snapshot: approvalSnapshot }),
...
assert.equal(assertManagedExecutionApprovalDecisionAuthority({
  hold: { required_role: "certified_reviewer" },
  decisionAuthority: { roles: ["supervisor"] },
}), true);
assert.throws(
  () => assertManagedExecutionApprovalDecisionAuthority({
    hold: { required_role: "supervisor" },
    decisionAuthority: { roles: ["certified_reviewer"] },
  }),
  (error) => error.code === "managed_execution_approver_role_required",
);
assert.equal(assertManagedExecutionApprovalDecisionAuthority({
  hold: { required_role: "supervisor" },
  decisionAuthority: { is_admin: true },
}), true);

assert.equal(assertManagedExecutionTransition({ current_status: "pending", next_status: "running" }), true);
assert.throws(
  () => assertManagedExecutionTransition({ current_status: "completed", next_status: "running" }),
...
const projection = projectManagedExecutionState({
  ...
  binding: { task_ticket_id: "task-2", lifecycle_state: "verified" },
});
assert.equal(projection.lifecycle_state, "reconciliation_required");
assert(projection.contradictions.includes("run_active_while_approval_open"));
assert(projection.contradictions.includes("terminal_run_has_running_steps"));
...
const source = [
  ...
  "managedExecutionLifecycleService.js",
  "routes/workflowOrchestrationRoutes.js",
].map((file) => readFileSync(file, "utf8")).join("\n");
const migration = readFileSync("migrations/1043_sprint69_tenant_managed_execution_lifecycle.sql", "utf8");
for (const contract of [
  ...
  "managed_execution_approver_role_required",
...
]) assert(migration.includes(object), `missing migration contract: ${object}`);
