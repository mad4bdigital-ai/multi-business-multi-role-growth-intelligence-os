...
const verificationRefresh = readJson(
  '.github/contracts/spec014/hostinger-storage-wave4-signed-schema-verification-refresh.json',
);
...
const waves = [
  {
    ...
    expected_tables: ['storage_provider_accounts', 'storage_targets', 'storage_target_bindings', 'storage_pressure_snapshots'],
    dependency: null,
  ...
  },
  {
    ...
    expected_tables: ['storage_cleanup_operations', 'storage_cleanup_plans', 'storage_cleanup_plan_items', 'storage_cleanup_plan_impacts', 'storage_cleanup_approvals', 'storage_execution_leases'],
  ...
  },
  {
    ...
    expected_tables: ['storage_cleanup_runs', 'storage_cleanup_run_items', 'storage_reconciliation_results', 'storage_emergency_reserves', 'storage_pressure_incidents'],
    expected_views: ['v_storage_admin_target_readiness', 'v_storage_tenant_target_readiness', 'v_storage_cleanup_operation_readback'],
    expected_tools: ['hostinger_storage_snapshot_read', 'hostinger_storage_plan_inspect', 'hostinger_storage_plan_apply'],
    dependency: '20260802_02_spec014_hostinger_storage_control_plane.sql',
    expected_preflight_status: 'warn',
    expected_risk_codes: ['create_view_without_or_replace', 'create_view_without_or_replace', 'create_view_without_or_replace', 'insert_without_ignore_or_on_duplicate'],
  },
  {
    id: 4,
    ...
    expected_tables: ['storage_authorized_injection_states', 'storage_authorized_injection_rollbacks'],
...
assert.equal(metadata.readback_contract_key, 'spec014_hostinger_storage_migration_readback_v4');
assert.equal(metadata.schema_verification_attestation_version, 'spec014-hostinger-storage-schema-verification-v2');
...
assert.equal(promotionContract.contract, 'spec014.hostinger-storage-durable-authorized-injection-runtime-sequence-promotion.v1');
assert.equal(promotionContract.candidate_source.pull_request, 4879);
assert.equal(promotionContract.candidate_source.integration_merge_sha, '4ca8d8691baa66210b679da51c5d92909c88c14c');
...
assert.equal(promotionContract.terminal_boundary.signed_schema_verification_contract_refreshed, true);
assert.equal(promotionContract.terminal_boundary.same_cycle_readback_contract_refreshed, true);
assert.equal(promotionContract.terminal_boundary.schema_verification_attestation_version, 'spec014-hostinger-storage-schema-verification-v2');
...
assert.equal(verificationRefresh.terminal_boundary.signed_schema_verification_contract_refreshed, true);
assert.equal(verificationRefresh.terminal_boundary.live_database_access_performed, false);
assert.equal(verificationRefresh.terminal_boundary.signature_created, false);

const report = {
  gate: 'hostinger_storage_runtime_migration_promotion',
  generated_at: new Date().toISOString(),
  waves: [],
  schema_verification_contract_refreshed: true,
  readback_contract_key: metadata.readback_contract_key,
  schema_verification_attestation_version: metadata.schema_verification_attestation_version,
...
for (const wave of waves) {
  ...
  if (wave.expected_views) {
    ...
    for (const tool of wave.expected_tools) assert.ok(sql.includes(`'${tool}'`), `${filename}: missing disabled tool ${tool}`);
    assert.equal((sql.match(/\n\s*0,\n\s*36[0-2]\n\)/gu) || []).length, 3, `${filename}: tools must remain disabled`);
    assert.equal(metadata.apply_blockers.length >= 6, true, `${filename}: explicit apply blockers required`);
  }
...
assert.equal(promotionContract.promotion.dependency, reports[2].migration);
assert.equal(promotionContract.promotion.dependency_checksum_sha256, reports[2].checksum_sha256);
assert.equal(metadata.apply_blockers.some((item) => item.includes('readback v4')), true);
assert.equal(metadata.apply_blockers.some((item) => item.includes('schema-verification v2')), true);
assert.equal(metadata.apply_blockers.some((item) => item.includes('must be refreshed')), false);
