...
for (const generatedEvidence of [
  ...
  assert(workflow.includes(generatedEvidence), `safe generated evidence must be explicitly classified: ${generatedEvidence}`);
}
assert(!workflow.includes("docs/surface-contract-*.json)"), "auto-merge must not accept an unbounded generated JSON wildcard");
...
assert(workflow.includes("--jq '.allow_auto_merge'"), "auto-sync must read back the repository auto-merge capability before registration");
